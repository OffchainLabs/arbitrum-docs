...
export async function convertExcalidrawToReactFlow(
  ...
  for (const el of elements) {
    ...
    if (el.type === 'arrow') {
      ...
      const hoverKey = getCustomString(el.customData, 'hoverContent');
      // customData.edgeType lets authors opt an arrow out of orthogonal routing
      // (e.g. "straight" draws one direct stroke between the two anchors)
      const edgeType =
        getCustomString(el.customData, 'edgeType') === 'straight' ? 'straight' : 'smoothstep';
      ...
      const edge: Edge = {
        ...
        type: hoverKey ? 'hoverEdge' : edgeType,
        animated: false,
        style: { strokeWidth: 1.5, stroke: '#969696' },
        // Render edges above nodes so arrows stay visible when they terminate on a
        // node drawn inside a larger container node (e.g., contracts on the parent chain)
        zIndex: 1,
      };

      if (isArrowhead(el.endArrowhead)) {
        edge.markerEnd = { type: MarkerType.ArrowClosed, color: '#969696' };
      }
      if (isArrowhead(el.startArrowhead)) {
        edge.markerStart = { type: MarkerType.ArrowClosed, color: '#969696' };
      }
