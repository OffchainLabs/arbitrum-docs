...
function generateNavigationSummary(analysis, sidebarData) {
  ...
  sections.push(`- **Total Sidebars**: ${navAnalysis.metrics?.totalSidebars || 0} navigation sections`);
  sections.push(`- **Navigation Entries**: ${navAnalysis.metrics?.totalEntries || 0} total items`);
  sections.push(`- **Categories**: ${navAnalysis.metrics?.totalCategories || 0} content categories`);
  sections.push(`- **Maximum Depth**: ${navAnalysis.metrics?.maxDepth || 0} levels deep`);
  sections.push(`- **External Links**: ${navAnalysis.metrics?.externalLinks || 0} outbound references`);
  sections.push('');
  ...
  sections.push(`- **Documents in Navigation**: ${coverage.inNavigation || 0} out of ${coverage.totalDocuments || 0} total`);
  ...
  if (coverage.duplicateEntries > 0) {
    sections.push(`- **Duplicate Entries**: ${coverage.duplicateEntries} documents appear in multiple sidebars`);
  }
  if (coverage.accuracy) {
    sections.push(`- **Orphaned Rate**: ${coverage.accuracy.orphanedRate}% of documents lack navigation access`);
    if (parseFloat(coverage.accuracy.duplicateRate) > 0) {
      sections.push(`- **Duplication Rate**: ${coverage.accuracy.duplicateRate}% of navigation entries are duplicates`);
    }
  }
  sections.push('');
  ...
  if (navAnalysis.hierarchyBalance && typeof navAnalysis.hierarchyBalance.overallScore === 'number' && !isNaN(navAnalysis.hierarchyBalance.overallScore)) {
    const balanceScore = Math.min(Math.max(navAnalysis.hierarchyBalance.overallScore, 0), 1); // Clamp between 0 and 1
    sections.push(`- **Structure Balance**: ${(balanceScore * 100).toFixed(1)}% (${balanceScore >= 0.7 ? 'Good' : 'Needs Improvement'})`);
  }
  if (navAnalysis.quality && typeof navAnalysis.quality.overallScore === 'number' && !isNaN(navAnalysis.quality.overallScore)) {
