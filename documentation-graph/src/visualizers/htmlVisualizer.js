...
export class HtmlVisualizer {
  ...
  async generateVisualization(graphData, analysis, outputPath, customOptions = {}) {
    logger.section('Interactive Visualization Generation');
    
    await fs.ensureDir(path.dirname(outputPath));
    
    const html = this.generateHtmlContent(graphData, analysis, customOptions);
  ...
  generateHtmlContent(graphData, analysis, customOptions = {}) {
    const template = this.getHtmlTemplate(customOptions);
    const graphDataJson = JSON.stringify(graphData, null, 2);
    const analysisJson = JSON.stringify(analysis, null, 2);
    const optionsJson = JSON.stringify(customOptions, null, 2);
    
    return template
      .replace('{{GRAPH_DATA}}', graphDataJson)
      .replace('{{ANALYSIS_DATA}}', analysisJson)
      .replace('{{CUSTOM_OPTIONS}}', optionsJson)
      .replace('{{TIMESTAMP}}', new Date().toISOString())
      .replace('{{TITLE}}', customOptions.title || 'Arbitrum Documentation Knowledge Graph')
      .replace('{{SUBTITLE}}', customOptions.subtitle || 'Interactive visualization of documentation structure, concepts, and relationships')
  ...
  getHtmlTemplate(customOptions = {}) {
    const title = customOptions.title || 'Arbitrum Documentation Knowledge Graph';
    const subtitle = customOptions.subtitle || '';
...
<head>
    ...
    <script>
        // Load extensions after core libraries are ready
        window.addEventListener('load', function() {
            const scripts = [
                'https://unpkg.com/cytoscape-dagre@2.5.0/cytoscape-dagre.js',
                'https://unpkg.com/cose-base@1.0.3/cose-base.js',
            ...
            function checkAllLoaded() {
                ...
                if (loadedCount === totalScripts) {
                    console.log('All extension scripts loaded');
            ...
            scripts.forEach(src => {
                ...
                script.onerror = function() {
                    ...
                    checkAllLoaded(); // Still count as "loaded" to continue
                };
...
<body>
        ...
        <div class="sidebar">
            ...
            <div class="controls">
                <div class="control-group">
                    <h3>Layout</h3>
                    <button class="btn" onclick="setLayout('fcose')">fCoSE</button>
                    <button class="btn" onclick="setLayout('cose-bilkent')">COSE</button>
                    <button class="btn" onclick="setLayout('dagre')">Dagre</button>
    ...
    <script>
        ...
        let visibleEdgeTypes = new Set(['contains', 'mentions', 'links_to', 'co_occurs']);
        let availableExtensions = [];

        // Register Cytoscape extensions with proper error handling
        function registerExtensions() {
            ...
            try {
                if (typeof cytoscape === 'undefined') {
                    console.error('Cytoscape core library not loaded');
                ...
                if (typeof dagre !== 'undefined' && typeof cytoscapeDagre !== 'undefined') {
                    try {
                        ...
                        registeredExtensions.push('dagre');
                        console.log('✓ Dagre layout registered');
                    } catch (error) {
                        console.warn('Failed to register dagre layout:', error);
                    }
                }

                // Register cose-bilkent layout  
                if (typeof window.cytoscapeCoseBilkent !== 'undefined') {
                    try {
                        cytoscape.use(window.cytoscapeCoseBilkent);
                        registeredExtensions.push('cose-bilkent');
                ...
                if (typeof window.cytoscapeFcose !== 'undefined') {
                    try {
                        ...
                        registeredExtensions.push('fcose');
                        console.log('✓ fCoSE layout registered');
                    } catch (error) {
                        console.warn('Failed to register fcose layout:', error);
                    }
                }

                console.log('Extension registration complete. Available layouts:', registeredExtensions);
                return registeredExtensions;
        ...
        window.initializeVisualization = function() {
            // Mark as called to prevent duplicate initialization
            window.initializeVisualization.__called = true;
            
            // Validate data before initialization
            if (!graphData || !graphData.nodes || !graphData.edges) {
                console.error('Invalid graph data provided');
                document.getElementById('loading').innerHTML = 'Error: Invalid graph data';
                return;
            }
            
            if (!analysisData) {
                console.warn('No analysis data provided, using defaults');
                analysisData = { quality: { overallScore: 0, issues: [], strengths: [] }, recommendations: [] };
            }

            try {
                console.log('Starting visualization initialization...');
                
                // Register extensions and track what's available
                availableExtensions = registerExtensions();
                
                // Initialize Cytoscape
                initializeCytoscape();
                loadRecommendations();
                
                // Apply a better initial layout after initialization
                setTimeout(function() {
                    if (cy && cy.nodes().length > 0) {
                        // Try layouts in order of preference
                        if (availableExtensions.includes('fcose')) {
                            setLayout('fcose');
                        } else if (availableExtensions.includes('cose-bilkent')) {
                            setLayout('cose-bilkent');
                        } else if (availableExtensions.includes('dagre')) {
                            setLayout('dagre');
                        } else {
                            setLayout('grid');
                        }
                ...
                document.getElementById('loading').innerHTML = 'Error: Failed to initialize visualization';
            }
        };

        // Fallback initialization if scripts don't load via the dynamic approach
        document.addEventListener('DOMContentLoaded', function() {
            // Wait a bit for dynamic script loading, then fallback
            setTimeout(function() {
                if (!window.initializeVisualization.__called) {
        ...
        function setLayout(layoutName) {
            ...
            // Check if layout is available using our tracked extensions
            const isAvailable = 
                availableExtensions.includes(layoutName) || 
                layoutName === 'circle' || 
                layoutName === 'grid';

            // Fall back to available layout if requested layout is not available
            if (!isAvailable) {
                console.warn('Layout "' + layoutName + '" not available, falling back to grid');
                layoutName = 'grid';
            }
            ...
            try {
                const layout = cy.layout(layouts[layoutName] || layouts['grid']);
                layout.run();
            } catch (error) {
                console.error('Error applying layout:', error);
                // Fall back to basic grid layout
        ...
        function highlightConnected(node) {
            if (!cy || !node) {
                console.error('Cytoscape not initialized or invalid node');
                return;
            }
            try {
                clearHighlights();
        ...
        function highlightHubs() {
            ...
            try {
                ...
                console.error('Error highlighting hubs:', error);
            }
        }
        ...
        function highlightOrphans() {
            ...
            try {
                ...
                console.error('Error highlighting orphans:', error);
            }
        }
        ...
        function showConceptClusters() {
            ...
            try {
                ...
                console.error('Error showing concept clusters:', error);
            }
        }
        ...
        function searchNodes() {
            ...
            if (!cy) {
                console.error('Cytoscape not initialized');
                results.innerHTML = '<div class="search-result">Graph not loaded</div>';
                return;
            }
            ...
            try {
                ...
                results.innerHTML = html || '<div class="search-result">No results found</div>';
            } catch (error) {
