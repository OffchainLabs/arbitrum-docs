...
- `eth_sendRawTransactionConditional` submits a raw signed transaction with extra preconditions the sequencer must verify before accepting it. If any condition fails, the tx is rejected with JSON-RPC error -32003 (rejected).

| `ConditionalOptions` payload        | Description                                |
| ----------------------------------- | ------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `knownAccounts`                     | map of address → (storageRootHash          | { `slot: expectedValue` }). The tx is rejected unless each account's storage root matches, or each listed slot still holds the expected value. |
| `blockNumberMin` / `blockNumberMax` | L1 block-number window the tx is valid in. |
