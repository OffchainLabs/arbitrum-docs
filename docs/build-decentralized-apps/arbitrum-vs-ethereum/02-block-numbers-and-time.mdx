...
- **Parent chain block numbers** (accessed via `block.number`): Updated approximately every 13 to 15 seconds (can take longer) but will not reflect the exact parent chain block at the time of your transaction.
