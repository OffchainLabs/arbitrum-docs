...
Arbitrum does not keep a long-lived "pending" mempool the way an Ethereum node does. Transactions submitted to the [Sequencer](/how-arbitrum-works/deep-dives/sequencer.mdx) are ordered and executed almost immediately, so out-of-order nonces cannot wait minutes or hours for their predecessor to arrive — they are held only briefly, then rejected.
...
- **On Ethereum**, `nonce too high` usually means the gap from the sender's current state nonce is larger than the pending pool can or will buffer (often a multi-thousand-slot gap, configurable per client).
- **On Arbitrum**, `nonce too high` simply means the predecessor transaction did not arrive at the sequencer within the retry window.
...
On Arbitrum, `eth_getTransactionCount(address, "pending")` returns the same value as `eth_getTransactionCount(address, "latest")`. The sequencer's retry buffer is internal sequencer state and is not exposed through the RPC interface, so there is no way for an external caller to observe whether a higher-nonce transaction is currently being held. Services that previously relied on `"pending"` to coordinate concurrent submitters need to track outstanding nonces themselves rather than fetching them from the RPC.

## Further reading

- [RPC methods](/build-decentralized-apps/arbitrum-vs-ethereum/03-rpc-methods.mdx) — full list of RPC method differences between Arbitrum and Ethereum.
- [The Sequencer and Censorship Resistance](/how-arbitrum-works/deep-dives/sequencer.mdx) — how the sequencer orders and processes transactions in Arbitrum.
