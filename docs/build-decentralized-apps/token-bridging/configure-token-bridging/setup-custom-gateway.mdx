...
To simplify the process, we’ll use Arbitrum’s SDK and call the `registerCustomToken` method of the `AdminErc20Bridger` class, which will call the `registerTokenOnL2` method of the token passed by parameter.
