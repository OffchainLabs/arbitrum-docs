...
A teleport deposit creates a chain of [retryable tickets](/build-decentralized-apps/04-cross-chain-messaging.mdx) that propagate your tokens through each layer:
...
const { Erc20L1L3Bridger, getArbitrumNetwork } = require('@arbitrum/sdk');
...
if (bridger.isCustomGasTokenChain) {
...
```javascript
const { Erc20L1L3Bridger } = require('@arbitrum/sdk');
...
```javascript
const { EthL1L3Bridger, getArbitrumNetwork } = require('@arbitrum/sdk');
...
Retryable tickets expire after their lifetime (default: 7 days). If a retryable expired, the tokens are sitting on L2 rather than L3. You can manually bridge them from L2 to L3 using the standard [deposit flow](/build-decentralized-apps/token-bridging/deposit-tokens.mdx).
...
- [Deposit tokens (L1 to L2)](/build-decentralized-apps/token-bridging/deposit-tokens.mdx)
- [Withdraw tokens (L2 to L1)](/build-decentralized-apps/token-bridging/withdraw-tokens.mdx)
- [Cross-chain messaging overview](/build-decentralized-apps/04-cross-chain-messaging.mdx)
