...
This how-to covers how gas operates in <a data-quicklook-from="arbitrum">Arbitrum</a>, how it's calculated, and how to estimate it before submitting transactions. For a deeper understanding of the underlying pricing mechanisms, see the [Gas and Fees](/how-arbitrum-works/deep-dives/gas-and-fees.mdx) concept page.
...
The gas limit includes the gas for child chain computation plus an additional buffer to cover the parent chain gas the <a data-quicklook-from="sequencer">Sequencer</a> pays when [posting the batch](/how-arbitrum-works/deep-dives/sequencer.mdx):
...
- **L1S** (Size of data to be posted on L1, in bytes): Depends on the transaction data. Arbitrum adds a fixed amount (~140 bytes) for the static part of the transaction.
  ...
  - For <a data-quicklook-from="arbitrum-nova">Arbitrum Nova</a> (AnyTrust), the data size is a fixed value since only the Data Availability Certificate (DAC) is posted on the parent chain, [as explained here](/how-arbitrum-works/deep-dives/anytrust-protocol.mdx#data-availability-certificates-dacert).
