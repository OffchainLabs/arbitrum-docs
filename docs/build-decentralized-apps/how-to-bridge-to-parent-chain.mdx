...
description: 'Step-by-step guide to programmatically withdraw ETH and send messages from an Arbitrum child chain to Ethereum (parent chain) using ArbSys and the Outbox contract.'
...
<ImageZoom
  ...
  alt="Process that funds follow during a withdrawal operation"
  className="img-800px"
/>

## Withdrawing **ERC-20** tokens

For **ERC-20** token withdrawals, see the dedicated [Withdraw tokens guide](/build-decentralized-apps/token-bridging/withdraw-tokens.mdx), which provides detailed instructions for using Arbitrum's canonical token <a data-quicklook-from="bridge">bridge</a>.
