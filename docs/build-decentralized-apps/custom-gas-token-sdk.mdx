...
- everything else is under the hood, and the custom gas token code paths will be executed just if the `ArbitrumNetwork` object config has a `nativeToken` field.
