...
<!-- waiting on language from Wen -->

This guide will... blah blah blah
...
1. **Create `.env` file**:
   ...
   (RPC URLs from official Arbitrum docs.)
...
2. **Update `foundry.toml`** (add chain IDs for verification, as Arbiscan requires them for non-Ethereum chains):
   ...
   (Chain IDs: Arbitrum Sepolia is 421614, Arbitrum One (mainnet) is 42161. API URLs from Arbiscan.)
