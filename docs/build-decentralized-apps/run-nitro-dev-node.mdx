...
While some teams use `nitro-testnode` for testing cross-layer messaging, which involves launching both <a data-quicklook-from="geth">Geth</a> as <a data-quicklook-from="parent-chain">parent chain</a> and Nitro as <a data-quicklook-from="child-chain">child chain</a>, this setup can be more complex and time-consuming. If your primary goal is to test contracts on a local node without needing cross-layer interactions, Nitro's `--dev` mode offers a lightweight and efficient alternative.
...
<VanillaAdmonition type="tip" title="Stylus contract testing">
...
In Nitro `--dev` mode, the default <a data-quicklook-from="chain-owner">chain owner</a> is set to `0x0000000000000000000000000000000000000000`. However, you can use the `ArbDebug` <a data-quicklook-from="precompile">precompile</a> to set the chain owner. This precompile includes the `becomeChainOwner()` function, which can be called to assume ownership of the chain.
...
- Modifying the computational <a data-quicklook-from="gas-target">gas target</a> and transaction gas limits
