...
Add WASM ([WebAssembly](https://webassembly.org/)) as a build target for the specific Rust toolchain you are using. The below example sets your default Rust toolchain to 1.88 or newer as well as adding the WASM build target:
...
5. The collection contract is then activated via ArbWasm [SME-VERIFY: confirm activation entrypoint name — activateProgram or new method for collections]. Activation reads all fragments through the collection, concatenates them, and decompresses the result.
