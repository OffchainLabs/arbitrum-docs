...
description: 'Learn how to debug Stylus transactions using cargo stylus replay, usertrace, and cross-environment tools.'
...
5. **Cross-environment debugging (Stylus ↔ Solidity)**: Debug transactions that cross Stylus and Solidity contracts by pairing StylusDB with SolDB through `soldb bridge`, enabling unified tracing and step-through across both environments.
...
### Cross-environment debugging (Stylus ↔ Solidity)

When debugging transactions that involve both Stylus and Solidity contracts, you can use these tools together:

- [StylusDB](https://github.com/walnuthq/stylusdb): provides visual function call trees and step-through debugging for Rust-based smart contracts.
- [SolDB](https://github.com/walnuthq/soldb): steps through Solidity execution, inspects variables, and debugs failing transactions.

Start the cross-environment bridge:
...
Debug Stylus transactions that call Solidity:
...
Debug Solidity transactions that call Stylus:

```shell
soldb trace <tx_hash> \
  --ethdebug-dir <address>:<name>:./out \
  --cross-env-bridge http://127.0.0.1:8765 \
...
- [SolDB Guide](https://github.com/walnuthq/soldb)
