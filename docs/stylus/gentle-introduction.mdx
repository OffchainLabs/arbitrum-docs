...
Contract size: As of ArbOS60, Stylus contracts are not bound by the 24 KB EVM contract code limit. As of ArbOS 60, the maximum size of a Stylus contract is 96 KB — four times the limit for Solidity contracts on the EVM. This is enabled by a fragment-based deployment model, see more (here)[/stylus/using-cli.md].
