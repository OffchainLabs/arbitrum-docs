...
To see how much gas contract initialization would cost, you need to call `programInitGas(address)` from the [ArbWasm precompile](/arbitrum-essentials/precompiles/reference#common-precompiles). This function returns both the initialization cost when the contract is cached and when it is not.


<VanillaAdmonition type="note">

Fragmented contracts for those over 24kb. Stylus contracts deployed across multiple fragments (see [contract size](/stylus/cli-tools/overview#deploying-contracts-larger-than-24-kb)) incur an additional activation cost roughly equivalent to one `EXTCODEREAD` per fragment, since ArbOS must read each fragment from state before concatenation and decompression. Caching the collection contract still benefits subsequent calls, as the cache stores the fully reconstructed and instrumented native code — not the underlying fragments.
