...
Fragmented contracts for those over 24kb. Stylus contracts deployed across multiple fragments (see contract size)[/stylus/using-cli.md] incur an additional activation cost roughly equivalent to one `EXTCODEREAD` per fragment, since ArbOS must read each fragment from state before concatenation and decompression. Caching the collection contract still benefits subsequent calls, as the cache stores the fully reconstructed and instrumented native code — not the underlying fragments.
