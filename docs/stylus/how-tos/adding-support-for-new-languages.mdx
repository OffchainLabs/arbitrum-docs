...
[Arbitrum Stylus](../gentle-introduction.mdx) is a new technology developed for Arbitrum chains which gives smart contract developers more tools. With Stylus, developers can write EVM-compatible smart contracts in many different programming languages, and reap massive performance gains. Stylus reduces fees, with performance gains ranging from 10-70x, and memory efficiency gains as high as 100-500x.
...
Let's build something a little bit fancier: this time we'll implement a primality checker in Zig using an ancient algorithm called the [sieve of erathosthenes](https://en.wikipedia.org/wiki/Sieve_of_Eratosthenes). Given a number, our contract will output 1 if it is prime, or 0 otherwise. We'll implement in a pretty naive way, while leveraging one of Zig's features: [comptime](https://kristoff.it/blog/what-is-zig-comptime/).
