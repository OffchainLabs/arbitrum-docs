...
description: 'Stylus Rust SDK VM variables and functions for blockchain context access'
author: chrisco
sme: chrisco
sidebar_position: 3
...
- For normal L2-to-L2 transactions, it behaves like EVM's `CALLER` opcode
...
Returns the original EOA (Externally Owned Account) that initiated the transaction, even through multiple contract calls.
...
See [Block Numbers and Time](https://developer.arbitrum.io/time) for more information on how this value is determined on Arbitrum.
...
Gets the account's code hash at the given address.
...
The code hash of an account without code will be the empty hash: `keccak("") = c5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470`.
...
Computes the units of gas for a specified amount of ink.
...
`crypto::keccak()` is the recommended approach as it's more ergonomic.
...
A maximum of four topics allowed. The first topic is typically the event signature hash.
...
Storage is cached for efficiency. Use the SDK's storage types instead of direct storage access.
...
Typically handled automatically by the SDK. Manual cache flushing is rarely needed.
...
The `#[entrypoint]` macro handles this automatically. Manual calls are not recommended and will unproductively consume gas.
