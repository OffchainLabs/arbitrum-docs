...
The Stylus SDK provides a robust testing framework that allows developers to write and run tests for their contracts directly in Rust without deploying to a blockchain. This guide walks you through writing and running tests for Stylus contracts using the built-in testing framework.
...
alloy-primitives = { version = "1.3", features = ["sha3-keccak"] }
...
1. **Test isolation**
   ...
   - Avoid relying on the state from previous tests
...
## Accessing host functions

Stylus contracts access host functions (block timestamp, msg sender, etc.) through the `HostAccess` trait's `.vm()` method:
...
This pattern makes contracts testable because `TestVM` implements the same trait, allowing your tests to mock blockchain state without deploying to a network.
