...
flowchart TB
    ...
    primitives --> uint["StorageUint8, u16, u32, u64..."]
    primitives --> bytes["StorageBytes, B1, B2, B3..."]
    primitives --> addr["StorageAddress"]
    primitives --> str["StorageString, B1...BN"]
    primitives --> signed["StorageSigned, I8..."]
    primitives --> fixed["StorageFixedBytes, B1...B32"]

    collections --> map["StorageMap&lt;T&gt;"]
    collections --> vec["StorageVec&lt;T&gt;"]
    collections --> arr["StorageArray&lt;T, N&gt;"]

    custom --> sol["sol_storage!()"]
    custom --> attr["#[storage] struct"]

    style storage fill:#e6ccff,stroke:#8b5cf6,stroke-width:2px
    style primitives fill:#bfdbfe,stroke:#3b82f6,stroke-width:2px
    style collections fill:#bbf7d0,stroke:#22c55e,stroke-width:2px
    style custom fill:#fed7aa,stroke:#f97316,stroke-width:2px
```
...
```mermaid
flowchart TB
    subgraph storage["Storage Slots (32 bytes each)"]
        slot0["Slot 0<br/>uint256 value"]
        slot1["Slot 1<br/>address (20 bytes) + uint96 (12 bytes)"]
        slot2["Slot 2<br/>Mapping base slot"]
        slot3["Slot 3<br/>keccak256(key || slot)"]
    end
    ...
    style slot0 fill:#cce5ff
    style slot1 fill:#ffcccc
    style slot2 fill:#e6ccff
    style slot3 fill:#ccffcc
    style mapping fill:#e6f2ff
```
