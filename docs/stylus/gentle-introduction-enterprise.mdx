---
id: gentle-introduction-enterprise
title: 'A gentle introduction to Stylus for enterprise architects'
sidebar_label: 'Introduction (enterprise)'
description: 'An introduction to Stylus for enterprise software architects coming from a Web2 and microservices background, with a focus on compute savings and leveraging existing Rust and C/C++ skills.'
user_story: 'As an enterprise software architect with a microservices background, I want to understand how Stylus lets me use my existing Rust or C/C++ skills to build onchain programs with significant compute savings.'
content_type: 'gentle-introduction'
author: gael-blanchemain
sme: amarrazza
target_audience: 'Enterprise software architects coming from Web2 and microservices backgrounds, familiar with Rust or C/C++ but not Solidity'
sidebar_position: 2
...
- Stylus lets you deploy compiled WASM modules to Arbitrum chains using languages like Rust and C++ — with their full ecosystems of libraries and tooling. No new language to learn; your existing Rust or C++ skills, crates, and testing patterns transfer directly.
...
<ImageZoom
  src="/img/stylus-enterprise-architecture-comparison.svg"
  alt="Traditional architecture vs. Stylus: blockchain as a verifiable compute layer within existing Web2 infrastructure"
/>
...
Stylus adds a second virtual machine alongside the EVM that executes <a data-quicklook-from="wasm">WASM</a>. Your code compiles through mature toolchains (`rustc`, Clang) and runs as native machine code — not interpreted bytecode. The result: the same logic runs at dramatically lower compute cost.
...
This is additive, not a replacement. Existing Solidity contracts keep working unchanged. Stylus programs and Solidity contracts call each other through standard interfaces. You can adopt incrementally — no big-bang migration required.

## How your existing skills transfer

### Build tooling

Stylus uses `cargo` (Rust) or standard C/C++ compilers. The Stylus CLI ([`cargo-stylus`](/stylus/cli-tools/commands-reference)) integrates into your existing workflow for compiling, checking, and deploying programs.

### Libraries

You can use crates and C/C++ libraries in your programs. The constraint is `no_std` (no standard library) for Rust, since the execution environment is sandboxed — similar to writing for embedded targets. Many crates already support `no_std`.
...
Write your program in Rust or C/C++. Compile to WASM using standard toolchains (`cargo build` for Rust, Clang for C/C++).
...
- **Porting existing Rust or C/C++ code onchain:** Libraries and algorithms you've already built and tested can be deployed with minimal modification — no rewrite in a new language required.
