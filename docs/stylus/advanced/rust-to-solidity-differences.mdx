...
- The receive function takes no parameters
...
- No direct equivalent to `immutable` (use storage set once in the constructor)
...
let contract_address = unsafe {
    RawDeploy::new()
        .salt(salt)
        .deploy(self.vm(), &bytecode, amount)?
};
```
...
#[public]
impl MyContract {
    pub fn call_token(&mut self, token: Address, recipient: Address, amount: U256) -> Result<bool, Vec<u8>> {
        let token_contract = IToken::new(token);
        let result = token_contract.transfer(self.vm(), Call::new_mutating(self), recipient, amount)?;
        Ok(result)
    }
}
```
