...
Rust provides a package registry at [crates.io](https://crates.io/) that lets developers conveniently access a wide range of open-source libraries as dependencies in their code. Stylus Rust contracts can leverage these crates to simplify their development workflow.

While [crates.io](http://crates.io/) is a fantastic resource, many of these libraries were not designed with the constraints of a blockchain environment in mind. Some produce large binaries that exceed the WASM smart contract compressed size limit of 24 KB on Arbitrum. Many also take advantage of unsupported features such as:
...
- [`rust_decimal`](https://crates.io/crates/rust_decimal): Decimal number implementation written in pure Rust. Suitable for financial and fixed-precision calculations. Requires `default-features = false` in your `Cargo.toml` to avoid pulling in `std`
...
- [`time`](https://crates.io/crates/time): Date and time library. Requires `default-features = false` in your `Cargo.toml` to avoid pulling in `std`
- [`hex`](https://crates.io/crates/hex): Encoding and decoding data into/from hexadecimal representation. Requires `default-features = false` in your `Cargo.toml` to avoid pulling in `std`
