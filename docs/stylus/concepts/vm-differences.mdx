...
The Stylus WASM VM also uses a stack-based execution model, though it offers a richer instruction set and compiles from modern languages (Rust, C, C++) rather than Solidity:

- **Operations**: Work with values on an implicit operand stack, using local variables as function-scoped storage slots
- **Instructions**: ~200 core instructions in the MVP spec, with additional instructions available through WASM extensions and fine-grained metering
...
(i32.add)        ;; Pop both values, push their sum
(local.set 2)    ;; Pop the result and store it in local variable 2
```

> **Note:** WASM local variables (accessed via `local.get` / `local.set`) may resemble registers, but the execution model is stack-based — all instructions implicitly consume inputs from and push outputs onto the operand stack.
...
4. **Flexibility**: Ink prices can be adjusted without changing the contract code
...
- **Gas forwarding**: Follows the 63/64 rule in both cases
...
- **One-time cost**: Pay the activation gas once
...
- Gas costs for computing (usually cheaper)
...
2. Maintain the same ABI for compatibility
...
- Familiar with existing developers
- No activation cost
