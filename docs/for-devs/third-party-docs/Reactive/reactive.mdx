...
Unlike traditional contracts that rely on EOA's to initiate function calls, reactive contracts respond to event logs emitted by contracts on other chains. These contracts can independently evaluate conditiosn and initiate outbound messages or state changes without requiring user transactions.
...
Reactive contracts are compatible with the standard EVM toolchain and can be written in Solidity using custom ABI's defined for event-based execution. Execution is trust-minimized: the logs are verifiable, emitted on origin chains, and the Reactive Network enforces that only matching, authentic events trigger contract logic.
