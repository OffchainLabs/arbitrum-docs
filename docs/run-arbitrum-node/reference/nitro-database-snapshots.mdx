...
Nitro stores the <a data-quicklook-from="chain-state">chain state</a> and data in a database in the local filesystem. When starting Nitro for the first time, it will initialize an empty database by default and start processing transactions from Genesis. It takes a long time for the node to sync from Genesis, so starting from a <a data-quicklook-from="database-snapshot">database snapshot</a> is advisable instead. Moreover, for the <a data-quicklook-from="arbitrum-one">Arbitrum One</a> chain, you must start from a snapshot because Nitro cannot process transactions from the <a data-quicklook-from="arbitrum-classic">Classic</a> Arbitrum node.

For an overview of snapshots in the context of initial node setup, see the [Database snapshots section of How to run a full node](/run-arbitrum-node/02-run-full-node.mdx#database-snapshots).

## Snapshot kinds

Each chain publishes snapshots per <a data-quicklook-from="state-scheme">state scheme</a>, and a snapshot only works with a node running the same scheme:

| Snapshot kind  | Node type | State scheme                               | Available for               |
| :------------- | :-------- | :----------------------------------------- | :-------------------------- |
| `pruned`       | Full node | <a data-quicklook-from="hashdb">HashDB</a> | Arbitrum One, Nova, Sepolia |
| `full-path`    | Full node | <a data-quicklook-from="pathdb">PathDB</a> | Arbitrum One, Nova, Sepolia |
| `archive`      | Archive   | HashDB                                     | Arbitrum One, Nova, Sepolia |
| `archive-path` | Archive   | PathDB                                     | Arbitrum One, Sepolia       |

Only the HashDB kinds distinguish pruned from unpruned. PathDB prunes as the node runs, so every PathDB snapshot is already pruned to its state-history window, and there is no separate unpruned variant to publish. The `pruned` kind exists because you prune a HashDB database manually, and the published HashDB full node snapshots happen to be pruned ones.

HashDB and PathDB full node snapshots are close in size at comparable dates, so size alone is not a reason to prefer one scheme. Sizes grow over time, so read the `metadata.json` in a snapshot directory for its current total before you provision a disk.

Snapshots for other Arbitrum chains may be available at the discretion of the chain's team. Ask them if you want to use one.
...
| Setting up a new **archive** node on **HashDB** | `--init.latest archive`. The publicly hosted `archive` snapshot is outdated on Arbitrum One and Sepolia.                                                                            |
...
| Reducing disk usage on an **existing** node     | `--init.prune full` (or `minimal`). HashDB only; PathDB prunes the state trie automatically.                                                                                        |
...
`--init.latest` can't help you here: it accepts only `archive`, `pruned`, and `genesis`, and all three resolve to HashDB snapshots. Snapshots built with the `path` scheme are published under the separate `full-path` and `archive-path` kinds, which `--init.latest` doesn't accept. See [Snapshot kinds](#snapshot-kinds) for which chains publish each one.

Every PathDB snapshot is <a data-quicklook-from="state-pruning">pruned</a> to its state-history window, because PathDB prunes as the node runs. There is no unpruned `path` variant to publish.
...
Use `latest-archive-path.txt` for an <a data-quicklook-from="archive-node">archive node</a>. For Arbitrum Sepolia, substitute `sepolia-rollup` for `arb1`.
...
Your node's `--execution.caching.state-scheme` must match the snapshot's `state_scheme`. See [Choose a state scheme](/run-arbitrum-node/02-run-full-node.mdx#choose-a-state-scheme).
