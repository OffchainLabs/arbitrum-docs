...
For an overview of snapshots in the context of initial node setup, see the [Database snapshots section of Prepare to run a node](/run-arbitrum-node/start-here.mdx#database-snapshots).
...
`--init.latest` can't help you here: it accepts only `archive`, `pruned`, and `genesis`, and all three resolve to HashDB snapshots. Snapshots built with the `path` scheme are published under a separate `archive-path` kind that `--init.latest` doesn't accept.

To initialize a PathDB node:

1. Find the `archive-path` snapshot for your chain in the [snapshot explorer](https://snapshot-explorer.arbitrum.io/). These exist for Arbitrum One and Arbitrum Sepolia. Arbitrum Nova has no `path` snapshot.
2. Confirm the scheme. Each snapshot directory publishes a `metadata.json` naming its `state_scheme`, `snapshot_kind`, size, and Nitro version. Check it before downloading terabytes.
3. Pass the snapshot URL to `--init.url`, or download it manually as described below.

<!-- todo: SME REVIEW — add a concrete copy-pasteable `--init.url` example for an `archive-path` snapshot once the question below is resolved.
     Deliberately omitted because it may not work on the current release. Nitro v3.11.2's downloadInitInParts
     (cmd/nitro/init/init.go:238) fetches `<url>.manifest.txt` to enumerate parts, but the published archive-path
     snapshots ship no .manifest.txt — only metadata.json (verified 404 at every prefix probed, e.g.
     arb1/2026-08-02-291ce8d7/archive-path.tar.manifest.txt). Those snapshots were built by v3.11.3-rc.*, which is
     newer than any released tag, so metadata.json support likely exists in unreleased code but could not be verified.
     ASK: does the current release read metadata.json for multi-part downloads? If yes, add the example and drop this
     comment plus the one in "Downloading snapshot parts". If no, document manual download as the only supported route. -->

Because only archive-kind `path` snapshots are published, PathDB has no snapshot-based path to a pruned full node today. Sync from genesis, or run HashDB. See [Choose a state scheme](/run-arbitrum-node/02-run-full-node.mdx#choose-a-state-scheme).
...
<!-- todo: SME REVIEW — confirm whether Nitro itself can still auto-download these snapshots in parts.
     The surrounding prose describes the .manifest.txt mechanism that Nitro v3.11.2 implements
     (cmd/nitro/init/init.go:238), but no published snapshot currently serves a .manifest.txt file.
     If current Nitro reads metadata.json instead, rewrite this whole "Downloading snapshot parts"
     section around metadata.json rather than leaving the note below as an exception. See the
     matching comment in the "PathDB snapshots" section above. -->

:::note

Snapshots published in the Arbitrum Foundation bucket list their parts in the `metadata.json` file at the root of the snapshot directory rather than in a `.manifest.txt` file. `metadata.json` gives each part's filename, size, and both SHA-256 and xxHash checksums. Read the part names from there when downloading one of those snapshots manually.
