...
| `nitro-node-validator` | `offchainlabs/nitro-node:validator-*` | `/usr/local/bin/split-val-entry.sh`                                                                                            | Runs a validation server behind the main `nitro` process. Used for block validation across WASM module root upgrades.                        |
...
services:
  nitro:
    ...
    command:
      ...
      # Set "--init.latest" for the most recent snapshot, or a specific URL to "--init.url".
