...
- You can use [ third party node providers ](/arbitrum-essentials/reference/node-providers.mdx) to get RPC access to fully-managed nodes
...
Beyond full and archive nodes, a Nitro node can take on other roles — sequencer, batch poster, validator, or feed relay — purely through configuration. For the flags that define each role and how to convert a node between roles, see [How to assign roles to a Nitro node](/run-arbitrum-node/assign-node-roles.mdx).
