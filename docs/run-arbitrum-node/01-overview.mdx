...
Beyond full and archive nodes, a Nitro node can take on other roles — <a data-quicklook-from="sequencer">sequencer</a>, <a data-quicklook-from="batch-poster">batch poster</a>, <a data-quicklook-from="validator">validator</a>, or <a data-quicklook-from="feed-relay">feed relay</a>—purely through configuration. For the flags that define each role and how to convert a node between roles, see [How to assign roles to a Nitro node](/run-arbitrum-node/assign-node-roles.mdx).
