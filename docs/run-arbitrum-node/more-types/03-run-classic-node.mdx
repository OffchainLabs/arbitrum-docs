...
<a data-quicklook-from="arbitrum-one">Arbitrum One</a> has been upgraded to <a data-quicklook-from="arbitrum-nitro">Nitro</a>, the latest Arbitrum tech stack. "<a data-quicklook-from="arbitrum-classic">Arbitrum Classic</a>" is our term for the old, pre-Nitro tech stack. The Nitro node databases have the raw data of all blocks, including pre-Nitro blocks. However, Nitro nodes cannot execute anything on pre-Nitro blocks. You need an Arbitrum Classic <a data-quicklook-from="archive-node">archive node</a> to execute data on pre-Nitro blocks.
...
- Arbitrum classic does not communicate with Nitro <a data-quicklook-from="sequencer">sequencer</a>, so the classic relay is no longer used.
