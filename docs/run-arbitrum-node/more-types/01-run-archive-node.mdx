...
For how archive mode relates to the other Nitro node roles, see [How to assign roles to a Nitro node](/run-arbitrum-node/operation/assign-node-roles.mdx).
...
For cache sizing and memory tuning that applies to archive nodes generally, see [node tuning and monitoring](/run-arbitrum-node/operation/node-tuning-and-monitoring.mdx).
