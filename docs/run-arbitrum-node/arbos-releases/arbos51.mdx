...
ArbOS 51 introduces the `gasFloorPerToken` parameter for chains posting to [EIP-7623](https://eips.ethereum.org/EIPS/eip-7623) enabled parent chains. This setting enforces a minimum price for data-heavy transactions to ensure that the cost of posting calldata to the parent chain is fully recovered, even when execution gas is low. \
