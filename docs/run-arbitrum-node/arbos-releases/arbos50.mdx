...
**Action Required:** Before Ethereum L1 Sepolia/Mainnet upgrade to Fusaka, **(1)** Nitro node operators must upgrade to Nitro `v3.8.0`, and **(2)** need to make sure they connect to a L1 Ethereum Beacon Chain node that has historical blob data, for the Nitro node software to function properly. **Read more [here](https://arbitrum.notion.site/EXT-Comms-Action-Required-for-Nitro-Node-Operators-Before-Sepolia-Mainnet-Fusaka-28601a3f59f880feb846c203d8480e73?source=copy_link)**.

:::

The minimum Nitro version that supports ArbOS 50 "Dia" is [Nitro v3.8.0-rc.X](https://github.com/OffchainLabs/nitro/releases/tag/v3.8.0-rc.5), which is available on Docker Hub with the image tag `Need to update - offchainlabs/nitro-node:v3.8.0-rc.5-6c74ee2-validator`. This release of Nitro is a mandatory upgrade for Arbitrum One and Nova validators. For Arbitrum One and Nova, the ArbOS 50 "Dia" upgrade will require a governance vote to activate.
...
- [Nitro v3.8.0-rc.X](https://github.com/OffchainLabs/nitro/releases/tag/v3.8.0-rc.5) or higher
- [nitro-contracts v3.1.0](https://github.com/OffchainLabs/nitro-contracts/releases/tag/v3.1.0) or higher
- WASM module root: `Need to update - 0xdb698a2576298f25448bc092e52cf13b1e24141c997135d70f217d674bbeb69a`
...
ArbOS 50 Dia is an upgrade to enable Arbitrum's support for the parent chain Ethereum's [Fusaka upgrade](https://ethereum.org/en/roadmap/fusaka/) tentatively scheduled for December 3, 2025 at epoch `XXX`. As a result, the majority of the ArbOS-specific changes revolve around implementing the relevant [Fusaka EIPs](https://eips.ethereum.org/EIPS/eip-7607) on Arbitrum chains.
...
### Special notes about Ethereum’s PeerDAS for Nitro operators
...
As part of the Fulu consensus node upgrade in Fusaka, Ethereum’s PeerDAS ([EIP-7594](https://eips.ethereum.org/EIPS/eip-7594)) will be activated.

This change optimizes data availability by removing the requirement for all nodes to download all blob data, while still guaranteeing full data availability network-wide. However, this means that node operators for Layer 2 networks, such as Arbitrum One and other Arbitrum L2 chains, will need to →

1. Make sure they connect to a L1 Ethereum Beacon Chain node that has historical blob data in order for the Nitro node software to function properly.
2. Send blob transactions with cell proofs rather than blob proofs.

Failure to act on these will result in Arbitrum L2 Nitro nodes →

1. Failing to fetch blob data properly from Ethereum (Sepolia or Mainnet).
2. Failing to post batches to Ethereum (Sepolia or Mainnet).

To read more about these required actions, go [here](https://arbitrum.notion.site/EXT-Comms-Action-Required-for-Nitro-Node-Operators-Before-Sepolia-Mainnet-Fusaka-28601a3f59f880feb846c203d8480e73?source=copy_link).
...
- [Nitro v3.8.0-rc.X](https://github.com/OffchainLabs/nitro/releases/tag/v3.8.0-rc.5)
