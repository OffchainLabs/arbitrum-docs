...
This page summarizes the changes brought by ArbOS 50 Dia, and what you should do to ensure a seamless upgrade.
...
The minimum Nitro version that supports ArbOS 50 "Dia" is [Nitro v3.8.0](https://github.com/OffchainLabs/nitro/releases/tag/v3.8.0), which is available on Docker Hub with the image tag `Need to update - offchainlabs/nitro-node:v3.8.0-rc.5-6c74ee2-validator`. This release of Nitro is a mandatory upgrade for <a data-quicklook-from="arbitrum-one">Arbitrum One</a> and <a data-quicklook-from="arbitrum-nova">Nova</a> validators. For Arbitrum One and Nova, the ArbOS 50 "Dia" upgrade will require a governance vote to activate.
...
- Running [Nitro v3.8.0](https://github.com/OffchainLabs/nitro/releases/tag/v3.8.0) or higher, which is available on Docker Hub with the image tag `Need to update - offchainlabs/nitro-node:v3.8.0-rc.5-6c74ee2-validator`.

  Note that it's important to run Nitro v3.8.0 only against trusted databases. If you want to use an untrusted database, you can first remove the `wasm` directory if it exists (potentially inside the `nitro` folder). Otherwise, the database may have malicious, unvalidated code that can result in remote code execution. Avoiding unvalidated code is also mitigated by ensuring you run the <a data-quicklook-from="arbitrum-nitro">Arbitrum Nitro</a> node inside Docker.
...
<VanillaAdmonition type="warning" title="This link is broken, it's a placeholder for the upcoming Nitro v3.8.0 release">

- [Nitro v3.8.0](https://github.com/OffchainLabs/nitro/releases/tag/v3.8.0)
