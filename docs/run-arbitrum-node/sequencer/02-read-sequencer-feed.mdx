...
[Running an Arbitrum relay locally as a feed relay](/run-arbitrum-node/data-availability/run-feed-relay.mdx) lets you subscribe to an uncompressed <a data-quicklook-from="sequencer-feed">sequencer feed</a> for real-time data as the sequencer accepts and orders transactions offchain.
