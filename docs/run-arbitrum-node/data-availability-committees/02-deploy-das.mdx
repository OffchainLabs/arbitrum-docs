...
```shell
docker run --rm -it -v /some/local/dir:/home/user/data --network="host" --entrypoint datool @@latestNitroNodeImage=offchainlabs/nitro-node:v3.8.0-62c0aa7@@ client rpc store --url http://localhost:9876 --message "Hello world" --signing-key /home/user/data/keys/ecdsa
```
...
```shell
docker run --rm -it --network="host" --entrypoint datool @@latestNitroNodeImage=offchainlabs/nitro-node:v3.8.0-62c0aa7@@ client rest getbyhash --url http://localhost:9877 --data-hash 0xDataHash
```
