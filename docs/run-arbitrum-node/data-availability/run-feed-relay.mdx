...
For context on where the feed relay fits into Arbitrum's overall data flow, see [Data availability](/how-arbitrum-works/deep-dives/data-availability.mdx#how-full-nodes-sync-the-data-from-the-sequencer-feed). For the wire format the feed delivers, see [How to read the sequencer feed](/run-arbitrum-node/sequencer/02-read-sequencer-feed.mdx). For how the feed relay compares to the other Nitro node roles, see [How to assign roles to a Nitro node](/run-arbitrum-node/operation/assign-node-roles.mdx).
