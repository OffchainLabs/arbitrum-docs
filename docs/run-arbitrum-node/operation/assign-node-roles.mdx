...
Flags can be passed on the command line or collected in a JSON configuration file loaded with `--conf.file`. For how flag names, defaults, and the config file relate to each other, see [Nitro configuration system](/run-arbitrum-node/reference/configuration-system.mdx). This article covers only the flags that define and support each role; for the complete flag list, see the [CLI flags reference](/run-arbitrum-node/reference/cli-flags-reference.mdx).
...
| Feed relay                           | `relay` binary + `--node.feed.input.url` + `--chain.id`                         | No wallet, no bond, no parent-chain connection            | Many (stateless fan-out)                  | [Run a feed relay](/run-arbitrum-node/data-availability/run-feed-relay.mdx)              |

Every role except the feed relay and the split-validation server needs a parent-chain connection through `--parent-chain.connection.url`. Nodes whose parent chain is Ethereum <a data-quicklook-from="L1">L1</a> also need `--parent-chain.blob-client.beacon-url` to read batches posted as EIP-4844 <a data-quicklook-from="blob">blobs</a>. For well-known chains, setting `--chain.id` or `--chain.name` auto-fills defaults such as `--execution.forwarding-target` and `--node.feed.input.url` from the chain info embedded in the binary (the forwarding target is only auto-filled when the sequencer is disabled).
...
For the full setup, including snapshots, <a data-quicklook-from="state-pruning">pruning</a>, and Docker details, see [How to run a full node](/run-arbitrum-node/02-run-full-node.mdx).

## Archive node

An <a data-quicklook-from="archive-node">archive node</a> is an RPC node that retains every historical state instead of garbage-collecting old ones, so it can serve `eth_call`, balance, and trace queries at arbitrary past blocks. It is a storage variant of the full node, not a different protocol role: same binary, one extra caching flag.
...
For Docker images, Helm charts, and the full setup, see [Run a split validator node](/launch-arbitrum-chain/run-a-node/split-validator-node.mdx) and [Docker images and CLI binaries](/run-arbitrum-node/reference/docker-and-cli-binaries.mdx).
...
For setup, Docker commands, and Kubernetes charts, see [How to run a feed relay](/run-arbitrum-node/data-availability/run-feed-relay.mdx).
...
For background on how nodes fit together, see the [Nodes overview](/run-arbitrum-node/01-overview.mdx) and, for the data flow between the sequencer, feed, and full nodes, [Data availability](/how-arbitrum-works/deep-dives/data-availability.mdx#how-full-nodes-sync-the-data-from-the-sequencer-feed).
