...
The [community Helm chart](https://github.com/OffchainLabs/community-helm-charts/tree/main/charts/nitro) wires this for you when you set `serviceMonitor.enabled=true`.

### Grafana dashboard

The chart repository's README points to the Grafana dashboard on its [Releases page](https://github.com/OffchainLabs/community-helm-charts/releases). If a release does not attach the dashboard JSON, pull the last published copy from the repository's git history and import it:
...
This exported dashboard has no `__inputs` datasource prompt, and its `Source` variable is hard-pinned to an internal Mimir UID. After importing, you **must** repoint the `Source` variable at your own Prometheus instance, or every panel reads "No data." Then set `nitronodejob` to this node's scrape job, and leave `sequencerjob`, `validatorjob`, and `relayjob` empty so only the full-node rows render.
...
The following INFO, WARN, and ERROR strings distinguish a healthy node from a broken one on any Arbitrum chain, whichever way you deploy it. The sequencer hostnames in the examples below are Arbitrum One's; your chain's differ. To get structured output, set `log-type=json`. The community Helm chart sets it for you.

| Event                              | Healthy signal                                                                              | Broken or warning signal                                                                                                                              |
| ---------------------------------- | ------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
...
A full node forwards user transactions to the <a data-quicklook-from="sequencer">sequencer</a>. When forwarding fails, the cause falls into one of three buckets. Forwarding failures are **log-based, not metric-based**—the forwarder emits no metrics—so alert by grepping the log strings below.
...
| **Sequencer returns a business error** (non-connection, for example `nonce too low`)       | `WARN error forwarding transaction trying different target` with the sequencer's `err`—not escalated; the error is returned to the caller      |
...
For error strings mapped to specific fixes, see [troubleshooting](/run-arbitrum-node/06-troubleshooting.mdx).
...
<a data-quicklook-from="archive-node">Archive nodes</a> benefit from larger caches to reduce disk I/O for historical queries:
