...
Sets how many blocks of state history Nitro retains, counting back from the chain head. `0` means the entire chain.

When you leave this flag unset, Nitro chooses the default at startup:
...
Nitro started deriving this default from the archive flag in v3.10.0. On earlier versions the default was always 24 hours' worth of blocks, so an archive node needs an explicit `--execution.caching.state-history=0`.
