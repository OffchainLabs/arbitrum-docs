...
Latest [Docker image](https://hub.docker.com/r/offchainlabs/nitro-node/tags): <code>@@latestNitroNodeImage=offchainlabs/nitro-node:v3.8.0-62c0aa7@@</code>
...
- `--init.prune <pruning mode>`
  - `minimal`: Only genesis + latest head state is left, takes the least amount of time (several house for Arbitrum One-sized database)
