...
| Full node (pruned) snapshot | Published                              | None published yet                      |
| Archive snapshot            | Published                              | Published for Arbitrum One and Sepolia  |
| Minimum Nitro version       | Any                                    | v3.9.x                                  |

**Run your full node on HashDB.** No published snapshot pairs PathDB with a pruned (full node) database, so a PathDB full node has to sync from genesis. On Arbitrum One, syncing from genesis also requires a [Classic state import](/run-arbitrum-node/nitro/02-migrate-state-and-history-from-classic.mdx#step-3-initialize-your-nitro-node-importing-the-exported-data). For most operators, that cost outweighs the disk savings.

PathDB pays off on **archive** nodes, where it cuts disk use substantially and a `path`-scheme snapshot is published. See [How to run an archive node](/run-arbitrum-node/more-types/01-run-archive-node.mdx).
...
`--init.latest` accepts only `archive`, `pruned`, and `genesis`, and every snapshot it resolves for those kinds uses HashDB. Adding `--execution.caching.state-scheme=path` to the examples below fails, because the downloaded snapshot's scheme won't match. To initialize a PathDB database, pass the `path`-scheme snapshot URL to `--init.url` instead. See [Nitro database snapshots](/run-arbitrum-node/nitro/03-nitro-database-snapshots.mdx#pathdb-snapshots).
...
On **HashDB**, the default, pruning is a _manual, opt-in_ operation. It removes all states from blocks older than the latest 128.

On **PathDB**, pruning is automatic and runs online. Nitro retains `--execution.caching.state-history` blocks of state history. When you leave that flag unset, Nitro picks the default at startup:
...
Nitro chose this default from the archive flag starting in v3.10.0. Before v3.10.0, the default was always 24 hours' worth of blocks, so an archive node needed an explicit `--execution.caching.state-history=0`.
