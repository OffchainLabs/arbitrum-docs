...
If you're looking to run a node with a different role — sequencer, batch poster, validator, archive node, or feed relay — see [How to assign roles to a Nitro node](/run-arbitrum-node/assign-node-roles.mdx) for the flags that define each role.
