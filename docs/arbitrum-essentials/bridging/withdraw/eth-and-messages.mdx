...
- Awareness that child-to-parent messages require @@arbOneDisputeWindowDays=6.4@@ days to finalize
...
`NodeInterface` is a "virtual" contract accessible at `0x00000000000000000000000000000000000000C8`. It isn't a true <a data-quicklook-from="precompile">precompile</a> but provides Arbitrum-specific data without requiring a custom RPC.
...
2. **Challenge period**: Wait @@arbOneDisputeWindowDays=6.4@@ days for the <a data-quicklook-from="assertion">assertion</a> to finalize
...
// After @@arbOneDisputeWindowDays=6.4@@ days, execute on parent chain using the steps above
```
...
For **ERC-20** token withdrawals, see the dedicated [Withdraw tokens guide](/arbitrum-essentials/bridging/withdraw/tokens.mdx), which provides detailed instructions for using Arbitrum's canonical token <a data-quicklook-from="bridge">bridge</a>.
...
| Waiting for finalization                 | The assertion containing the message is in the challenge period (@@arbOneDisputeWindowDays=6.4@@ days) |
