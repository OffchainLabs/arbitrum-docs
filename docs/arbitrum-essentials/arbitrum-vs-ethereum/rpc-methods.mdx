...
| Field name      | Description                                                                                                                                                                                                           |
| --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `l1BlockNumber` | The block number of the first non-Arbitrum ancestor chain that is usable for `block.number` calls. More information in [Block numbers and time](/arbitrum-essentials/arbitrum-vs-ethereum/block-numbers-and-time.mdx) |
...
| Field name      | Description                                                                                                                                                                                                                          |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `l1BlockNumber` | An approximate block number of the first non-Arbitrum ancestor chain that occurred before this child chain block. More information in [Block numbers and time](/arbitrum-essentials/arbitrum-vs-ethereum/block-numbers-and-time.mdx) |
...
| `gasLimit`   | Value is fixed at `0x4000000000000`, but it's important to note that Arbitrum One currently has a 32M gas limit per block. See [Chain params](/arbitrum-essentials/reference/chain-params.mdx) for the gas limit of other chains |
...
To enable them, include `txpool` in the node's `--http.api` list:

```shell
--http.api=net,web3,eth,arb,txpool
```

Enabling the namespace does not make the results mean what they mean on Ethereum. On Ethereum, the transaction pool holds transactions that are waiting to be picked up by whichever validator builds the next block. On an Arbitrum chain, the <a data-quicklook-from="sequencer">sequencer</a> orders transactions, and a single known party holds the resulting queue. Pool contents reflect that node's view of the sequencer's intake, not a competitive mempool. Nonce handling differs for the same reason — to learn more, see [Nonce management](/arbitrum-essentials/arbitrum-vs-ethereum/nonce-management.mdx).
...
For a sense of what ranges are widely tolerated, Nitro chunks its own parent chain log queries conservatively: message extraction prefetches 499 blocks at a time, and the <a data-quicklook-from="bold">BoLD</a> validator reads in chunks of 5,000 via `--node.bold.max-get-log-blocks`. Those are Nitro acting as a <a data-quicklook-from="client">client</a> against someone else's endpoint, not limits it enforces on yours.

If you are running your own node and querying it directly, large ranges are bounded by memory and response time rather than by a configured cap. Paginate wide historical queries regardless.
