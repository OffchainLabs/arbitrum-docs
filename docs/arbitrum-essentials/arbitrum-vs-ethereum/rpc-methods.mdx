...
Enabling the namespace does not make the results mean what they mean on Ethereum. On Ethereum, the transaction pool is where transactions wait to be picked up by whichever validator builds the next block. On an Arbitrum chain, the <a data-quicklook-from="sequencer">sequencer</a> orders transactions and the result is a queue held by a single known party, so pool contents reflect that node's view of the sequencer's intake rather than a competitive mempool. Nonce handling differs for the same reason — to learn more, see [Nonce management](/arbitrum-essentials/arbitrum-vs-ethereum/nonce-management.mdx).
