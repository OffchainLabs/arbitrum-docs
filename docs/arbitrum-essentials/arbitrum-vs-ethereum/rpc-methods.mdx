...
- `eth_sendRawTransactionConditional` submits a raw signed transaction with extra preconditions that Nitro evaluates before the sequencer includes it. If any condition fails, the node rejects the transaction with JSON-RPC error -32003. For the conditions you can attach, where Nitro evaluates them, and the errors they return, see [Conditional transactions](/arbitrum-essentials/arbitrum-vs-ethereum/send-raw-transaction-conditional.mdx).
