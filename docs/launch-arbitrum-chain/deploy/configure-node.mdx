...
Before reading this guide, we recommend that you're familiar with the general process for creating new chains explained in the introduction and the first section of [How to deploy an Arbitrum chain](/launch-arbitrum-chain/deploy/deploy-chain.mdx).
...
For [Arbitrum AnyTrust chains](/how-arbitrum-works/deep-dives/anytrust-protocol.mdx), the Nitro node configuration object has an additional `da.anytrust` segment under the `node` field to configure the AnyTrust-specific properties (prior to Nitro v3.10.0, this segment was named `data-availability` and also held the `sequencer-inbox-address` and `parent-chain-node-url` keys, which current versions no longer accept):
...
You can find information about what these parameters configure in [How to configure a DAC](/launch-arbitrum-chain/chain-config/data-availability/configure-dac.mdx).
...
If the chain uses a custom genesis, also give the node the generated `genesis.json` on its first startup by setting `--init.genesis-json-file`. Follow [Generate a custom genesis file with the Chain SDK](/launch-arbitrum-chain/deploy/custom-genesis-state.mdx#5-initialize-every-nitro-node) to keep the node's file consistent with the genesis assertion committed during deployment.
