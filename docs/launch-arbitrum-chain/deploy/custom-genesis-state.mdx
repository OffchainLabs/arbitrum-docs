...
- [Chain SDK](https://github.com/OffchainLabs/arbitrum-chain-sdk) >= v0.28.0 **or**
...
- [genesis-file-generator tool](https://hub.docker.com/r/offchainlabs/genesis-file-generator)
  ...
  - You can also use the `genesis-file-generator` to customize the `genesis.json` file or pass accounts/custom chain config.
...
- Generates a `genesis.json` file with the pre-deployed contracts when `loadDefaultPredeploys` is enabled. You can also supply custom account allocations.
...
| `ENABLE_NATIVE_TOKEN_SUPPLY`   | Flag to launch your chain with native interop tokens as [minting/burning gas tokens via third-party bridges](/launch-arbitrum-chain/chain-config/costs/configure-native-mint-burn.mdx) via third-party protocols. |
...
2. Generate genesis
   ...
   - To preallocate custom accounts, set `customAllocAccountFile` to `custom-alloc.json` in `genesis-input.json` and place the file (standard Geth `alloc` format) in the working directory. See the [`genesis-file-generator` section](#use-the-genesis-file-generator-tool) below for a sample, and review the solvency warning above before setting non-zero balances.
     :::info Configuration
...
| `chainOwner`                 | `string`  | Yes      | —       | The address that will have admin ownership of the deployed chain.                                                                                                                                                 |
| `l1BaseFee`                  | `string`  | Yes      | —       | The initial L1 gas price (in wei) used to calibrate the chain initiation. Use a nonzero value.                                                                                                                    |
| `isAnyTrust`                 | `boolean` | No       | `false` | Whether the chain is an AnyTrust (`true`) or rollup (`false`).                                                                                                                                                    |
| `loadDefaultPredeploys`      | `boolean` | No       | `false` | Include the default contracts supplied by the genesis file generator.                                                                                                                                             |
| `enableNativeTokenSupply`    | `boolean` | No       | `false` | Flag to launch your chain with native interop tokens as [minting/burning gas tokens via third-party bridges](/launch-arbitrum-chain/chain-config/costs/configure-native-mint-burn.mdx) via third-party protocols. |
...
| `ENABLE_NATIVE_TOKEN_SUPPLY`   | (optional) Set to `true` if you want to launch your chain with native interop tokens as [minting/burning gas tokens via third-party bridges](/launch-arbitrum-chain/chain-config/costs/configure-native-mint-burn.mdx) via third-party protocols. | `false`               |
