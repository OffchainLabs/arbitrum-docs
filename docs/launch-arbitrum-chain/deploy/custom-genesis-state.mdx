...
Arbitrum chain operators increasingly want to deploy chains with pre-existing state by loading an initial state from a file such as `genesis.json`. Specifically, chain operators want to predeploy smart contracts (like Gnosis Safe) to an <a data-quicklook-from="arbitrum-chain">Arbitrum chain</a> so they exist from genesis, before any user interaction or post-launch governance.
...
- [Nitro node](/run-arbitrum-node/02-run-full-node.mdx) >= v3.11.0 is needed to use custom-genesis.
...
The Chain SDK provides a [custom genesis example](https://github.com/OffchainLabs/arbitrum-chain-sdk/tree/main/examples/generate-genesis) that shows how to configure account allocations, generate a genesis file, and use the output to deploy your chain.
