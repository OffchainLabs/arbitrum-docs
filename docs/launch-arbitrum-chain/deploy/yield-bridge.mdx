...
`MasterVault` is the escrow contract which holds an **idle balance** available for immediate withdrawals and allocates the rest to a single **sub-vault**. It is the only contract that talks to the sub-vault.
...
Roles are hierarchical, so the authority to change risk parameters is separated from the authority to run the vault day to day. The `ADMIN_ROLE` is held by your chain's <a data-quicklook-from="upgrade-executor">UpgradeExecutor</a>, which means allowlisting a sub-vault goes through the same governance path as any other privileged chain action.
...
That makes the beneficiary the single decision that determines the economics of your Yield Bridge. With the proceeds from the Yield Bridge, a chain owner may choose to:

- sponsor or subsidize gas for end users on their chain
- offset the cost of posting data to the parent chain and lower fees for your users
- fund incentives for liquidity providers or market makers on their chain
- reward end users for activies aligned with growth goals
...
- **The underlying asset must be a plain ERC-20.** Fee-on-transfer and rebasing tokens would break the vault's accounting and are therefore rejected.
