...
A Yield Bridge is a new capability with an operational burden that a traditional token bridge does not have: the <a data-quicklook-from="chain-owner">chain operator</a> manages and monitors a vault, decides how much capital stays liquid, and accepts the risk that the underlying strategy loses money.
