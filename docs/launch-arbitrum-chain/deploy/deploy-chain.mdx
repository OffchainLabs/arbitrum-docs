...
If you want to launch with predeployed contracts, balances, or storage, first [generate a custom genesis file with the Chain SDK](/launch-arbitrum-chain/deploy/custom-genesis.mdx). The generated block hash, send root, chain configuration, and initial L1 base fee must be included in the deployment configuration described below.
