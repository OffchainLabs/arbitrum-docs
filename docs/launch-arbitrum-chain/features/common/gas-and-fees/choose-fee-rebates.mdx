...
- **With Custom Gas Tokens**: Rebates work but require a fee token pricer for parent chain reimbursements (e.g., converting custom tokens to `ETH`). Pricers (manual, <a data-quicklook-from="oracle">oracle</a>-based, or trade-tracking) ensure accurate reimbursements to batch posters, which can include refund-like mechanics if reserves are managed.
