...
- **Add new methods to an existing [precompile](/build-decentralized-apps/precompiles/02-reference.mdx)**: Extend a built-in precompile (e.g., `ArbSys`) with additional functions.
...
- **Call and modify state**: Integrate with [ArbOS](/how-arbitrum-works/deep-dives/arbos.mdx) state by adding storage variables and methods to read/write them, allowing persistent chain-specific data.
...
- Compatible with [BoLD](/how-arbitrum-works/bold/gentle-introduction.mdx) or permissioned validation.
...
**All** modifications to precompiles should consider ArbOS version control or it may cause a chain fork. More information can be found on the [Customize ArbOS page](/launch-arbitrum-chain/chain-configuration/core/arbos#where-should-i-insert-arbos-upgrade-related-code).
