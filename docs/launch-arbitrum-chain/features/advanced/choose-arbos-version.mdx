...
- **Backward compatibility**: Changes must preserve old block results; new features activate only post-upgrade via version gates (e.g., `if state.ArbOSVersion() >= targetVersion`). For more details, please refer to [ArbOS version control](/launch-arbitrum-chain/chain-configuration/core/arbos.mdx#4-any-changes-in-the-stf-logic-that-will-affect-the-final-execution-result-arbos-version-control).

## Compatibility

- Works with all DA modes (Rollup, AnyTrust, Alt-DA), gas tokens, and validation ([BoLD](/how-arbitrum-works/bold/gentle-introduction.mdx)/permissioned), as long as changes meet STF rules.
...
- **New [precompile](/build-decentralized-apps/precompiles/02-reference.mdx) method**: Add `sayHi()` to ArbSys, gated by `ArbOwner.methodsByName["SayHi"].arbosVersion = 32`.
...
This reflects Arbitrum's modularity but demands rigorous testing. Refer to the docs or your <a data-quicklook-from="raas">RaaS</a>; a [list of RaaSes is on the Third-party providers page](/launch-arbitrum-chain/integrations/infrastructure-providers.mdx#rollup-as-a-service-raas-providers).
