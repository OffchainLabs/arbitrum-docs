...
- Works with all Arbitrum chain DA modes (Rollup, AnyTrust, Alt-DA), gas tokens, and validation ([BoLD](/how-arbitrum-works/bold/gentle-introduction.mdx) or permissioned), but STF changes must meet strict rules (e.g., no non-determinism).
...
- **Adding a new [precompile](/build-decentralized-apps/precompiles/02-reference.mdx) method**: Extend ArbSys with a "sayHi" function that activates at ArbOS v32, returning different strings pre/post-upgrade.
...
Refer to the docs or your RaaS; a [list of RaaSes is on the Third-party providers page](/launch-arbitrum-chain/06-third-party-integrations/02-third-party-providers.mdx#rollup-as-a-service-raas-providers).
