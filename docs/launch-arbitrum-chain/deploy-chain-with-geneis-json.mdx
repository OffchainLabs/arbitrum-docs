# Deploying Arbitrum Orbit Chains Using Genesis Configuration
...
- nitro repository with genesis-generator compiled
- Access to Arbitrum Sepolia testnet for rollup deployment
...
Visit the [official nitro build documentation](https://docs.arbitrum.io/run-arbitrum-node/nitro/build-nitro-locally) to install the required **prerequisite tools**.
...
- Check that `alloc.json` has valid JSON format
- Ensure sufficient disk space for file generation

**Genesis Generator Issues**

- Confirm nitro's genesis generator is properly compiled
- Verify genesis.json format matches expected input
- Check for any missing dependencies in the nitro environment

**Rollup Deployment Fails**

- Verify genesis block hash is correct and properly formatted
- Ensure sufficient funds for gas fees on Arbitrum Sepolia
