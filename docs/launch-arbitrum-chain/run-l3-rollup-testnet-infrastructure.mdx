...
This page gives **step-by-step instructions** to run your chain's full infrastructure as a production-level testnet with high availability (HA).

**Before you start:** Complete [Run an L3 rollup from scratch](/launch-arbitrum-chain/run-l3-rollup-from-scratch.mdx) Steps 1–4 (deploy, generate config, **fund batch poster and validator**). You need `node-config.json` in your project folder. Do **not** run Step 5 (local node)—this page replaces it with production infrastructure.
...
2. Add Helm repo and create namespace
3. Set up Redis
...
| :--------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Deployed chain**     | Completed [Run an L3 rollup from scratch](/launch-arbitrum-chain/run-l3-rollup-from-scratch.mdx) Steps 1–4 (deploy, generate config, fund batch poster and validator) |
...
Ensure Redis is reachable from your Kubernetes cluster (same VPC or network).
...
Sequencer relays combine feeds from all sequencer replicas. Other components connect to these relays instead of directly to sequencers.
...
The batch poster posts transaction batches to the parent chain. Save the private key to a file (do not commit this file to git):
...
The validator validates blocks and posts assertions to the parent chain. It is required for chain security. The validator wallet needs ETH on the parent chain (Arbitrum Sepolia) for gas. Save the validator private key to a file (do not commit to git):
...
The chain works without this step—sequencers coordinate via Redis automatically. The SQM lets you view which sequencer is active and manually switch if needed. Run it on your local machine.
...
You should get a JSON response with a block number (e.g. `{"jsonrpc":"2.0","id":1,"result":"0x..."}`). Wait until blocks are producing before Step 13.
...
Replace `YOUR_LOADBALANCER_IP` with your external IP. The script reads from your `.env` file. When you see `Token bridge ready.`, you're done.
...
For production mainnet, consider a RaaS provider.
