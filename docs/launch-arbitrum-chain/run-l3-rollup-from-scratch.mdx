...
This is the **simplest way** to run a complete Arbitrum L3 chain that settles to <a data-quicklook-from="arbitrum-sepolia">Arbitrum Sepolia</a>. You'll run five steps in order. No configuration choices—everything uses defaults. One node runs as sequencer, batch poster, and validator.
...
After Step 2, add the three lines the deploy script prints (`CHAIN_DEPLOYMENT_TRANSACTION_HASH`, `BATCH_POSTER_PRIVATE_KEY`, `VALIDATOR_PRIVATE_KEY`). Your `.env` will grow as you progress through the steps.
...
This step deploys your chain's contracts to Arbitrum Sepolia. It will cost a small amount of ETH for gas.

Create a new file named `deploy.mjs` in your project folder. Copy and paste this entire script:
...
When it succeeds, the script prints lines starting with `CHAIN_DEPLOYMENT_TRANSACTION_HASH=`, `BATCH_POSTER_PRIVATE_KEY=`, and `VALIDATOR_PRIVATE_KEY=`. **Copy those three lines into your `.env` file** (add them below `DEPLOYER_PRIVATE_KEY`). Your `.env` should then have four lines. The script also prints `BATCH_POSTER_ADDRESS=` and `VALIDATOR_ADDRESS=`—you will fund these in Step 4. Save the file before continuing.

## Step 3: Generate the node config

This step creates `node-config.json`—the file your node needs to know how to connect to your chain and Arbitrum Sepolia.
...
You should see `Node config written to node-config.json`. That file is ready for the next step.
...
Get the addresses from your Step 2 output (the lines starting with `BATCH_POSTER_ADDRESS=` and `VALIDATOR_ADDRESS=`). Send at least 0.01 ETH to each address from your deployer wallet or a [faucet](https://arbitrum.faucet.dev/). Without this, the node will not produce blocks or finalize.

## Step 5: Run the node

This starts your chain. One node runs everything: it sequences blocks, posts them to Arbitrum Sepolia, and validates. Keep this terminal open—the node runs in the foreground.
...
This step enables bridging tokens between your chain and Arbitrum Sepolia. **Your node must be running** (Step 5). Open a **new terminal** in the same project folder—leave the node running in the first one.
...
It will take a minute or two. When you see `Token bridge ready.`, you're done.

## You're done

You now have a fully running L3 chain that settles to Arbitrum Sepolia, with sequencer, validator, and token bridge. Your chain's RPC is at `http://localhost:8547`.
...
- **Product-level testnet:** To run your chain with production infrastructure (HA sequencers, separate nodes, public RPC) instead of locally, complete Steps 1–4 above, then follow [Run L3 rollup infrastructure (product-level testnet)](/launch-arbitrum-chain/run-l3-rollup-testnet-infrastructure.mdx).
