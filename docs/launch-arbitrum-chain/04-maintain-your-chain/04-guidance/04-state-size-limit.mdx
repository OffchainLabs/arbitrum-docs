---
title: 'Guidance for Arbitrum chains target'
sidebar_label: 'Manage target gas per second'
description: 'How to manage your Arbitrum chain's target equilibrium'
author: anegg0
sme: anegg0
target_audience: 'Developers deploying and maintaining Arbitrum chains.'
sidebar_position: 6
user_story: As a current or prospective Arbitrum chain deployer, I need to understand how to manage my target.
content_type: how-to
---

## What is the target on an Arbitrum chain?

The parameter that governs an Arbitrum chain's throughput limit is known as the `target`.

The target is measured in _gas per second_ and is used as a threshold for increasing gas prices.
...
The effect of raising gas prices at the target is to curb user demand when the chain is congested. Doing so protects the chain's underlying infrastructure from being overloaded.
