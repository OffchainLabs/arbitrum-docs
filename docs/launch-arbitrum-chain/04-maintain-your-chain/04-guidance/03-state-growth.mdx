...
The critical Nitro stack parameter affecting state growth and state growth rate is the **gas target**. Offchain Labs has set the gas target to a default (7,000,000 gas/s), ensuring a safe, sustained operating limit for Arbitrum chains. You can read more about the nuances of the gas target [here](/how-arbitrum-works/deep-dives/gas-and-fees.mdx#the-gas-target).
