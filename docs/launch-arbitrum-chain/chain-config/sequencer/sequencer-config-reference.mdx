...
Expected surplus estimates whether your chain is collecting enough in <a data-quicklook-from="parent-chain">parent chain</a> data fees to cover what it will cost to post the data it already owes. Nitro computes it as:
...
2. **Neither works without a parent chain reader.** Both are skipped when the sequencer has no parent chain reader configured, because the calculation needs parent chain headers.
...
3. Leave the hard threshold disabled until the soft threshold has been quiet for a while. It stops your chain accepting transactions, which is a more disruptive failure than the deficit it prevents for most chains.

## Block production timing

| Flag                             | Default | What it controls                                                                                                                             |
| -------------------------------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `max-block-speed`                | `250ms` | Minimum delay between blocks, so the maximum block production rate                                                                           |
