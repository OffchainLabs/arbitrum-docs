...
While any Arbitrum chain can adopt Fast Withdrawals, we only _recommend_ that fast withdrawals be adopted by [AnyTrust](/how-arbitrum-works/deep-dives/anytrust-protocol.mdx) chains with a minimum <a data-quicklook-from="validator">validator</a> and DAC member requirement. We explain both these recommendations below:
...
1. Soft finality from the [Sequencer](/how-arbitrum-works/deep-dives/sequencer.mdx)'s confirmation of <a data-quicklook-from="transaction">transaction</a> inclusion (~@@l2BlockTimeMs=250@@ms)
