...
As of Nitro v3.10.0, the `daserver` and `datool` binaries have been renamed to `anytrustserver` and `anytrusttool`, and the parent chain flags moved: `--data-availability.parent-chain-node-url` is now `--parent-chain.node-url`, and `--data-availability.sequencer-inbox-address` is now `--parent-chain.sequencer-inbox-address`. The Docker image keeps a deprecated `datool` symlink for backward compatibility, but no longer includes `daserver`. This guide uses the new names throughout.
...
- The latest Nitro docker image: `@@latestNitroNodeImage=offchainlabs/nitro-node:v3.11.2-3599aca@@`
...
@@latestNitroNodeImage=offchainlabs/nitro-node:v3.11.2-3599aca@@ keygen --dir /data/keys
```
...
template:
    ...
    - command:
        ...
        image: @@latestNitroNodeImage=offchainlabs/nitro-node:v3.11.2-3599aca@@
...
```shell
docker run --rm -it -v /some/local/dir:/home/user/data --entrypoint anytrusttool @@latestNitroNodeImage=offchainlabs/nitro-node:v3.11.2-3599aca@@ keygen --dir /home/user/data/keys --ecdsa
```
...
```shell
docker run --rm -it -v /some/local/dir:/home/user/data --network="host" --entrypoint anytrusttool @@latestNitroNodeImage=offchainlabs/nitro-node:v3.11.2-3599aca@@ client rpc store --rpc-client.rpc.url http://localhost:9876 --message "Hello world" --signing-key /home/user/data/keys/ecdsa
```
...
```shell
docker run --rm -it --network="host" --entrypoint anytrusttool @@latestNitroNodeImage=offchainlabs/nitro-node:v3.11.2-3599aca@@ client rest getbyhash --url http://localhost:9877 --data-hash 0xDataHash
```
