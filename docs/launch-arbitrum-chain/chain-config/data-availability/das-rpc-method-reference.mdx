...
These methods are usd by the **RPC aggregator** (`node.da.anytrust.rpc-aggregator.rpc-client`) when communicating with standalone DAS servers.
...
Functionally equivalent to `das_sendChunk`. Max chunk body size is goverend by `node.da.external-provider.data-stream.max-store-chunk-body-size` (default: 5,242,880 bytes / 5 MB).
