...
The following fields are populated automatically from the SDK's hardcoded `defaults`. They are standard Ethereum genesis/fork-activation parameters and are **not** exposed for customization by `prepareChainConfig`. **Changing them can produce an invalid or non-functional chain configuration**. Leave them at their default values.
