...
1. When you want to make changes to your Nitro code that affect the State Transition Function, or STF (you may refer to the [Customize STF docs](../../chain-configuration/core/stf.mdx#introduction)), and
...
After you add `sayHi()` to `ArbSys.go` according to the guide in [customize precompile Option 1](../../chain-configuration/core/precompiles.mdx#option-1-add-new-methods-to-an-existing-precompile),
...
After you add a new precompile named `ArbHi` according to the guide in [customize precompile Option 2](../../chain-configuration/core/precompiles.mdx#option-2-create-a-new-precompile) and make changes to [`precompile.go`](https://github.com/OffchainLabs/@@nitroRepositorySlug=nitro@@/blob/@@nitroVersionTag=v3.9.9@@/@@nitroPathToPrecompiles=precompiles@@/precompile.go),
...
It should be noted that when you initialize the state (initial code is in [customize precompile Option 5](../../chain-configuration/core/precompiles.mdx#option-5-call-and-modify-state)), you need to initialize it to `0` or `null` value first to avoid a potential re-org on your chain.
...
Please continue reading [Customize STF](../../chain-configuration/core/stf.mdx#step-3-run-the-node-without-fraud-proofs) for follow-up operations.
