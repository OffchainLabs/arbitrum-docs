...
- This extends Step 1 in preparation of the full node config (e.g., for running a [sequencer](/how-arbitrum-works/deep-dives/sequencer.mdx) or validator node). Place this in a file like `deployOrbitChain.ts`. It assumes you’re deploying to a parent chain, such as Sepolia (testnet).
