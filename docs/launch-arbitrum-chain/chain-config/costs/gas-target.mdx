...
Call `ArbGasInfo.getGasPricingConstraints()` first. If it returns an empty array, your chain uses a single gas target and `setSpeedLimit` applies. If it returns one or more constraints, your chain uses multiple gas targets.
