...
<a data-quicklook-from="arbitrum-chain">Arbitrum chains</a> support a new gas pricing algorithm that uses multiple-gas targets, as part of the [ArbOS 51 upgrade](/run-arbitrum-node/arbos-releases/arbos51), to reduce the frequency and severity of child-chain gas price spikes. This is the first optimization of many towards Dynamic Pricing.
...
- `long_term_gas_target` is the desired long-term average gas throughput for the chain. A higher gas target means that your chain can handle more TPS before the gas begins to spike. However, a higher gas target also leads to faster state growth and requires more powerful hardware.
  - [State size limits](/launch-arbitrum-chain/chain-config/costs/gas-target): Guidance for setting your chain's gas target
  - [State growth guidance](/launch-arbitrum-chain/operate/state-growth): Impact of high gas targets on state growth and its consequences
