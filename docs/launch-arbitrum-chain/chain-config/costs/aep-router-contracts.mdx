...
The `RewardDistributor` can be _permissionlessly_ called to perform a withdrawal, which simultaneously transfers 90% of accrued fees to the chain’s fee collector and @@aepRevenueSharePercent=10@@% of accrued fees to a target address on the <a data-quicklook-from="parent-chain">parent chain</a>. From here, the chain owner has complete control over their earned fees, and the routing contracts can direct AEP fees to a collecting address for the Arbitrum DAO.
