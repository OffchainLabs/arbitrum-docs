...
Layer-3 Arbitrum chains can make their own determination about what level of finality to accept. We consider it safe to rely on soft finality, <should we change that, Derek?> which would practically enable an Arbitrum chain to configure fast withdrawals down to 15 seconds.
