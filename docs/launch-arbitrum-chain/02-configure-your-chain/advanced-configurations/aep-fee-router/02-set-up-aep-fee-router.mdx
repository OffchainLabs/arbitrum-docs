...
Starting with arbitrum-orbit-sdk v0.24.2, you can deploy your own ERC-20 router on Base programmatically.

# When you need this 

You should deploy your own ERC-20 router if:

Your chain’s gas token or AEP-fee-denominated token is an ERC-20 on Base

You want to route AEP fees upward into the standard AEP router chain

The canonical Base router only supports ETH
...
import { deployChildToParentRouterForErc20 } from "@arbitrum/orbit-sdk";

async function main() {
  const tx = await deployChildToParentRouterForErc20({
    baseRpcUrl: process.env.BASE_RPC_URL,
    parentRpcUrl: process.env.PARENT_RPC_URL,
    parentChainId: Number(process.env.PARENT_CHAIN_ID),
    erc20Address: process.env.ERC20_TOKEN,
    targetOnParent: process.env.PARENT_TARGET_ADDRESS,
    deployerPrivateKey: process.env.DEPLOYER_PK,
  });
