...
- Chain SDK >= xx.xx **or** <!-- what Chain SDK version??? -->
...
- genesis-file-generator tool
  - If you want to customize the `genesis.json` file or pass accounts/custom chain config, you should use the [`genesis-file-generator`](https://hub.docker.com/r/offchainlabs/genesis-file-generator).
  <!-- it seems a little misleading, as you eventually feed the output to the Chain SDK -->
...
<!-- the ARBOS_VERSION VARIABLE HERE DIFFERS FROM ABOVE -->
...
| `ENABLE_TRANSACTION_FILTERING` | (optional) Set to `true` if you want to launch your chain with protocol-level transaction filtering for regulatory or compliance purposes.                                                                                                                  | `false`               | <!-- is this going to be in Nitro 3.10.0 or ArbOS 60??? --> |
