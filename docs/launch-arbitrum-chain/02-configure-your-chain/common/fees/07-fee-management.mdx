...
Arbitrum chains automatically adjust the Arbitrum chain surplus fee based on the traffic of the chain. If the gas consumed goes over the target, the chain's base fee will start increasing. Likewise, the base fee will gradually go down if demand of gas returns to below the configured target, until it reaches the minimum base fee configured.
