...
:::info
This setting is only available starting with [ArbOS 51 Dia](/run-arbitrum-node/arbos-releases/arbos51).
:::

- **Description**: This refers to the minimum gas price floor applied for batch posting to [EIP-7623](https://eips.ethereum.org/EIPS/eip-7623) enabled parent chains. It ensures that transactions with large calldata pay enough to cover data posting costs.
- **Default**: Varies based on parent chain -
  - If your chain posts calldata to Ethereum, the value should be set to **10**, as specified in the [EIP](https://eips.ethereum.org/EIPS/eip-7623). \
