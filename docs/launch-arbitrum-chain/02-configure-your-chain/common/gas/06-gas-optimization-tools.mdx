...
- **[RECOMMENDED] Multiple Targets (supported starting [ArbOS 51 Dia](/run-arbitrum-node/arbos-releases/arbos51))**

  - **Description**: Provides smoother price transitions. It uses multiple targets to reduce the frequency and severity of child chain gas price spikes.
...
- **Single Target**
  - **Description**: Uses a single target as the reference for price adjustments. If gas usage exceeds this limit, the base fee rises to throttle demand; if usage is below, the price moves towards the floor.
