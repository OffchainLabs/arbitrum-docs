...
Arbitrum chains support a new gas pricing algorithm that uses multiple targets, as part of the [ArbOS 51 upgrade](/run-arbitrum-node/arbos-releases/arbos51), to reduce the frequency and severity of child-chain gas price spikes. This is the first optimization of many towards Dynamic Pricing.

- This change is **backward compatible**. Setting new targets is **not mandatory**; if no new configuration is set, the legacy gas pricing algorithm with a single target remains active.
- **Additional details:** For those interested in how these values are derived, refer to the [Gas and fees](/how-arbitrum-works/deep-dives/gas-and-fees#the-gas-target) page as it provides the underlying parameters used to calculate the new targets and adjustment windows.

:::note

The upgrade to ArbOS 51 doesn't require activation of these new targets. The new targets are completely opt-in, so you can choose when to enable them.

:::

## Purpose

This improvement aims to reduce the severity, frequency, and duration of high L2 gas prices during periods when demand exceeds the target.
...
You can call `SetGasPricingConstraints` on the [ArbOwner precompile](/for-devs/dev-tools-and-resources/partials/precompile-tables/_ArbOwner) with an array of targets (the code refers to these targets as "constraints"), where each constraint contains `[gasTargetPerSecond, adjustmentWindowSeconds, startingBacklogValue]`.
...
Offchain Labs has developed a [script to help calculate pricing constraints](https://colab.research.google.com/drive/1D28YghOmCjvvrLWS5FsTp5xNKz9JHAA3?usp=sharing), which takes in certain parameters to find the most optimized targets to use for your chain. The script takes in the following params:

- `long_term_gas_target` is the desired long-term average gas throughput for the chain. A higher target means that your chain can handle more TPS before the gas begins to spike. However, a higher target also leads to faster state growth and requires more powerful hardware.
  - [State size limits](/launch-arbitrum-chain/maintain-your-chain/guidance/state-size-limit): Guidance for setting your chains target
  - [State growth guidance](/launch-arbitrum-chain/maintain-your-chain/guidance/state-growth): Impact of high targets on state growth and its consequences
...
- `long_term_adjustment_window_seconds` is the maximum timeframe over which gas prices adjust. This window specifically applies to the **lowest target** to ensure price stability during low-demand periods.

The values listed below are used for calculating the targets for Arbitrum One. These values have been calculated based on various UX and research considerations.
...
Arbitrum chain teams can use the following recommended values for the script parameters. The script will generate the optimized targets and adjustment windows for your chain, which you can then use to enable the new pricing system.
