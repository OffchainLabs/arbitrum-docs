...
While Native Mint/Burn was introduced in <a data-quicklook-from="arbos">ArbOS</a> 41, we strongly recommend that teams upgrade to [ArbOS 51 Dia](/run-arbitrum-node/arbos-releases/arbos51) to enable this feature.
...
| `go-ethereum` fork | Included in [ArbOS 51 Dia](/run-arbitrum-node/arbos-releases/arbos51) (please use the latest <a data-quicklook-from="wasm">WASM</a> module root) or later |
...
- <a data-quicklook-from="outbox">Outbox</a> sweep bot: run once per hour to prevent stray collateral
  build-up.
