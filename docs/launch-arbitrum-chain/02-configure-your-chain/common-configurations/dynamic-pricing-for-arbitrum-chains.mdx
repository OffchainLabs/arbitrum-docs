...
Arbitrum chains are going to be using a new gas pricing algorithm that uses multiple-gas targets, as part of the [ArbOS 51 upgrade](../../../run-arbitrum-node/arbos-releases/arbos51) to reduce the frequency and severity of child chain gas price spikes. This is the first optimization of many towards Dynamic Pricing.
...
- **The Change:** The new algorithm uses multiple gas targets and multiple adjustment time windows to balance traffic spikes with overall fee stability.
...
Offchain Labs has developed a [script to help setting pricing constraints](https://colab.research.google.com/drive/1D28YghOmCjvvrLWS5FsTp5xNKz9JHAA3?usp=sharing) which takes in certain parameters to find the most optimized gas targets to use for your chain. The script takes in below params:
...
- long_term_gas_target \- Desired long term average gas throughput for the chain. A higher gas target means that your chain can handle more TPS before the gas begins to spike. However, a higher gas target also leads to faster state growth and requires more powerful hardware.
  ...
  [State Size Limits](../../maintain-your-chain/guidance/state-size-limit): Guidance for setting your chains gas target
...
| gas_limit                               | 128 Mgas/s          | Based on the block limit for the chain.Can be set to (MaxBlockGasLimit \+ MaxEffectiveGasLimit) / 2                  |
