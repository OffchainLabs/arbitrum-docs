...
The recommended way to get the current configured batch posters is to use the Arbitrum chain (Orbit) SDK to query the `SequencerInbox` contract on the parent chain. This method reads from events emitted when configuring a new batch poster:
...
You can also query batch posters by calling the `getBatchPosters()(address[])` method of the [`ArbAggregator`](/build-decentralized-apps/precompiles/02-reference.mdx#arbaggregator) precompile on your Orbit chain:
...
If you want to configure a fee collector before the batch poster sends its first batch, you can optionally pre-register the address on the Orbit chain by calling `addBatchPoster(address)` of the [`ArbAggregator`](/build-decentralized-apps/precompiles/02-reference.mdx#arbaggregator) precompile:
