...
- [WARN]-The L1 node couldn't give state at the finalized bound block, so it falls back to the latest block — usually transient.

#### `unknown L1 block bound config value; falling back on using finalized`. The L1 node couldn't give state at the finalized bound block, so it falls back to the latest block — usually transient.
