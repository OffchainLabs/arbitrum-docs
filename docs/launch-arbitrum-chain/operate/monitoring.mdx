...
- **RBlocks (nodes) created in the Rollup contract on the parent chain**: RBlocks are created by validators and contain [assertions](/how-arbitrum-works/deep-dives/assertions.mdx) of the current state of the chain (viewed by the validators). If RBlocks are not being created or confirmed on the parent chain, further analysis should be conducted to understand why.
