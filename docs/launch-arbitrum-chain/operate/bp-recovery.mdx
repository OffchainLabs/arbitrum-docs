...
This section covers how the batch poster recovers state after a crash, restart, or bad start—including DB restore from a batch-poster checkpoint, storage backends, revert/half recovery, Redis failover, nonce sync, and reorg handling.
...
2. If the primary crashes, the lock **expires after `LockoutDureation`**; a backup with `background-lock` acquires it.
...
So failover state-sharing rides on persistent, signed Redis storage plus L1-derived nonce/position.
...
- _Corrupt/stuck queue_ → restart with `data-poster.dangerous.clear-dbstorage` (once), then
  remove it.
- _Batch reverted / poster halted_ → investigate the revert, then restart to clear
  `batchReverted`.
- _Message-count mismatch after force-inclusion / long downtime_ → restart with
