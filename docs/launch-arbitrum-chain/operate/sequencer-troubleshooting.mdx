...
A relay does not sequence, validate, or execute anything. It reads an upstream feed and distributes it to more subscribers. The message content is identical either way, so a <a data-quicklook-from="client">client</a> cannot tell from the payload whether it is connected to a sequencer or a relay.
...
For installation steps, see [How to run a feed relay](/run-arbitrum-node/data-availability/run-feed-relay.mdx). For the message format, see [How to read the sequencer feed](/run-arbitrum-node/sequencer/02-read-sequencer-feed.mdx).
...
| `sequencer temporarily not available`                                                | This node is forwarding rather than sequencing, and its <a data-quicklook-from="forwarder">forwarder</a> is disabled |
