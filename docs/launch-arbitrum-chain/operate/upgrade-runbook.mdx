...
| `unexpected wasmModuleRoot! cannot validate! found <onchain>, current <x>, pending <y>` | Yes          | The onchain root changed to a root that is neither the validator's current nor its pending root, so the validator refuses to adopt it. The legacy staker then stops acting entirely; BoLD logs a warning and continues |
