...
| `ResolveNodes` | Bonds on and resolves assertions **that already exist**. It never creates new ones, so a chain whose validators are all `ResolveNodes` will stop advancing. At least one `MakeNodes` validator is required to post new assertions. |
...
| `latest`    | No lag, full reorg exposure. The validator may act on data that later disappears.                                                                             |
