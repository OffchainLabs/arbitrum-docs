...
Nect, navigate to (arbitrum_signer.go)[https://github.com/OffchainLabs/go-ethereum/blob/v1.12.2/core/types/arbitrum_signer.go] and add the new precompile address.
...
var ArbHiAddress = common.HexToAddress("0x11a")
```
...
```go
insert(MakePrecompile(pgen.ArbHiMetaData, &ArbHi{Address: types.ArbHiAddress})) // 0x011a here is an example address
```
