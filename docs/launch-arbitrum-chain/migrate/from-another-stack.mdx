...
This document is provided for informational purposes only and does not constitute an end-to-end guide for executing the migration. <a data-quicklook-from="offchain-labs">Offchain Labs</a> doesn’t have special knowledge or a private “fast path.” This document provides the extent of our guidance. Your current <a data-quicklook-from="raas">RaaS</a> provider owns and executes the migration. We can provide [introductions to RaaSes](../integrations/infrastructure-providers#rollup-as-a-service-raas-providers).
...
- **Lower fees**: <a data-quicklook-from="arbitrum-chain">Arbitrum chain</a> Rollup or [AnyTrust](/how-arbitrum-works/deep-dives/anytrust-protocol.mdx) for low <a data-quicklook-from="transaction">transaction</a> fees.
- **Various customization options**: Custom gas token, flexible DA, [Timeboost](/how-arbitrum-works/timeboost/gentle-introduction.mdx), [BoLD](/how-arbitrum-works/bold/gentle-introduction.mdx), etc.
...
Zero knowledge (zk) pre-compiles don’t carry over. Port logic to Solidity or [Stylus](/stylus/gentle-introduction.mdx) or remove/replace functionality. Please check with your RaaS regarding this matter.
...
1. Freeze legacy chain writes and make RPC read-only:
   1. Stop the legacy chain’s proposer/[Sequencer](/how-arbitrum-works/deep-dives/sequencer.mdx) so no new blocks or transactions get created after the chosen upgrade block.
...
- [ ] Testnet deployment: Sequencer, [Batch Poster](/how-arbitrum-works/deep-dives/assertions.mdx), <a data-quicklook-from="bonder">Bonder</a>, DA config.
