...
2. There is a current Geth-based client that can extract data.
3. Upgrade will require a few hours of downtime (also depends on the size of their state)
4. Nitro nodes will be used as full nodes, which include the previous chain’s history
...
6. For the dual-node full-archive mode, the Nitro node needs to be able to detect which requests are getting directed to the previous stack’s node and forward them
