---
title: 'Migrate to Arbitrum chain from another stack'
...
- Ecosystem migration (simpler, faster): launch a fresh Arbitrum (Orbit) chain. Users/app bridge funds and redeploy contracts. This is faster and less risky, but puts a burden on apps and users.
- Chain-state migration (harder, slower): transplant state/history so apps keep addresses and positions. This requires more engineering, testing, and downtime planning.
...
Teams should determine the canonical bridge for each asset and the authoritative token addresses before the handover. This is important because confusion here could cause liquidity fragmentation and cause users to bridge the “wrong” way.

Recommended actions:

- Publish an official asset list (symbols, decimals, canonical token address, and canonical bridge) before migration.
- Enforce a one-way migration from the legacy chain → new Arbitrum (Orbit) chain (freeze legacy deposits while allowing Arbitrum (Orbit) chain deposits).
- Coordinate with third-party bridges/routers to repoint routers.
...
Problem: After migration, historical data lives on the old stack, while new data lives on Arbitrum (Orbit) chain.
...
True zero-downtime is not realistic for a Rollup stack migration; however, you can minimize user-visible downtime.
...
Important: Pause withdrawals earlier than the main freeze window to ensure no funds are trapped in in-flight bridges (i.e., deposits paused at the start of the freeze and withdrawals paused earlier, as per bridge requirements).

#### How can we migrate without losing state? Ideally, we would like to keep the full state

You can use the chain-state migration process. Two common approaches:

- State replay: re-execute legacy blocks into the Arbitrum (Orbit) chain. This process can be heavy, but it can preserve more history. **Caution: I**f the legacy stack has different opcode semantics, precompiles, or system contracts, replay can diverge, resulting in a different end state even with identical transactions. Validate equivalence early on a testnet.
...
zk pre-compiles don’t carry over. Port logic to Solidity or Stylus or remove/replace functionality. Please check with your RaaS regarding this matter.
...
- **Chain state migration:** A more complex migration where the chain state is transplanted into the new rollup stack / contract system, but the ecosystem and user experience may stay relatively the same.
...
1. Freeze legacy chain writes and make RPC read-only:
   1. Stop the legacy chain’s proposer/sequencer so no new blocks or transactions are produced after the chosen upgrade block.
...
2. There is a current Geth-based client from which data can be extracted.
3. Upgrade will require a few hours of downtime (also depends on the size of their state)
4. Nitro nodes will be used as full nodes, which include pthe revious chain’s history
5. Full archive functionality (fetching data from the state) will require an archive node from the previous stack to be running in parallel. This archive node wouldn’t try to advance beyond the upgrade block.
6. For the dual-node full-archive mode, the Nitro node needs to be able to detect which requests are directed to the previous stack’s node and forward them
...
Below is a rough outline of the tasks that would need to be completed to ensure a smooth cutover, assuming a 30-day lead time until chain migration. Please consult with your RaaS provider for the most accurate guidance on this matter, as our checklist may not be comprehensive.

#### 2 months before → 4 weeks before migration: planning and thinking

- [ ] Choose migration path
- [ ] Select Arbitrum (Orbit) chain mode: Rollup (ETH DA) vs. AnyTrust (DA committee)
...
- [ ] RaaS: set up private Arbitrum (Orbit) chain testnet deployment: sequencer, batch poster, staker, DA config.
