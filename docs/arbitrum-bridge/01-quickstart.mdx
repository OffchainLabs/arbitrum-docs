...
The only prerequisite for this quickstart is to have a Web3 <a data-quicklook-from="wallet">Wallet</a> installed, such as MetaMask or OKX Wallet. If you don't have one installed, visit [Arbitrum's portal](https://portal.arbitrum.io/?categories=wallet) for a list of wallets to choose from to download.

## Deposit `ETH` or `ERC-20` tokens (from parent chain to child chain)

### Step 1: Get some native currency

You'll need the native currency of the parent chain to <a data-quicklook-from="bridge">Bridge</a> your assets to the destination chain. For example, if you want to bridge assets from Ethereum to Arbitrum One, you'll need `ETH` on Ethereum to initiate the process.
