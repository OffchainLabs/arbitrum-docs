...
The first thing to do is find which batch your child chain transaction is in. With that info, you can query the `SequencerInbox` contract to find the block number that the batch was posted in, giving you certainty that the original parent chain message was sent before the block with the posted batch. From there, search through the `Bridge` or `DelayedInbox` contract for their `MessageDelivered` and `InboxMessageDelivered` logs (respectively). You can then wrap the transaction that was responsible for emitting those logs with either the `ParentToChildMessage` or `ParentEthDepositTransactionReceipt` depending on what you're looking for. When wrapped, you can check for their respective messages as shown above, and if the hash calculated matches the hash you have, then that is the original parent chain message.
