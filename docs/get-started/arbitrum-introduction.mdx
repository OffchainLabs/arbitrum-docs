...
Smart contracts are simply a program that is deployed to a blockchain that runs exactly as written, automatically, when called (hence the smart part). In short, smart contracts are:
...
Arbitrum runs the same EVM as Ethereum, so any smart contract that works on Ethereum works on Arbitrum—just faster and cheaper, because execution happens on the L2 (Arbitrum) and only compressed results get posted to L1 (Ethereum).

If you want to build on Arbitrum, check out the smart contract quickstart pages:

- [Solidity](/build-decentralized-apps/01-quickstart-solidity-remix.mdx)
- [Stylus](/stylus/quickstart.mdx)

#### Why does Ethereum need help scaling? Is there something wrong?

Ethereum is awesome, and there is _nothing_ wrong with it! The constraints are the consequences of intentional design choices that prioritize decentralization and security. The tradeoff is known as the blockchain trilemma: it's hard to balance decentralization, security, and scalability simultaneously. Ethereum deliberately optimizes for the first two, which caps the third.
...
A rollup aggregates (sequencing on Arbitrum) thousands of transactions into batches. Then it executes them on its own infrastructure, and posts compressed data and a state commitment back to L1. Users still inherit Ethereum's security, but throughput goes up by orders of magnitude and fees drop dramatically.

If you want to learn more about Arbitrum, refer to the [How it works](/how-arbitrum-works/01-inside-arbitrum-nitro.mdx) section.
...
The combination of these factors means Ethereum transaction throughput has to be capped fairly low. The intent of the roadmap is to have L2's like Arbitrum scale and improve TPS.

#### And Arbitrum fixes this?

Yes; but "fixes" deserves a careful definition. Arbitrum doesn't make Ethereum faster; it lets you transact at much higher throughput while still inherting Ethereum's security.
...
- **Settlement/data availability**, agreeing on the canonical result and making sure the data is publicly retrievable.
...
If you want to learn more about BoLD—the dispute protocol (fraud proofs), check out the [BoLD gentle introduction](/how-arbitrum-works/bold/gentle-introduction.mdx).

#### But who does the work and challenges?

Any party can validate Arbitrum's chain state. Anyone who chooses to do so is called a validator. Most people will not elect to be a validator on Arbitrum—just like most won't run their own Ethereum staking node.

The beauty of the fraud-proof system on Arbitrum is that it takes only a single honest validator to keep the chain secure. A single honest validator can catch multiple malicious actors. This makes the system trustless—users are not relying on any single or designated party to keep their funds secure.

To learn more about validators and the different types of validators, refer to the [Run a validator node](/run-arbitrum-node/more-types/02-run-validator-node.mdx) page.

#### So how is fraud proven? It sounds complicated...

**TLDR**: two validators disagree. The disagreement would occur over an executed transaction. Since that data is posted to Ethereum—only one of the validators is telling the truth.
...
To learn more about bridging, refer to the [Token bridging](/how-arbitrum-works/deep-dives/token-bridging.mdx) page. If you want to learn how to bridge tokens, refer to the [Arbitrum bridge quickstart](/arbitrum-bridge/01-quickstart.mdx) page.
...
If you want to learn about Arbitrum, refer to the [How it works](/how-arbitrum-works/01-inside-arbitrum-nitro.mdx) section.

#### As far as using Arbitrum, it's similar to Etheruem?

**TLDR**: For a user, it's Ethereum, but faster and cheaper. You bridge in, do stuff, bridge out. The mental model is mostly the same. Wallets, apps, addresses—all work.
...
Much development went into achieving this level of Ethereum compatibility. But at its core, Arbitrum itself uses a fork of [Geth](http:///how-arbitrum-works/reference/geth.mdx)—the most widely used Ethereum implementation — with modifications to transform it into a trustless layer 2. This means that most of the code running on Arbitrum is identical to the code running on Ethereum. We call this cutting-edge approach Nitro (developers can see the [Nitro codebase](https://github.com/OffchainLabs/nitro)).

If you want to know more about the differences between Arbitrum and Ethereum, refer to the [Comparison overview](http:///build-decentralized-apps/arbitrum-vs-ethereum/01-comparison-overview.mdx).
...
You can also create your own Arbitrum chain with custom gas tokens. For example, you could create a chain that uses USDC for gas. Arbitrum chains are endlessly customizable beyond the gas token. You can learn more about Arbitrum chains [here](/launch-arbitrum-chain/01-a-gentle-introduction.mdx).
...
If you want to learn more about AnyTrust, refer to the [AnyTrust protocol](/how-arbitrum-works/deep-dives/anytrust-protocol.mdx) page.
...
If you want to create your own chain, check out the [Arbitrum chain introduction](http:///launch-arbitrum-chain/01-a-gentle-introduction.mdx).
