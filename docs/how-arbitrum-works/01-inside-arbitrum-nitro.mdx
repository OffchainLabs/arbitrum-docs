...
Once the transactions are sequenced, they are run through the [_state transition function_](/how-arbitrum-works/deep-dives/stf.mdx), one by one. The state transition function (STF) takes as input the current state of the chain (account balances, contract code, and so on) along with the next transaction. It updates the state and sometimes emits a new Layer 2 block on the Nitro chain.
...
- [State Transition Function: Gentle Introduction](/how-arbitrum-works/deep-dives/stf.mdx): Foundational concepts of the STF
