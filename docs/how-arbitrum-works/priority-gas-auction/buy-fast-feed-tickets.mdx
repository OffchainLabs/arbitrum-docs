...
The <a data-quicklook-from="fast-feed">Fast Feed</a> admits one WebSocket connection per ticket, and a ticket is valid for one round. To keep a connection open, you buy a new ticket every round from the `Tickets` contract on <a data-quicklook-from="arbitrum-one">Arbitrum One</a>. This guide shows you how to buy tickets by hand, then how to run the reference bot that buys them for you.

To learn what the Fast Feed publishes and how rounds and prices work, read the [introduction to the Fast Feed](/how-arbitrum-works/priority-gas-auction/fast-feed) first. The Fast Feed publishes within Priority Gas Auction (PGA) rounds, which the [introduction to PGA](/how-arbitrum-works/priority-gas-auction/pga) explains.

<VanillaAdmonition type="info">

The `Tickets` contract address on Arbitrum One appears on the [contract addresses page](/arbitrum-essentials/reference/contract-addresses) once the contract is live. The examples below use `$TICKETS` as a placeholder for that address and `$USDG` for the payment token address.
...
- The `Tickets` contract address and the USDG token address.
