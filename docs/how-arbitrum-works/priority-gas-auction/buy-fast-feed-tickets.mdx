...
Round state updates lazily: the first state-changing call in a new round rolls it forward. View functions compute the rolled-forward values on the fly, so the values you read are current. Before the first round starts, these views revert with `BeforeFirstRoundStart()`.
