...
The Fast Feed admits one WebSocket connection per ticket, and a ticket is valid for one round. To keep a connection open, you buy a new ticket every round from the `Tickets` contract on Arbitrum One. Rounds follow the Priority Gas Auction (PGA) cadence, which the [introduction to PGA](/how-arbitrum-works/priority-gas-auction/pga) explains.
...
- [Foundry's `cast`](https://getfoundry.sh/), or any EVM client, to send transactions. The examples use `cast` with `--rpc-url $RPC_URL --private-key $PRIVATE_KEY`. The examples leave those flags out to stay short.
