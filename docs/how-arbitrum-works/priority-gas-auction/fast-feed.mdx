...
2. **Buy a ticket.** Send a payment transaction to the `Tickets` contract that includes the Keccak-256 hash of your key. The contract records that hash as your Subscription Credential.
...
For the contract calls, the WebSocket handshake, the reference purchase bot, and the errors you can hit, see [how to connect to the Fast Feed](/how-arbitrum-works/priority-gas-auction/connect-to-fast-feed).
...
The Arbitrum Treasury Management (ATM) Council and OAT selected USDG as the stablecoin. The `Tickets` and USDG addresses are listed in [how to connect to the Fast Feed](/how-arbitrum-works/priority-gas-auction/connect-to-fast-feed#addresses-and-endpoints).

</VanillaAdmonition>

Trail of Bits audited both contracts before deployment. The `Tickets` contract is audited under the name "Sequencer Feed Ticketing", so read the [Sequencer Feed Ticketing report](../../hosted-pdfs/audit-reports/2026_07_31_sequencer_feed_ticketing_summary_report.pdf) for that contract, and the [Reward Distributor Fixes report](../../hosted-pdfs/audit-reports/2026_06_16_trail_of_bits_security_audit_reward_distributor_fixes_summary_report.pdf) for the RewardDistributor. The [security audit reports page](/audit-reports) lists every Arbitrum audit.
