...
**It does not publish state.** You receive a list of transactions and an incomplete receipt for each one. You do not receive state changes or partial blocks.
...
Subscribe to the Fast Feed if earlier sight of ordered transaction flow changes what you can do:
...
If you send ordinary transactions through a wallet or an app, the Fast Feed does not affect you. It changes no ordering, no inclusion guarantee, and no fee that anyone pays.
...
**Receipts are incomplete.** A receipt omits `block_hash`, because the block is not final when the Sequencer publishes it.
...
The contract sells tickets in fixed rounds and prices them with the mechanism defined in [EIP-4844](https://eips.ethereum.org/EIPS/eip-4844). Every ticket in a round costs the same. Demand in one round sets the price for the next: sales above the target push the price up, sales below it push the price down, and the price never falls below the floor.
...
Trail of Bits audited both the Fast Feed Payment Contract and the RewardDistributor before deployment. You can read the reports on the [audit reports page](/audit-reports).
...
## When the Fast Feed activates

PGA and the Fast Feed reach the Arbitrum DAO as one constitutional Arbitrum Improvement Proposal (AIP) with one execution payload. Each proposal passed a separate Snapshot temperature check before the two merged into a single onchain vote.

Executing that payload grants the authority to run both products. It does not start either one. The Fast Feed begins broadcasting after the ATM Council selects the payment asset, the contracts deploy, and testing and operational readiness work finish. PGA activates on its own schedule, so the two can go live on different days.
