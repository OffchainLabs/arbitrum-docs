...
| Component             | Role                                                               |
| --------------------- | ------------------------------------------------------------------ |
| `Sequencer`           | Receives transactions, queues them, drives block creation          |
| `ExecutionEngine`     | Produces blocks by executing transactions against ArbOS state      |
| `TransactionStreamer` | Persists sequenced messages to DB, bridges consensus and execution |
| `SeqCoordinator`      | Redis-based leader election for sequencer HA                       |
| `BatchPoster`         | Compresses and posts message batches to L1 SequencerInbox          |
| `DelayedSequencer`    | Sequences L1-originated messages (deposits, retryables) into L2    |
| `Broadcaster`         | WebSocket feed server pushing sequenced messages to nodes          |
| `ExpressLaneService`  | Manages Timeboost Express Lane transaction ordering                |
