...
This section covers batch posting from the Sequencer's side. For the batch poster's internals, see [The batch poster](/how-arbitrum-works/deep-dives/batchposter.mdx). That page traces one batch from the transaction streamer to the parent chain transaction.
...
Now that transactions are batched and compressed, they pass to the [batch poster](/how-arbitrum-works/deep-dives/batchposter.mdx) for transmission to the parent chain.
