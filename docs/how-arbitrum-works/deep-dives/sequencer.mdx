...
Independent of the loop above, the BatchPoster reads sequenced messages from the TransactionStreamer, Brotli-compresses them with adaptive compression levels, and posts them to the SequencerInbox using either calldata or EIP-4844 blobs, as described in the [batch poster](/how-arbitrum-works/deep-dives/batchposter.mdx) documentation. Once those batches are finalized on the parent chain, the transactions reach hard finality.
