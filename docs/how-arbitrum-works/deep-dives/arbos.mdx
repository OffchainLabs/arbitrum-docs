...
ArbOS is the child-EVM virtual machine monitor (VMM) for <a data-quicklook-from="arbitrum-nitro">Arbitrum Nitro</a>: a trusted system component that lives inside the [State Transition Function (STF)](/how-arbitrum-works/deep-dives/01-stf-gentle-intro.mdx) and provides the execution environment for the chain. Unlike a conventional operating system, ArbOS is not a process running beside the chain—it's ordinary Go code compiled into the STF, so every node (and <a data-quicklook-from="fraud-proof">fraud prover</a>) can replay it deterministically.

## Core responsibilities

1. **Network resource management**: Allocating and tracking the required resources to execute <a data-quicklook-from="child-chain">child-chain</a> transactions and pricing them.
...
- Parent-chain data is tracked by `l1PricingState`, which records what the [batch poster](/how-arbitrum-works/deep-dives/sequencer.mdx#batch-posting) spent posting compressed data to Ethereum and reimburses the batch poster from user fees.
