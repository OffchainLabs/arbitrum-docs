...
3. **Cross-chain messaging**: Enables cross-chain communication between parent and child chains for deposits, withdrawals, and <a data-quicklook-from="retryable-ticket">retryable tickets</a>.
4. **Enhanced EVM execution**: Running instrumented Geth to execute smart contracts with child-chain specific logic.
5. **<a data-quicklook-from="stylus">Stylus</a> support**: Managing host I/O, memory, and execution context for <a data-quicklook-from="wasm">WASM</a>-based contracts.

## Architecture: the Geth "sandwich"

Rather than implementing the EVM, Arbitrum embeds it. The core of execution is _unmodified-in-spirit_, and still <a data-quicklook-from="geth">Geth</a>—the same engine that defines the Ethereum standard. ArbOS wraps Geth on both sides, an arrangement we call the sandwich.
...
- Parent-chain data is tracked by `l1PricingState`, which records what the [batch poster](/how-arbitrum-works/deep-dives/batchposter.mdx) spent posting compressed data to Ethereum and reimburses it from user fees.
