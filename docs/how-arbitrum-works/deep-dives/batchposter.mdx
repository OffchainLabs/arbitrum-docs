...
6. Sign it with the poster's key.
...
2. When a transaction has been waiting past its scheduled replacement time, it resends the **same** transaction with a higher fee (replace-by-fee).
...
3. The transaction body itself only carries lightweight `BlobHashes` (versioned hashes) that point at the blobs.
...
1. "Backlog" answers: roughly how many batches' worth of messages are waiting but not yet posted?
...
3. It raises the fee bid (squared) so batches compete harder for L1 inclusion.
...
1. The delay buffer is an onchain mechanism that rewards the poster for including delayed (L1-submitted) messages promptly.
...
`LookupBatchesInRange` filters `SequencerBatchDelivered` events from the SequencerInbox contract, parses them into `SequencerInboxBatch` structs with time bounds, accumulator hashes, and data location.
...
2. A known set of "expected" errors gets a longer grace period (debug, then warn, then error) before it bothers an operator.
