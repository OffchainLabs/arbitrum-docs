...
// Get the latest confirmed assertion
const assertionHash = await rollup.latestConfirmed();
// Query AssertionCreated events and check the afterState for the last processed block
```

For a complete working example, see the [block-verification tutorial](https://github.com/OffchainLabs/arbitrum-tutorials/tree/master/packages/block-verification-in-parent-chain-assertion).
