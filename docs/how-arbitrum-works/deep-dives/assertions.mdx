...
The Rollup chain consists of assertions, which serve as checkpoints summarizing multiple child chain blocks. Assertions are what give Arbitrum its <a data-quicklook-from="hard-finality">hard finality</a> guarantee — for the broader soft- vs. hard-finality picture, see [Finality](/how-arbitrum-works/deep-dives/finality.mdx).
...
:::note Validators and <a data-quicklook-from="proposer">proposers</a> serve different roles

Validators validate transactions by computing the next <a data-quicklook-from="chain-state">chain state</a> using the chain's [STF](/how-arbitrum-works/deep-dives/stf.mdx), whereas proposers can also assert and challenge the chain state on the <a data-quicklook-from="parent-chain">parent chain</a>.
