...
The [batch poster](/how-arbitrum-works/deep-dives/batchposter.mdx#4-the-data-is-routed-to-storage-da-handoff) drives this exchange. It hands the sealed batch to the DAC through a single storage interface, so the same code path serves a DAC, a custom backend, or Ethereum itself.
