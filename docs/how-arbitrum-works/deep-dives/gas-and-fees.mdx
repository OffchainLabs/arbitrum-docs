...
- **Compression complexity**: The data posted to the parent chain is compressed using a general-purpose compression algorithm ([Brotli](/how-arbitrum-works/deep-dives/sequencer.mdx#posting-to-the-parent-chain)). The effectiveness of compression depends on shared patterns among transactions in a batch.
...
For an operational overview of how the Sequencer applies these levels, see [Posting to the parent chain](/how-arbitrum-works/deep-dives/sequencer.mdx#posting-to-the-parent-chain). Dynamic adjustments of compression levels are based on backlog size ($B$):
...
The sequencer prioritizes transactions on a first-come, first-served basis (chains running [Timeboost](/how-arbitrum-works/timeboost/gentle-introduction.mdx) modify this for express-lane transactions). Because tips do not make sense in this model, they are ignored. Arbitrum users always pay the basefee regardless of the tip they choose.
