...
- **Compression complexity**: The data posted to the parent chain is compressed using a general-purpose compression algorithm ([Brotli](/how-arbitrum-works/deep-dives/batchposter.mdx#3-messages-become-a-compressed-batch)). The effectiveness of compression depends on shared patterns among transactions in a batch.
...
For an operational overview of how the Sequencer applies these levels, see [Compression](/how-arbitrum-works/deep-dives/batchposter.mdx#3-messages-become-a-compressed-batch). Dynamic adjustments of compression levels are based on backlog size ($B$):
