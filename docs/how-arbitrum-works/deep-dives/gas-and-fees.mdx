...
The algorithm compares gas usage against a parameter called the [gas target](#the-gas-target) which is the target amount of gas per second that the chain can handle sustainably over time. The algorithm tracks a gas backlog. Whenever a transaction consumes gas, that gas is added to the backlog. Whenever the clock ticks one second, the gas target is subtracted from the backlog; but the backlog can never go below zero.
...
To make this more precise, the basefee is an exponential function of the backlog, $F = e^{-a (B - b)}$, where $a$ and $b$ are suitably chosen constants: $a$ controls how rapidly the price escalates with backlog, and $b$ allows a small backlog before the basefee escalation begins.
