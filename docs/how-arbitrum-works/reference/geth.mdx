...
We created the `AdvancedPrecompile` interface, which executes and charges gas with the same function call. [Arbitrum precompiles](/build-decentralized-apps/precompiles/01-overview.mdx) use this interface, and it wraps Geth's standard precompiles.
