...
const renderMethodsInTable = (interfaceCode: string, implementationCode: string, precompileName: string, methodOverrides?: PrecompileMethodOverrides) => {
  ...
  implementationCode.split('\n').forEach( line => {
    ...
    if (trimmedLine.startsWith('func')) {
      ...
      const methodDescription = previousLine.trim().startsWith('//') ? previousLine.split('//')[1].trim() : '';
  ...
  if (methodOverrides) {
    ...
    Object.keys(methodOverrides).map( (methodName: string) => {
      methodsInformation[methodName] = { ...methodsInformation[methodName], ...methodOverrides[methodName] };
    });
  }
  ...
  const tableHtml = `<table>
    ...
    <tbody>
      ${Object.values(methodsInformation).map( (methodInfo: PrecompileMethodInfo) => {
        ...
        return `<tr>
            ...
            <td><a href="${interfaceBaseUrl}${precompileName}.sol#L${methodInfo.interfaceLine}" target="_blank">Interface</a></td>
            <td><a href="${implementationBaseUrl}${precompileName}.go#L${methodInfo.implementationLine}" target="_blank">Implementation</a></td>
  ...
  const deprecationNotice = showDeprecationFlag ? `<p>Note: methods marked with ⚠️ are deprecated and we do not support their use.</p>` : '';
  
  return tableHtml + deprecationNotice;
}

const renderEventsInTable = (interfaceCode: string, implementationCode: string, precompileName: string, eventOverrides?: PrecompileEventOverrides) => {
  ...
  interfaceCode.split('\n').forEach (line => {
    ...
    if (trimmedLine.startsWith('event')) {
      ...
      const eventDescription = previousLine.trim().startsWith('//') ? previousLine.split('//')[1].trim() : '';
  ...
  if (eventOverrides) {
    ...
    Object.keys(eventOverrides).map( (eventName: string) => {
      eventsInformation[eventName] = { ...eventsInformation[eventName], ...eventOverrides[eventName] };
    });
  }
  ...
  if (Object.keys(eventsInformation).length > 0) {
    const tableHtml = `<table>
      ...
      <tbody>
        ${Object.values(eventsInformation).map( (eventInfo: PrecompileEventInfo) => {
          return `<tr>
...
const generatePrecompileReferenceTables = async (
  ...
  const interfaceCodeRawResponse = await fetch(`${interfaceBaseUrl.replace('github.com', 'raw.githubusercontent.com').replace('blob/', '')}${precompileName}.sol`)
  const interfaceCode = await interfaceCodeRawResponse.text()
  const implementationCodeRawResponse = await fetch(`${implementationBaseUrl.replace('github.com', 'raw.githubusercontent.com').replace('blob/', '')}${precompileName}.go`)
  const implementationCode = await implementationCodeRawResponse.text()
  
  const methodsTable = renderMethodsInTable(interfaceCode, implementationCode, precompileName, methodOverrides)
  const eventsTable = renderEventsInTable(interfaceCode, implementationCode, precompileName, eventOverrides)
