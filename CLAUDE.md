...
- **CI runs on push and PR to `main` ([`.github/workflows/ci.yml`](.github/workflows/ci.yml)) in three jobs — only the first one blocks.** A green PR does not mean the content is clean: nine checks block, three report and pass anyway.
  ...
  - **`Content debt` (non-blocking):** `format:check`, `content:lint` — both marked `continue-on-error`. `format:check` still fails on pre-existing debt (49 files as of 2026-09-21). `content:lint` reached zero on 2026-09-21 and is eligible to move up. The job comment records the counts as of the last update; **promote a step into `Gates` once its count reaches zero.** That promotion is the point of the split — the tier is a backlog, not a policy.
