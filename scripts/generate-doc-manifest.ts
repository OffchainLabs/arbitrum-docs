...
async function main() {
  ...
  const statsJson = JSON.stringify(
    {
      generated: compactManifest.generated,
      docsRoot: compactManifest.docsRoot,
      stats: compactManifest.stats,
    },
    null,
    2,
  );
  ...
  const summaryLines: string[] = [
    ...
    `| Field | Count | % |`,
    `|-------|-------|---|`,
    `| title | ${manifest.stats.frontmatter.withTitle} | ${pct(
      manifest.stats.frontmatter.withTitle,
      manifest.stats.totalFiles,
    )} |`,
    `| description | ${manifest.stats.frontmatter.withDescription} | ${pct(
      manifest.stats.frontmatter.withDescription,
      manifest.stats.totalFiles,
    )} |`,
    `| content_type | ${manifest.stats.frontmatter.withContentType} | ${pct(
      manifest.stats.frontmatter.withContentType,
      manifest.stats.totalFiles,
    )} |`,
    `| author | ${manifest.stats.frontmatter.withAuthor} | ${pct(
      manifest.stats.frontmatter.withAuthor,
      manifest.stats.totalFiles,
    )} |`,
    '',
