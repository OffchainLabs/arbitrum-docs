...
function isPartial(doc: CompactDoc): boolean {
  return !!doc.f?.par;
}

function isGenerated(doc: CompactDoc): boolean {
  return !!doc.f?.gen;
}
...
function generateFrontmatterAudit(docs: CompactDoc[]): string {
...
function generateNavigationAudit(docs: CompactDoc[]): string {
  const nonPartials = docs.filter((d) => !isPartial(d) && !isGenerated(d));

  // Orphaned: not in sidebar, not linked by other docs, not imported
  const allLinkedIds = new Set<string>();
  const allImportedIds = new Set<string>();
  for (const d of docs) {
    if (d.l) d.l.forEach((id) => allLinkedIds.add(id));
    if (d.im) d.im.forEach((id) => allImportedIds.add(id));
  }

  const orphaned = nonPartials.filter(
    (d) => (!d.sb || d.sb.length === 0) && !allLinkedIds.has(d.id) && !allImportedIds.has(d.id),
  );
  ...
  lines.push('## Docs in Multiple Sidebars', '', `${multiSidebar.length} docs appear in >1 sidebar.`, '');
  ...
  for (const [sec, stat] of Object.entries(sectionCoverage).sort(
    ...
    lines.push(`| ${sec} | ${stat.total} | ${stat.inSidebar} | ${pct(stat.inSidebar, stat.total)} |`);
  }
  lines.push('');
...
function generateContentMetrics(docs: CompactDoc[]): string {
  const nonPartials = docs.filter((d) => !isPartial(d) && !isGenerated(d));
  ...
  lines.push('## Very Long Docs (>3000 words)', '', `${veryLong.length} docs exceed 3000 words.`, '');
...
function generateSectionSummaries(docs: CompactDoc[], stats: ManifestData['stats']): string {
  const nonGenerated = docs.filter((d) => !isGenerated(d));
...
function generateActionablePriorities(docs: CompactDoc[]): string {
  const nonPartials = docs.filter((d) => !isPartial(d) && !isGenerated(d));

  // Build linked/imported sets
  const allLinkedIds = new Set<string>();
  const allImportedIds = new Set<string>();
  for (const d of docs) {
    if (d.l) d.l.forEach((id) => allLinkedIds.add(id));
  ...
  // Quick wins: missing title (easy to add)
  const missingTitle = nonPartials.filter((d) => !d.fm?.t);

  // SEO impact: missing description on pages that are in a sidebar
  const missingDescInSidebar = nonPartials.filter(
    (d) => !d.fm?.d && d.sb && d.sb.length > 0,
  );

  // Non-standard content types to normalize
  const nonStandardCt = nonPartials.filter(
    (d) => d.fm?.ct && !CANONICAL_CONTENT_TYPES.includes(d.fm.ct.toLowerCase()),
  );

  // Navigation: orphaned pages
  const orphaned = nonPartials.filter(
    (d) => (!d.sb || d.sb.length === 0) && !allLinkedIds.has(d.id) && !allImportedIds.has(d.id),
  );

  // Content quality: very short docs in sidebar
  const shortInSidebar = nonPartials.filter(
    (d) => d.w < 100 && d.sb && d.sb.length > 0,
  );
...
async function main() {
  ...
  const reports: [string, string][] = [
    ['01-frontmatter-audit.md', generateFrontmatterAudit(manifest.docs)],
    ['02-navigation-audit.md', generateNavigationAudit(manifest.docs)],
    ['03-content-metrics.md', generateContentMetrics(manifest.docs)],
    ['04-section-summaries.md', generateSectionSummaries(manifest.docs, manifest.stats)],
    ['05-actionable-priorities.md', generateActionablePriorities(manifest.docs)],
  ];
