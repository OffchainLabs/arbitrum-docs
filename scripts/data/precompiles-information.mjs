/**
 * Hand-curated overrides for the generated precompile tables.
 *
 * The generator derives signatures, source line numbers, and descriptions from the
 * Solidity interface and Go implementation. These overrides supply what the source does
 * not carry: deprecation flags, the ArbOS version a method appeared in, and descriptions
...
export const precompilesInformation = {
  ...
  ArbAggregator: {
    methodOverrides: {
      getpreferredaggregator: { deprecated: true },
      getdefaultaggregator: { deprecated: true },
      gettxbasefee: { deprecated: true },
      settxbasefee: { deprecated: true },
    },
  },
  ArbDebug: {
    eventOverrides: {
      basic: { description: 'Emitted in <code>Events</code> for testing' },
      mixed: { description: 'Emitted in <code>Events</code> for testing' },
      store: { description: 'Never emitted (used for testing log sizes)' },
    },
  },
  ArbFunctionTable: {},
  ArbGasInfo: {
    methodOverrides: {
      ...
      GetL1PricingEquilibrationUnits: { availableSinceArbOS: 20 },
      GetLastL1PricingUpdateTime: { availableSinceArbOS: 20 },
      GetL1PricingFundsDueForRewards: { availableSinceArbOS: 20 },
      GetL1PricingUnitsSinceUpdate: { availableSinceArbOS: 20 },
      GetLastL1PricingSurplus: { availableSinceArbOS: 20 },
    },
  },
  ArbInfo: {},
  ArbNativeTokenManager: {
    methodOverrides: {
      mintNativeToken: { availableSinceArbOS: 41 },
      burnNativeToken: { availableSinceArbOS: 41 },
    },
    eventOverrides: {
      nativetokenminted: {
  ...
  ArbOwnerPublic: {
    methodOverrides: {
      RectifyChainOwner: { availableSinceArbOS: 11 },
    },
    eventOverrides: {
      chainownerrectified: { description: 'Emitted when verifying a chain owner' },
    },
  },
  ArbosTest: {},
  ArbRetryableTx: {
    ...
    eventOverrides: {
      ticketcreated: { description: 'Emitted when creating a retryable' },
      lifetimeextended: { description: "Emitted when extending a retryable's expiry date" },
      redeemscheduled: { description: 'Emitted when scheduling a retryable' },
      canceled: { description: 'Emitted when cancelling a retryable' },
      redeemed: {
        description:
  ...
  ArbWasm: {
    eventOverrides: {
      programactivated: { description: 'Emitted when activating a WASM program' },
  ...
  ArbWasmCache: {
    eventOverrides: {
      updateprogramcache: { description: 'Emitted when caching a WASM program' },
    },
  },
};
...
export const nodeInterfaceInformation = {
  methodOverrides: {
    ...
    blockl1num: { description: 'Returns the L1 block number of the L2 block' },
    l2blockrangeforl1: {
