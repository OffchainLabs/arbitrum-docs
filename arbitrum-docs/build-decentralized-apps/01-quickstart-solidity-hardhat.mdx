...
### The Solidity version of the vending machine

Here is a Solidity implementation of our vending machine.
...
Putting a smart contract in production is called _deploying a contract_. When we deploy a contract, we send a deployment transaction to a node. The transaction has our contract's code as payload. Once that transaction is confirmed, our contract is available on the chain it's been deployed to, on a unique contract address. 
A contract's address is the equivalent of a server's IP address.

Let's start by deploying our contract to a virtual chain, that will be a way to start without taking any risk, or losing tokens.

We'll be using remix to do that.
...
We'll use Remix to deploy our smart contract:

##### Set Remix up

###### Load Remix: https://remix.ethereum.org
