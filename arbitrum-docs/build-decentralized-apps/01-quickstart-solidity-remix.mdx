...
Ensure that Remix's compiler version matches the contract's. You can find your contract's compiler version at the top of your contract's file range. It looks like this:
...
Note that in the context of this quickstart, "account" refers to an EOA (externally owned account) public wallet address and its associated private key[^5].
...
Once you've acquired some `ETH`, you'll be able to deploy your smart contract to Arbitrum's Sepolia testnet by issuing the following cast command:
...
```shell
cast send --rpc-url https://sepolia-rollup.arbitrum.io/rpc --private-key YOUR_PRIVATE_KEY --create --constructor-args --value 0 --gas-limit 2000000 $(cat VendingMachine.sol-abi)
```
