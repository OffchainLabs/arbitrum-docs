...
Even though these contracts are used to create Orbit chains, it is recommended to go through the [Orbit SDK](/launch-orbit-chain/orbit-sdk-introduction.md) to interact with them. Doing so prevents misconfiguring certain parameters and using appropriate defaults for most of them.
...
- **End-to-end deployment and initialization**: the canonical contracts will create all needed contracts, initialize them and configure them with the parameters passed. Additionally, they will send the appropriate initialization messages to the chain's Inbox contract so the sequencer can process them upon starting. This whole process is done atomically in a single transaction, guaranteeing a successful chain or token bridge creation.
- **Gas efficiency**: the canonical contracts have been designed to be gas efficient. Creating and initializing all contracts for a chain or token bridge is a gas-consuming process, so special care has been taken to be able to fit everything into one transaction.
- **Updated with the latest features**: whenever a new version is available for the contracts of an Orbit chain, the canonical factory contracts are updated so new rollups and token bridges are created using the updated version.
