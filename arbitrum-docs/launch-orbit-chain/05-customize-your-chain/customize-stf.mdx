...
The instructions provided in [How to run a full node](/run-arbitrum-node/03-run-full-node.mdx) **will not** work with your Orbit node. See [Optional parameters (Orbit)](/run-arbitrum-node/03-run-full-node.mdx#optional-parameters) for Orbit-specific CLI flags.
