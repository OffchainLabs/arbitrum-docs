...
| **AnyTrust**         | Implements the [AnyTrust protocol](/how-arbitrum-works/inside-arbitrum-nitro.md#inside-anytrust), relying on an external [Data Availability Committee (DAC)](/glossary.mdx#data-availability-committee-dac) to store data and provide it on-demand instead of using their [parent chain](/glossary.mdx#parent-chain) as the Data Availability (DA) layer. | Suitable for applications that require lower transaction fees.                        |
