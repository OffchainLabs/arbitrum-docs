...
Custom gas token support for Rollups is currently limited to L2s with BoLD enabled. An expected future upgrade will add support for L3s.

:::

When deploying an Orbit chain in [Rollup mode](/how-arbitrum-works/12-data-availability.mdx#rollup-mode), you can use a custom `ERC-20` token as the native gas token. This token is usable for transaction fees on that specific Orbit chain and reimbursing the batch poster for data posted to the parent chain. An example would be an L2 Rollup that uses `USDC` as its custom gas token but pays `ETH` to post data to L1 Ethereum.
