...
You should use `EthBridger` when bridging the native token between the parent chain and the Orbit chain.
...
- You should use `Erc20Bridger` when bridging an `ERC-20` token between the parent chain and the orbit chain.
- These APIs are just needed for `custom gas token` Orbit chains and for `ETH`-powered rollup and Anytrust Orbit chains, you don't need to use them.
