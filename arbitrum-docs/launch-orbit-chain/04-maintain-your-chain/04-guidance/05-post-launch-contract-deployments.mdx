...
This method leverages Arbitrum's built-in cross-chain messaging infrastructure. Specifically, it uses the `Inbox.sendL2Message` function to submit a pre-signed child chain transaction from the parent chain. Unlike retryable tickets, messages sent this way do not incur parent chain data fees during execution;they're paid upfront when the message is submitted on the parent chain. This also means they're not limited by the 100,000 gas cap that applies to certain deterministic deployment flows.
...
Make sure the deployer address is sufficiently funded on the child chain before sending the transaction. If the transaction runs out of gas or fails for any reason, the nonce will be burned and the deployment will need to be retried with a new nonce and payload.
