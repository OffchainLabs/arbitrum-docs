...
Before updating the contracts, you want to make sure your nodes are ready for the update. Nitro v3.5.1 introduced compatibility with pre-BoLD and BoLD chains to ensure a smooth upgrade. Nodes will automatically detect whether the chain is running pre-BoLD or BoLD Rollup and Challenge contracts and will perform the appropriate calls depending on that check.
...
This section explains how to upgrade of your chain contracts to v3.0.0. Note that this process expects that your chain uses the canonical contracts. If your chain uses customized contracts, you might need a different updating script/contract than the one available.
...
To effectively upgrade your Nitro contracts using the provided upgrade action, your contracts must be in one of these versions:
...
The action contract that performs the upgrade will need to be parametrized with your chain information. To set this configuration, create a new file, `scripts/files/configs/custom.ts`, with the following contents:
...
Adjust the configuration to match that of your chain. **Please pay close attention** to every parameter since it will override the existing configuration of your chain. Note the following:
...
- Note that in the first `contracts` property, you need to specify the contract addresses, while the next `proxyAdmins` property expects the addresses of the ProxyAdmin of those contracts (they all generally use the same ProxyAdmin contract).
...
- Adjust `maxDataSize` to match that of your chain (you can call `Inbox.maxDataSize()` to get the value needed)
