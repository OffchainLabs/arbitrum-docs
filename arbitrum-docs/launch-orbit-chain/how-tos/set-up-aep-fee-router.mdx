...
<aside>
  ⚠️ The standard script deploys and sets up the AEP fee router contracts for L2 chains. For L3
  chains (or further layers), a different target address on the parent chain will need to be
  specified, depending on the native token of the chain. Please contact the Arbitrum Foundation to
  confirm the target address to withdraw the AEP fees to.
</aside>
