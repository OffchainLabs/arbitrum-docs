...
1. Clone, install dependencies, set .env
```shell
...
2. Periodically trigger withdrawals to the parent chain

::::info

If the `ChildToParentRouter` is routing its chain's native token, this step can be skipped.
...
3. Periodically redeem withdrawals on the parent chain
