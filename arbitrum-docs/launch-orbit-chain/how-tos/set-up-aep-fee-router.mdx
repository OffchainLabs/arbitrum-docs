...
2. Trigger withdrawals to the parent chain

::::info

If the `ChildToParentRouter` is configured to send the native token, this step can be skipped.

::::

Run this command periodically to trigger withdrawals from the router to its parent chain.

```shell
TODO, we don't have a script for this yet
```

3. Redeem withdrawals on the parent chain

Run this command periodically to redeem withdrawals from a `ChildToParentRouter`.
