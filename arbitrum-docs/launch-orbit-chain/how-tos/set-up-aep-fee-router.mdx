...
```shell
cast send $C2P_ROUTER_ADDRESS "routeToken()" --rpc-url $CHILD_CHAIN_RPC --private_key $CHILD_CHAIN_PK
```
...
yarn redeem-child-to-parent \
  --parentRPCUrl $PARENT_CHAIN_RPC \
  --childRPCUrl $CHILD_CHAIN_RPC \
  --childToParentRewardRouterAddr $C2P_ROUTER_ADDRESS \
  --oneOff
```
