...
While any Orbit chain can adopt Fast Withdrawals, we only _recommend_ Fast Withdrawals be adopted by AnyTrust chains with a minimum validator and DAC member requirement. We explain both these recommendations below:

### Fast Withdrawals for AnyTrust Chains
As AnyTrust chains are an optimium (an optimistic rollup using a separate data availability layer), AnyTrust chains are already placing a trust assumption on their Data Availability Committee (DAC) to provide the data needed for fraud proofs and recreating the chain.

The optimal setup for an AnyTrust chain is to have all DAC members _also_ run validators as part of the Fast Withdrawals committee. This will leverages the existing trust assumption placed on the DAC operators such that **enabling Fast Withdrawals does not add any new trusted parties.**
...
We recommend that any Fast Withdrawals enabled chain have a minimum of 3 DAS members and 3 validators in the Fast Withdrawals committee. Given that Fast Withdrawals will enable confirmation of new rollup state much faster than the usual 7 day challenge period (as fast as 15 minutes), it becomes even more important to have more parties involved in validation to further reduce trust assumptions.

## Technical lower bound for Fast Withdrawals
Once Fast Withdrawals is enabled, the committee will confirm transactions at the configured frequency. However, higher network load can cause the Fast Withdrawals committee to experience slight delays from the configured rate.   
...
Chain owners and operators should be aware that the Fast Withdrawals committee may take longer to confirm new assertions under conditions with greater network load. This behaviour is to be expected and does not interfere with the security or trust model of the Fast Withdrawals committee.

## Practical lower bounds with respect to parent chain finality
...
For an Ethereum-based Layer-2, we recommend that the Fast Withdrawals frequency remain above 12.8 minutes, which is the the time for Ethereum to achieve finality. For non-Ethereum L1s, we similarly recommend staying above the accepted finality threshold specific to that L1. 
...
Upgrading to the new nitro-contracts version also requires updating the node software. For v2.1.0, validator nodes and the batch poster node should be running [nitro v3.1.2](https://github.com/OffchainLabs/nitro/releases/tag/v3.1.2) or above.
