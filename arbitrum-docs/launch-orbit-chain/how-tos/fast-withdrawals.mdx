...
- The optimal setup for an AnyTrust chain is to have all DAC members _also_ run validators as part of the Fast Withdrawals committee, which leverages the existing trust assumption on the DAC. **This means enabling Fast Withdrawals does not add any new trusted parties.**
...
3. Execute the action. Upgrade can be executed using the `cast` CLI command (part of Foundry installation), using the owner account (the one with executor rights on parent chain UpgradeExecutor) to send the transaction:
