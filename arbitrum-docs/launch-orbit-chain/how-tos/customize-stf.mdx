...
Once you have the WASM module root, you can put it on-chain by calling `setWasmModuleRoot(newWasmModuleRoot)` through the upgradeExecutor contract as the owner, to call this method, you need to set `target` as your rollup contract address, set `targetCallData` start with `0x89384960` (this is the signature of `setWasmModuleRoot(byte32)`), and followed by your wasm module root.
