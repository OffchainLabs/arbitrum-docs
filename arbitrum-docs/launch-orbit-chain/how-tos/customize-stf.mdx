...
This is the recommended way if you are running your orbit chain locally through the provided docker-compose file. In `docker-compose.yml`, modify the docker image used for the nitro container:
...
And run `docker compose up` to run all your containers.
...
Once you have set the new WASM module root on-chain, you may then run the new node image. Following a similar process than before, we will first remove the parameter `--node.staker.dangerous.without-block-validator` to enable fraud proofs back.

To do that, open your `nodeConfig.json` file again, and remove the `"dangerous"` section you created before.

After that, you'll have, again, two ways of running your node.

**1. Using the docker-compose file**

As mentioned before, this is the recommended way if you are running your orbit chain locally through the provided docker-compose file. In `docker-compose.yml`, modify the docker image used for the nitro container. Notice that we will now use the `custom-nitro-node-dev` you just created.
