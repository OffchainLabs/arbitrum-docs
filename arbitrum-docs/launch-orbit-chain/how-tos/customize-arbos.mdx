...
In this example, we use precompiles as example, some logic might also affect stf such as the methods in [block_process.go], [internal_tx.go], [tx_processor.go] and so on.
