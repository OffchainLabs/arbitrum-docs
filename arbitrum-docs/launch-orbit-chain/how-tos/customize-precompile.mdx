...
4.  Set gas usage needed for a specific method
...
## Option 4: Customize gas usage needed for a specific method

In the above, we introduced how to define a new precompile function. However, if it is simply defined without performing gas collection within the function, it will lead to potential DOS attacks. For this reason, we will continue to introduce how to perform gas collection in precompile. (event does not need to customize the gas cost, because the program will calculate the gas cost of the event when it starts).

At the same time, you can also customize a calculation library with lower gas usage than conventional solidity chain contracts based on this document, so that users can use it as a third-party library call with low gas usage.

In procompile, we perform gas deduction by calling the `Burn` method of parameter` ctx `in the precompile function, such as the `GetBalance` method in ArbInfo.go:
...
```
The purpose of this method is to obtain the balance of an address. At the same time, according to the definition in [EIP1884](https://eips.ethereum.org/EIPS/eip-1884), the operation code gas usage of obtaining the address balance is 700 gas, so before returning Before the result, the function consumed 700 gas by calling `c.Burn(int64)` (the return value of `params.BalanceGasEIP1884` is 700).

Next, we will try to define a function that consumes gas based on the caller address balance as an example. In this step, we will not charge gas according to the specifications in EIP1884, but reduce the gas usage by 400 to 300 gas.
...
How to register a new precompile method please see option1.
...
First call:
```
cast send 0x0000000000000000000000000000000000000064 "GetBalance()({Any_Address})"
```
Secondly call:
```
cast send 0x0000000000000000000000000000000000000064 "GetBalanceCustom()({Any_Address})"
```

Afterwards we will obtain the following two results respectively.
...
```
We can get that the gas used for execution of tx1 is `gasUsed - gasUsedForL1 = 22938`. Similarly, the gas used for tx2 is `22538` (for the gas model, see [how to estimate gas](/devs-how-tos/how-to -estimate-gas)), subtract the two, and the result is `400`, which is exactly what we expected.
