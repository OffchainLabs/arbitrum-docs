...
(⚠️⚠️⚠️ The tutorial only works when you use eth_call to call the new precompiles, if you want to add non-view/pure methods or call from other contracts, it will break validation, but we will have `how to update the wasm module root tutorial` to fix this soon.)
