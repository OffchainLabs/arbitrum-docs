...
Note that the instructions provided in [How to run a full node](/run-arbitrum-node/03-run-full-node.md) **will not** work with your Orbit node. See [Optional parameters (Orbit)](/launch-orbit-chain/how-tos/running-an-orbit-node.mdx#optional-parameters) for Orbit-specific CLI flags.
