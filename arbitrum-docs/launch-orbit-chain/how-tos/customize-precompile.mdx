...
Here we will still use the precompile `Arbsys` as an example add a simple `Hi` event to be emitted in `SayHi` which we added in Option 1, first go to the [precompiles implementation][precompile_impl_dir_link] directory, and find `ArbSys.go`:
...
Navigate to the [precompiles interface][precompile_interface_dir_link] directory, open `Arbsys.sol`, and add the required interface. Ensure that the Event name on the interface matches the name of the function you introduced in the previous step:
...
Now as emiting events need to cost gas, so we should remove `view` function behavior:
...
### Send the tx and get tx receipt
...
```
From the `logs` field, we can see the topics[0] `0xa937..cfa`, which is the event encode of `Hi(address)`, you can check it using [4 byte directory](https://www.4byte.directory/event-signatures/?bytes_signature=0xa9378d5bd800fae4d5b8d4c6712b2b64e8ecc86fdc831cb51944000fc7c8ecfa), and topics[1] is `Your_address`.
