...
First you need to open the [arbosstate.go][arbosstate_file_link] file, then find the [ArbosState][arbosstate_link] structure in this file. This is where the state stored in arbos is defined.
...
After that we need to back to [precompiles directory][precompile_impl_dir_link] and create a new `ArbHi.go` as we introduced in Option 2.
But this time we will add two more new methods to read and write the arbos state:
