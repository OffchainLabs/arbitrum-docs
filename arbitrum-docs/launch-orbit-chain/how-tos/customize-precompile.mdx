...
Since in the evm chain, all states need to be stored under a certain address, in this example, we will introduce how to use arbos to store your state in a precompile contract.

First you need to open the [arbosstate.go][precompile_impl_dir_link] file, then find the [ArbosState][arbosstate_link] structure in this file. This is where the state stored in arbos is defined.
...
You also need to define some methods in this file to read and write this state `myNumber`:
...
But this time we will add two new methods to read and write the state in precompile:
...
After that, follow the Option 2 guide and add this new precompile contract then run your node.
