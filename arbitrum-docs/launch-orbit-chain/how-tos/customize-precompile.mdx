...
Since in the evm chain, all states need to be stored under a certain address, in this example, we will introduce how to use arbos to store your state.

First you need to find the [arbosstate.go][precompile_impl_dir_link] file, find the [ArbosState][arbosstate_link] structure in this file. This is where the state stored in arbos is defined.

We can define a state called myNumber here, myNumber uses the `storage.StorageBackedUint64` type. You can find more related types in [storage.go][storage_link] :
...
Next you need to define the offset of your newly added state (note please add it to the end so it won't affect other states):
...
Then, initialize the state under method [OpenArbosState][arbosstate_openstate] and [InitializeArbosState][arbosstate_initializeArbosState]:
...
We also need to define some methods in this file to read and write this state (myNumber):
...
After that we can back to [precompiles implementation][precompile_impl_dir_link] and create a new ArbHi.go as we introduced in Option 2.
But this time we need to add two new methods to read and write the state we defined above:
...
After that, follow the Option 2 guide and add this new precompile contract and run your node.

Your smart contract interface should be like this:
...
[arbosstate_file_link]: https://github.com/OffchainLabs/@nitroRepositorySlug@/blob/@nitroVersionTag@/arbos/arbosState/arbosstate.go
[arbosstate_link]: https://github.com/OffchainLabs/nitro/blob/v2.2.0/arbos/arbosState/arbosstate.go#L38
[storage_link]: https://github.com/OffchainLabs/@nitroRepositorySlug@/blob/@nitroVersionTag@/arbos/storage/storage.go
