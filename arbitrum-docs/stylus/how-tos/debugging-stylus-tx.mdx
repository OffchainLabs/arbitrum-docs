...
If on Linux or Windows, install GDB if it's not already installed:

```sh
sudo apt-get install gdb
```

If on MacOS, you should have LLDB installed already. If you don't you can install it with:
...
GDB / LLDB will load and set a breakpoint automatically at the `user_entrypoint` internal Stylus function.
...
5. **Debugging with GDB / LLDB**: Now, set a breakpoint at the `increment()` method:
