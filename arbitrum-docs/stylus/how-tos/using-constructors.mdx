...
Constructors allow you to initialize your Stylus smart contracts with specific parameters when they're deployed. This guide will show you how to implement constructors in Rust, understand their behavior, and deploy contracts using them.
...
- Understand constructor rules and limitations
...
Instructions on how to set up a local Arbitrum test node can be found [here](https://github.com/OffchainLabs/nitro-devnode).

</CustomDetails>

## Understanding Stylus constructors

Stylus constructors provide an atomic way to deploy, activate, and initialize a contract in a single transaction. Without a constructor, your contract's storage might be accessed before your initialization logic runs, which can lead to unexpected behavior.

<VanillaAdmonition type="info" title="Constructors and composition">
  Stylus uses trait-based composition instead of traditional inheritance. When building contracts that 
  compose multiple traits, constructors help initialize all components properly. See the 
  [Constructor with trait-based composition](#constructor-with-trait-based-composition) section for examples.
</VanillaAdmonition>
...
<VanillaAdmonition type="info" title="Trait-based composition in Stylus">
  ...
  initialized explicitly in the constructor. For more details on trait-based composition, see the 
  [Composition and trait-based routing model](/stylus/how-tos/using-inheritance) guide.
</VanillaAdmonition>
...
Constructor arguments are automatically encoded by `cargo stylus`. The arguments should be provided in the same order as defined in your constructor function.
...
The complete example project demonstrating these concepts is available at `~/tmp/stylus-constructor-example-code` and includes comprehensive tests and deployment instructions.
