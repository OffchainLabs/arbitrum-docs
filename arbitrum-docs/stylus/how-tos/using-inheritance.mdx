...
<VanillaAdmonition type="caution" title="No explicit override keywords">
  Stylus does not currently contain explicit override or virtual keywords for marking
  override functions. It is important to carefully ensure that contracts are only overriding the
  functions you intend to override.
</VanillaAdmonition>
...
1. Stylus doesn't support Solidity-style multiple inheritance yet
2. There's a risk of undetected selector collisions with functions from implemented traits
3. No explicit override or virtual keywords for clearly marking overridden functions

### Best practices

1. Be explicit with function selectors using `#[selector(name = "FunctionName")]` when implementing standard interfaces
2. Use `cargo expand` to examine the expanded code and verify trait-based composition is working as expected
3. Be cautious with method overriding since there are no explicit override keywords
4. Design your contracts with composition in mind
5. Test thoroughly to ensure all implemented methods work correctly
6. Always use `self.vm()` methods for accessing blockchain context (instead of deprecated functions)
7. When overriding methods that emit events, make sure to explicitly re-emit the events
8. Use feature flags to control which contract is the entrypoint when working with multiple contracts

## Debugging issues

If you encounter issues with trait-based composition in your Stylus contracts, try these approaches:

1. Verify the `#[implements]` annotation includes the correct traits
2. Ensure all function selectors are correctly specified when implementing standard interfaces
3. Check for method name conflicts between traits and contract implementations
...
6. Test individual methods to isolate issues
