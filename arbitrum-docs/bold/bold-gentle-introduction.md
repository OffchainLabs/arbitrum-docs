...
- Validation for Arbitrum One and Nova is currently is limited to a [permissioned set of parties maintained by the Arbitrum DAO](https://docs.arbitrum.foundation/state-of-progressive-decentralization#allowlisted-validators) to reduce the risks of *[delay attacks](https://medium.com/offchainlabs/solutions-to-delay-attacks-on-rollups-434f9d05a07a)* - a class of attacks where malicious entities can indefinitely open disputes and delay confirmations of assertions about a rollup's state.
- BOLD, an acronym for Bounded Liquidity Delay, is a new challenge protocol for Arbitrum chains that mitigate the risks of delay attacks against [rollups like Arbitrum](../inside-arbitrum-nitro/inside-arbitrum-nitro.mdx#arbitrum-rollup-protocol). This is possible because BOLD's design ensures disputes will be resolved within a fixed time window, currently set to equal 1 challenge period (~6.4 days) for Arbitrum One and Nova. Since the risks of delay attacks are mitigated with BOLD, a permissioned validator set is no longer needed - paving the way for permissionless validation of Arbitrum chains. 
...
BOLD, an acronym for Bounded Liquidity Delay Protocol, is a set of rules used by validators to open and resolve disputes about Arbitrum’s state to ensure only valid states get confirmed on an Arbitrum chain’s parent chain, such as Ethereum, for example. BOLD is shipped as part of the Nitro tech stack - the software that powers every Arbitrum validator node. 

 BOLD is an upgrade to Arbitrum's existing challenge protocol - which has working fraud proofs and is used in production today by Arbitrum chains. The new changes BOLD brings enhances the security around withdrawals to L1 by enabling anyone to participate in the validation of the state of the chain.

In short, BOLD further strengthens the secure withdrawal assets from Arbitrum back to Ethereum.
...
Under the hood, the reason why BOLD can offer time-bound, permissionless validation is because a correct Arbitrum state assertion is **not tied to a single staker or entity**. This property, coupled with the fact that L2 states are completely deterministic and can eventually be proven on Ethereum, means that **any party** can stake on this correct, deterministic state and, through interactive fraud proofs, can prove that their claim is correct and will always win.
...
2. **A challenge is opened:** When another validator observes and disagrees with the end state represented by `Block Z`, they can permissionlessly open a challenge by asserting and staking a claim on a different end state, represented by an RBlock `Block Y`. At this point in time, there are now 2 asserted states: `Block A → Block Z` and `Block A → Block Y`. Each of these asserted states, at this point in time now that there's a challenge, are referred to *edges* while a Merkle tree of asserted states from some start to end point (e.g. `Block A → Block Z`) is more formally known as a *history commitment.* It is important to note that Ethereum at this point in time has no notion of which edge(s) is correct or incorrect - edges are simply a portion of a claim made by a validator about the history of the chain from some end state all the way back to some initial state.
...
* Checking out the [BOLD testnet status page](https://status-bold.vercel.app/) where you can observe the state of the testnet, including on-going challenges and various statistics.
