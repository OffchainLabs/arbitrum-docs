...
5. **Confirmation:** Once the honest one-step edge is confirmed, the protocol will work on confirming or rejecting the parent edges until it reaches the level-zero edge of the honest party. With the honest party’s level-zero edge now confirmed, the honest party’s assertion bond gets automatically refunded. Meanwhile, the dishonest party has their bonds taken away to ensure the dishonest party is always punished. Reimbursements for the honest party's L1 gas costs and mini-bonds made at the other challenge levels are handled by the Arbitrum DAO.
...
* Checking out this [BOLD Technical Deep Dive](https://docs.google.com/document/d/1uJAFTJeL2gaCh0YGyjsVidqySENNIY_RFj3kDWG3Pi0/edit?usp=sharing) to learn about BOLD's implementation, alongside [the BOLD sourcecode on Github](https://github.com/OffchainLabs/bold) to understand how BOLD works under the hood.
