...
- **Challenge manager client:** Software that can manage the life cycles of challenges that an active validator participates in. Validators need to be able to participate in multiple challenges at once and manage individual challenge vertices correctly to act upon, confirm, or reject them.
...
- **OneStepProver:** A set of existing contracts that implement a miniature `WASM` VM capable of executing one-step-proofs of computation of the child chain state transition function. This is implemented in Solidity and exists on Ethereum. No changes to the `OSP` contracts are needed for BoLD.
...
- **Validating bridge:** The smart contract that leverages Ethereum's security and censorship-resistance to unlock bridged assets from the child chain back to the parent. Assets can be unlocked after an assertion (showing that those assets have been bridged from the child chain) has been posted and confirmed.
...
![900px-img](../assets/transaction-lifecycle.svg)
