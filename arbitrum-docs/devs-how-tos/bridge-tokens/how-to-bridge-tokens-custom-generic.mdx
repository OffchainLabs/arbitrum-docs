---
title: "How to bridge tokens via Arbitrum’s custom generic gateway"
sidebar_label: Custom generic gateway
description: Learn how to use the custom generic gateway to bridge tokens programmatically
...
As stated in the [token bridge conceptual page](/asset-bridging.mdx#the-arbitrum-generic-custom-gateway), there are a few prerequisites to keep in mind while using this method to make a token bridgeable.
...
These methods are needed to register the token via the gateway contract. If your L1 contract does not include these methods and it is not upgradeable, registration could alternatively be performed as a chain-owner registration via an [Arbitrum DAO](https://forum.arbitrum.foundation/) proposal. Keep in mind that this registration can only be done once.
...
We’ll also add the required methods to make our token bridgeable via the custom generic gateway.
...
## Step 2: Create a token and deploy on L2
...
## Step 3: Register the custom token to the custom generic gateway

Once both our contracts are deployed in their respective chains, it’s time to register the token in the custom generic gateway.
...
Once this step is done, your L1 and L2 tokens will be connected through the custom generic gateway.
...
### Can I set up the custom generic gateway after a standard ERC20 token exists on L2?

Yes, if your token has a standard ERC20 counterpart on L2, you can go through the process of registering your custom L2 token as outlined in this page. At that moment, your L1 token will have 2 counterpart tokens on L2, but only your new custom L2 token will be minted when depositing tokens from L1 (L1-to-L2 bridging). Both L2 tokens will be withdrawable (L2-to-L1 bridging), so users holding the old standard ERC20 token will be able to withdraw back to L1 and then deposit to L2 to get the new custom L2 tokens.
