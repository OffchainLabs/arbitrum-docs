---
title: "How to bridge tokens via Arbitrum’s standard ERC20 gateway"
...
In this how-to you’ll learn how to bridge your own token between Ethereum (Layer 1 or L1) and Arbitrum (Layer 2 or L2), using [Arbitrum’s standard ERC20 gateway](/asset-bridging.mdx#default-standard-bridging). For alternative ways of bridging tokens, don’t forget to check out this [overview for token bridging](/devs-how-tos/bridge-tokens/how-to-bridge-tokens-overview.mdx).
...
## Step 1: Create a token and deploy on L1
...
As stated in the [token bridge conceptual page](/asset-bridging.mdx#default-standard-bridging), when using Arbitrum’s standard ERC20 gateway, you don’t need to do any pre-configuration process. Your token will be “bridgeable” out of the box. 
...
We’ll explain below what specific contracts and methods need to be called to bridge your token, but you can abstract this whole process of finding the right addresses by using Arbitrum’s SDK. You can use the [deposit](/sdk/classes/assetBridger_erc20Bridger.Erc20Bridger#deposit) function of the [ERC20Bridger](/sdk/classes/assetBridger_erc20Bridger.Erc20Bridger) class to bridge your tokens, which will use the appropriate router contract based on the network you’re connected to, and will relay the request to the appropriate gateway contract. You can also use the function [getL1GatewayAddress](/sdk/classes/assetBridger_erc20Bridger.Erc20Bridger#getl1gatewayaddress) to get the address of the gateway contract that’s going to be used. But don’t worry about any of this yet, we’ll use those functions in the next steps.
...
We typically do that by using the `approve` method of the token, but you can use Arbitrum’s SDK to abstract this process, by calling the method [approveToken](/sdk/classes/assetBridger_erc20Bridger.Erc20Bridger#approvetoken) of the  [ERC20Bridger](/sdk/classes/assetBridger_erc20Bridger.Erc20Bridger) class, which will call the approve method of the token passed by parameter, and set the allowance to the appropriate gateway contract.
...
You can use Arbitrum’s SDK to abstract this process, by calling the method [deposit](/sdk/classes/assetBridger_erc20Bridger.Erc20Bridger#deposit) of the  [ERC20Bridger](/sdk/classes/assetBridger_erc20Bridger.Erc20Bridger) class, which will estimate the gas parameters (_maxGas, _gasPriceBid and maxSubmissionCost, explained below) and call the `outboundTransferCustomRefund` method of the router contract. You will only need to specify the following parameters:
...
## Step 5: Wait for execution in L2

After calling the deposit method (or the outboundTransferCustomRefund, if you’re going the manual way), you’ll have to wait a bit until the message is executed in L2. We will verify the status of the underlying retryable ticket created to bridge the tokens. Check this page, to know more about [L1-to-L2 messages, also known as retryables](/arbos/l1-to-l2-messaging.mdx).

You can programmatically wait for the execution of the transaction in L2 using Arbitrum’s SDK. You should first wait for the execution of the submission transaction (the one sent to the router contract) and then the execution of the L2 transaction.
...
If you’re going the manual way, you can verify if the message has been executed in L2 through the [Retryables Dashboard](https://retryable-dashboard.arbitrum.io/). Just paste the hash of transaction submitted to the router contract and the tool will tell you whether it’s been redeemed or not.

## Step 6: Check the new token contract created in L2

Finally, let’s find the token contract that has been created in L2. 

Using Arbitrum’s SDK, you can call method [getL2ERC20Address](/sdk/classes/assetBridger_erc20Bridger.Erc20Bridger#getl2erc20address) of the  [ERC20Bridger](/sdk/classes/assetBridger_erc20Bridger.Erc20Bridger) class, which will return the address of the token contract in L2 which corresponds to the L1 token contract send as parameter.
