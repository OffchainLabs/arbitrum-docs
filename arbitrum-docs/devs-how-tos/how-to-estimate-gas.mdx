...
- L1P (L1 estimated price per byte of data) ⇒ Estimated cost of posting 1 byte of data on L1:
  - Call `NodeInterface.gasEstimateL1Component()`, get the third element `l1BaseFeeEstimate` and multiply it by 16.
- L1S (Size of data to be posted on L1, in bytes) ⇒ This will depend on the data of the transaction. Keep in mind that Arbitrum adds a fixed amount to this number to make up for the static part of the transaction, which is also posted on L1 (140 bytes). We can do a small calculation to obtain this value: call `NodeInterface.gasEstimateL1Component()` take the first element, `gasEstimateForL1` (this is equivalent to `B` in our formula), multiply it by P and divide it by L1P.
...
const totalEstimated = gasEstimateComponents.gasEstimate;
const l1GasEstimated = gasEstimateL1Component.gasEstimateForL1;
const l2GasUsed = totalEstimated.sub(l1GasEstimated);
const l2EstimatedPrice = gasEstimateComponents.baseFee;
const l1EstimatedPrice = gasEstimateL1Component.l1BaseFeeEstimate.mul(16);
