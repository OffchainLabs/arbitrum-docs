...
- `block.difficulty` and `block.prevrandao` return the constant 2500000000000000
- `block.number` returns an "estimate" of the L1 block number at which the Sequencer received the transaction (see [Block Numbers and Time](./time.mdx))
