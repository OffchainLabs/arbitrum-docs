...
| **Money**             | Devs and users pay centralized service providers for server access using fiat currency. | <- same, but only for the presentation-layer concerns (code that supports frontend UI/UX). | <- same, but devs and users pay **testnet $ETH** to node-runners to deploy smart contracts and execute transactions. | <- same, but instead of testnet $ETH, they pay using **mainnet $ETH**.                    |
