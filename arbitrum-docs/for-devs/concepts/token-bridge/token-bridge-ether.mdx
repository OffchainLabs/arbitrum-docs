...
import DepositEthDiagram from '@site/../arbitrum-docs/diagrams/_deposit-eth.mdx';
import WithdrawEthDiagram from '@site/../arbitrum-docs/diagrams/_withdraw-eth.mdx';

Ether (ETH) is the native currency of Ethereum, Arbitrum One and Arbitrum Nova. It is used to pay the necessary fees to execute a transaction in those chains. Bridging ETH from Ethereum (Layer 1, or L1) to an Arbitrum chain (Layer 2, or L2) follows, thus, a different process than the one followed when bridging other types of asset.
