...
**Arbitrum One** is the flagship Arbitrum mainnet chain, an optimistic rollup chain running on top of the Ethereum mainnet. It offers a unique combination of benefits including trustless security, compatibility with Ethereum, scalability, and minimum transaction costs. It uses the [Nitro](/inside-arbitrum-nitro/inside-arbitrum-nitro.mdx) technology stack, which is a major upgrade of the technology, improving over the "Classic" stack in several ways such as advanced calldata compression, separate contexts for common execution and fault proving, Ethereum L1 gas compatibility, and more.

### Arbitrum Nova

**Arbitrum Nova** is another production chain that uses the AnyTrust technology stack, a variant use case of the Nitro technology. In general, AnyTrust lowers transaction costs by accepting a mild trust assumption. Instead of requiring all Arbitrum nodes to have access to the data of every L2 transaction in the Arbitrum chain's inbox, AnyTrust relies on an external data availability committee to store data and provide it on demand, reducing the costs of batching and posting all L2 transaction data to L1.
