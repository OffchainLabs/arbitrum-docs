...
Similarly, Arbitrum chains can also send messages to the parent chain, although these follow more strict rules. Find more information about them in [L2 to L1 messaging and the outbox](/arbos/l2-to-l1-messaging.mdx).
...
Arbitrum Nitro software includes a special `NodeInterface` contract available at address `0xc8` that only exists in the RPC (it's not deployed on-chain). Find more information about this interface in [NodeInterface.sol](/arbos/gas.mdx#nodeinterfacesol).
