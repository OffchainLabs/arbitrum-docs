...
This overview will be focused on the Arbitrum Rollup protocol; see [Inside AnyTrust](./inside-anytrust.mdx) for differences in the Arbitrum AnyTrust protocol. Also, for convenience/simplicity, we'll be describing the security properies of the core system itself; see ["State of Progressive Decentralization"](TODO_STATE_OF_DECENTRALIZATION_URL) for current decentralization status.
