...
An Arbitrum **archive node** is a full node that maintains an archive of historical chain states. This how-to walks you through the process of configuring an archive node on your local machine so that you can query both pre-Nitro and post-Nitro state data. Note that most users won't need to configure an archive node. This type of node is great for a small number of use-cases - for example, if you need to process historical data.

### Before we begin

Before the Nitro upgrade happened, Arbitrum One was running on the Classic stack for about one year (before block height 2207817). Although the Nitro chain uses the latest snapshot of the Classic chain's state as its genesis state (see [state migration](../migration/state-migration.mdx)), the Nitro stack can't serve archive requests for pre-Nitro blocks. **Running an Arbitrum One full node in archive mode lets you do this**, but it requires you to run both Nitro and Classic nodes together. You may not need to do this, depending on your use case:

- If you want to run an archive node for Nitro Goerli Rollup Testnet or Arbitrum Nova, see [How to run a full node](./running-a-node.md).
- If you want to access the Arbitrum network but you don’t want to run your own node, see [Node Providers](./node-providers.mdx). This contains a list of third-party node providers and RPC addresses pointing to the fully-managed nodes.
- If you only need to send **post-Nitro** archive requests, see [How to run a full node](./running-a-node.md).
- If you only need to send **pre-Nitro** archive requests, see [How to run a classic node](./running-a-classic-node.mdx).
- If you need both **post-Nitro** _and_ **pre-Nitro** archive requests, you should run **both** an archive Nitro node _and_ an archive Classic node. That's what this how-to is for.
...
4. Latest Docker image for **Arbitrum One**: `offchainlabs/nitro-node:v2.0.13-174496c`
5. Latest Docker image for **Arbitrum Classic**: `offchainlabs/arb-node:v1.4.5-e97c1a4`
6. Arbitrum One Nitro genesis database snapshot
   ...
   - Size: 33.5573504 GB
7. Arbitrum One Classic database snapshot
