...
_Special note that Arbitrum Nitro node software is incompatible with Erigon's Consensus Layer client, [Caplin](https://erigon.tech/), due to a formatting mismatch for a specific response type (i.e. `SecondsPerSlot` is used in Caplin instead of `SECONDS_PER_SLOT`). Therefore, if any of the providers above use Caplin, errors will occur.
