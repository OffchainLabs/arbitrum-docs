...
5. **Database snapshots:**
   - Nitro database snapshot
     - Use the parameter `--init.url=` on first startup to initialize the Nitro database (you can find a list of snapshots [here](#snapshots-available)). Example: <code>--init.url="@arbOneNitroArchiveSnapshot@"</code> 
   - Arbitrum One Classic database snapshot
     - Download the latest Arbitrum One Classic database snapshot at [@arbOneClassicArchiveSnapshot@](@arbOneClassicArchiveSnapshot@) and put it to the mounted point directory.
