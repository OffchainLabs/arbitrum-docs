...
| `--l2.chain-id=<L2 Chain ID>`               | See [public chains](/for-devs/concepts/public-chains.mdx) for a list of Arbitrum chains and the respective L2 chain IDs                          |
...
Here's a list of the database snapshots available to use with the `--init.url` parameter when first initializing your archive node:

- Arbitrum One:
  - Latest Nitro archive snapshot
    - URL: [@arbOneNitroArchiveSnapshot@](@arbOneNitroArchiveSnapshot@)
    - Size: 
    - SHA5 checksum: 
  ...
  - Classic archive snapshot (do not use with `--init.url`; download and place it in the mounted directory instead)
    - URL: [@arbOneClassicArchiveSnapshot@](@arbOneClassicArchiveSnapshot@)
    - Size:
    - SHA5 checksum:

- Arbitrum Nova:
  - Latest archive snapshot
    - URL: [@arbNovaArchiveSnapshot@](@arbNovaArchiveSnapshot@)
    - Size: 
    - SHA5 checksum: 

- Arbitrum Goerli:
  - Latest archive snapshot
    - URL: [@arbGoerliArchiveSnapshot@](@arbGoerliArchiveSnapshot@)
    - Size: 
    - SHA5 checksum: 
