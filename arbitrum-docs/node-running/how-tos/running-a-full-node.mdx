...
## Watchtower mode
- By default, the full node will work in Watchtower mode. This means that the node watches the on-chain assertions and if it disagrees with them, it will log an error containing the string `found incorrect assertion in watchtower mode`.
- You can deactivate this mode by using the parameter `--node.staker.enable=false`.
