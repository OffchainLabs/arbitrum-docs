...
- L2 chain id or name
  - Use the parameter `--chain.id=<L2 chain ID>` to set the L2 chain from its chain id. See [RPC endpoints and providers](/build-decentralized-apps/reference/node-providers#rpc-endpoints) for a list of Arbitrum chains and their respective L2 chain ids.
