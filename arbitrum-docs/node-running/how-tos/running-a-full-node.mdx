...
- Parent chain RPC URL
  ...
  - It must provide a standard evm node RPC endpoint that you run yourself or from a node provider
  - Would be set to appropriate Arb1, Nova, Sepolia RPC URL
  - Note: [Arbitrum.io public RPCs](/node-running/node-providers.mdx) will rate limit connections, will need to run local node for parent chain or use 3rd party RPC providers
...
- For Orbit chains
  - `nodeConfig.json` will be generated when orbit chain is created, you need to set it using either `--conf.file` or `--chain.info-json` 
  - `--conf.file` is the docker container's path of `nodeConfig.json`, you need put this file to mount point first. `--chain.info-json` is the json content itself
  - `--chain.name="My Arbitrum L3 Chain"` must be provided, and matches the chain name used in `--conf.file` file or `--chain.info-json` content
  - If anytrust chain, also need to supply `--node.data-availablity.enable` and then some ways to get the data like `--node.data-availability.rest-aggregator.enable` and associated options or `--node.data-availability.ipfs-storage.enable` and associated options
