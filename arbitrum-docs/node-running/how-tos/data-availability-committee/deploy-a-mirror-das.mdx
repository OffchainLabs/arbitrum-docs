---
title: 'How to configure a Data Availability Committee: deploy a mirror Data Availability Server (DAS)'
...
The main use case for running a mirror DAS is to complement your setup as a Data Availability Committee (DAC) member. That means that you should run your main DAS first, and then setup the mirror DAS. When following this guide, you should be familiarized with how a regular DAS work. You can find that information in [How to deploy a DAS](./deploy-a-das.mdx).
...
In this how-to you'll learn how to deploy a mirror DAS and enable a REST interface to respond to requests of stored information. For more information related to configuring a DAC, please see the [Introduction](./introduction.mdx).

You should be familiarized with how the AnyTrust protocol works and what's the role of the DAC in the protocol. You can find more information about the AnyTrust protocol in [Inside AnyTrust](/inside-anytrust.mdx). You should also be familiarized with how a regular DAS works and what configuration options are available. You can find that information in [How to deploy a DAS](./deploy-a-das.mdx). Finally, it is also recommended to be familiarized with [Kubernetes](https://kubernetes.io/) as the examples on this guide are based on that software.
...
We now start the process of deploying the mirror DAS. Remember that you can reach out to us on Discord if you are having trouble following this process.
...
Archive DA servers are servers that don't discard any data after expiring. Each DAC should have at the very least one archive DAS to make sure all historical data is available.
