...
- Here is an example of how to run a node for an Orbit chain:
  ...
  - Note for Anytrust chains, it is essential to add `--node.data-availability.enable` to the command and add `--node.data-availability.rest-aggregator.urls=<Your DAS rest endpoints>`/`--node.data-availability.rest-aggregator.urls=<A url can return a list of your das rest endpoints>` as well.
