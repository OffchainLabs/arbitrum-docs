...
It must provide a standard RPC endpoint for an EVM node, whether self-hosted or obtained from a node service provider should be set to appropriate Arbitrum One, Arbitrum Nova, or Arbitrum Sepolia RPC URL.
    
:::note

[Arbitrum RPC endpoints](/node-running/node-providers.mdx/#rpc-endpoints) rate limit connections, So you will need to run a local node for the parent chain or rely on third-party RPC providers

:::

#### L3 Orbit chain info and chain name

Providing the following flag is mandatory:
  
```bash
  --chain.info-json=<Orbit Chain's chain info>
```
    
The chain info json string contains vital information about the Orbit chain, which is necessary for the node to commence operations. An example of the chain info json string is available in the subsequent sections.
...
And it must match the chain name that is used in `--chain.info-json` content.


For an Anytrust chain, you will need to provide 
...
Here is an example of how to run a node for an Orbit chain, Note that is important that `/some/local/dir/arbitrum` already exists, otherwise the directory might be created with `root` as owner, and the docker container won't be able to write to it:
...
When using the flag `--chain.info-json=<Orbit Chain's chain info>`, it is essential to replace `Orbit Chain's chain info` with the specific `chain info` json string of the Orbit chain for which you wish to run the node. For your convenience, an example of an chain info json string is provided below:
...
For Anytrust chains, it is essential to add the following to the command or configuration: 
...
When shutting down the Docker image, it is important to allow for a graceful shutdown so that the current state can be saved to disk. Here is an example of how to do a graceful shutdown of all Docker images currently running:
