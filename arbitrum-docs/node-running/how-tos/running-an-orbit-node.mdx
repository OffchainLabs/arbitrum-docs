...
This guide provides a step-by-step process for deploying and operating an Orbit node on your local machine. To initiate an Orbit node, it's essential to first deploy the core contracts of the Orbit chain. Additionally, you must supply `"chain info:"` of the orbit chain, which includes crucial configurations required for the Orbit chain's operation. An example of the "chain info" is available in this section. For more information about orbit chain deployment please check our [Launch Your Own Chain](../../launch-orbit-chain/orbit-gentle-introduction.md) part of our docs.
...
- Parent chain RPC URL
  ...
  - Note: [Arbitrum.io public RPCs](/node-running/node-providers.mdx) RPCs rate limit connections, So you will need to run a local node for the parent chain or rely on third party RPC providers
- L3 orbit chain id and name
  - Providing the flag --chain.info-json="Orbit Chain's chain info" is mandatory. The chain info contains vital information about the Orbit chain, which is necessary for the node to commence operations. An example of the chain info is available in the subsequent sections.
...
- Here is an example of how to run orbit-node:
  ...
  ```
  - Please note that when using the flag `--chain.info-json="Orbit Chain's chain info"`, it is essential to replace "Orbit Chain's chain info" with the specific `chain info` file of the Orbit chain for which you wish to run the node. For your convenience, an example of an chain info file is provided below:
