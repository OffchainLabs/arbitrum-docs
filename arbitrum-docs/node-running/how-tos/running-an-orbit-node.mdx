...
This guide provides a step-by-step process for running an Orbit node on your local machine. To initiate an Orbit node, you must supply `"chain info"` json string of the orbit chain, which includes crucial configurations required for the Orbit chain's operation. `"chain info"` json string should be provided by the orbit chain owner, and the node runner can consume it to run the node for that specific orbit chain. An example of the "chain info" json string is available in this section. 
...
- Parent chain RPC URL
  ...
  - Note: [Arbitrum RPC endpoints](/node-running/node-providers.mdx/#rpc-endpoints) RPCs rate limit connections, So you will need to run a local node for the parent chain or rely on third party RPC providers
