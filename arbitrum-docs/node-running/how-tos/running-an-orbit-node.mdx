...
- Here is an example of how to run orbit-node:
  ...
  ```shell
  docker run --rm -it  -v /some/local/dir/arbitrum:/home/user/.arbitrum -p 0.0.0.0:8547:8547 -p 0.0.0.0:8548:8548 @latestNitroNodeImage@ --chain.info-json --parent-chain.connection.url <Parent chain RPC URL> --chain.id=<L3ChainId> --chain.name="My Arbitrum L3 Chain" --http.api=net,web3,eth,debug --http.corsdomain=* --http.addr=0.0.0.0 --http.vhosts=*
  ```
...
### Example of `nodeConfig.json`

Here is an example of node config file generated after an orbit chain deployment.
