...
This guide provides a step-by-step process for running an Orbit node on your local machine. To initiate an Orbit node, you must supply `"chain info"` of the orbit chain, which includes crucial configurations required for the Orbit chain's operation. `"chain info"` should be provided by the orbit chain owner, and the node runner can consume it to run the node for that specific orbit chain. An example of the "chain info" is available in this section. 
