...
- L3 orbit chain id and name
  - Providing the flag --chain.info-json="Orbit Chain's info-json" is mandatory. The info-json contains vital information about the Orbit chain, which is necessary for the node to commence operations. An example of the info-json is available in the subsequent sections.
...
- Here is an example of how to run orbit-node:
  ...
  ```
  - Please note that when using the flag `--chain.info-json="Orbit Chain's info-json"`, it is essential to replace "Orbit Chain's info-json" with the specific `info-json` file of the Orbit chain for which you wish to run the node. For your convenience, an example of an info-json file is provided below:
