...
This document details the mechanisms for proving correct execution and resolving disputes in the Rollup Protocol. It explains how the system separates ordinary execution from proving, transforms <a data-quicklook-from="wasm">WASM</a> into a speicalized proving format (WAVM), and employs interactive fraud proofs to resolve disputes efficiently.

## 1. Separating execution from proving

<ImageZoom
  src="/img/haw-<a data-quicklook-from="geth">Geth</a>-sandwich.svg"
  alt="<a data-quicklook-from="transaction">transaction</a> lifecycle diagram showing various pathways for submitting transactions"
  className="img-600px"
/>
...
- **Modifications made in WAVM**:
	- **Removal of unused features**:
		- WAVM removes some WASM features that the Go compiler never generates. The transfomration phase ensures these features are absent.
...
- **Step-by-step breakdown**:
	...
	- This interative bisection continues until the dispute reaches a single instruction.
...
- **Drawbacks**:
	...
	- In a dispute, the entire transaction muts be re-executed onchain, leading to high gas costs.
...
- **Branch instructions (`br` and `br_if`):
	- Translated into `ArbitaryJump` and `ArbitaryJumpIf` with jump destinations known at compile time.
...
- **Floating-point instructions**:
	...
	- Cross-module calls go to the flatoing-point library.
...
<a data-quicklook-from="stylus">Stylus</a> introduces an execution path alonside the EVM, enabling <a data-quicklook-from="smart-contract">smart Contract</a> execution in WebAssembly (WASM). Given that Stylus program are fraud-proven directly rather than interpreted via Geth's EVM implementation––the proving mechanism required modifications to the standard Arbitrum proving architecture.
