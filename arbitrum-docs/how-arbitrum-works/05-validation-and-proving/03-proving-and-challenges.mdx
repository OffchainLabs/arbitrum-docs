...
This document details the mechanisms for proving correct execution and resolving disputes in the Rollup Protocol. It explains how the system separates ordinary execution from proving, transforms WASM into a speicalized proving format (WAVM), and employs interactive fraud proofs to resolve disputes efficiently.

## 1. Separating execution from proving

<ImageZoom
  src="/img/haw-geth-sandwich.svg"
...
2. **Bisection of the dispute**:
	- **First round**:
		- Alice posts an assertion covering `N` steps.
...
1. A challenge initiates if a validator disputes the execution of a Stylus transaction.
