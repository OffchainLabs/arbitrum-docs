...
- **Modifications made in WAVM**:
  ...
  - **Restrictions on certain features**:
    - Floating-point instructions: WAVM does not contain floating-point instructions. Instead, any floating-point operations get replaced by calls to the Berkeley SoftFloat library.
      - Rationale: Using software floating-point libraries reduces the risk of incompatibilities between different architectures.
  - **Nested control flow**:
...
An interesting instruction in WAVM is `ReadPreImage`, utilized by the "hash oracle trick". This mechanism works as follows:
...
Dispute resolution in Optimistic Rollups is a critical design decision. The protocol must decide which execution version is correct when two parties disagree.
...
- **Dispute Resolution Options**:
  - **Interactive proving**:
    - Parties engage in a challenge game to narrow down the disputed execution steps.
...
#### Why does bisection correctly identify a cheater
...
Interactive fraud proofs are fundamental for resolving disputes among proposers when conflicting claims arise.
...
- **Example setup**:
  ...
  - Alice is bonded on assertion 93, while Bob's bond is on assertion 95.
...
1. **Bisection phase**:
   ...
   - The parent chain referee only checks that the moves have "the right shape" (e.g., that the bisection occurred correctly) without evaluating the correctness of execution.
2. **One-step proof phase**:
   ...
   - If the one-step proof is correct, the disputed edge is confirmed, and the challenges resolve.
...
- **Gas limits**:
  - The interactive proving model can work with higher per-transaction gas limits since most work gets completed offchain.
- **Implementation flexibility**:
  - The system only requires verification of a one-step proof onchain, rather than full transaction re-execution.
...
## 6. One-step proof of assumptions
...
- **Safety in honest challenges**:
  ...
  - If a dishonest party attempts to force an unreachable case, it will be preceded by an invalid reachable case, ensuring that the fraudulent party eventually loses the challenge.

### 6.3 Additional safety assumptions

- **WAVM code validity**:
  - The WAVM code produced by the arbitrator comes from a valid WASM compile. It gets checked using `wasm-validate` from the WebAssembly Binary Toolkit (WABT).
- **Inbox message size**:
...
- **Added opcodes**:
  - New opcode additions for interacting with the blockchain (e.g., reading global state, fetching inbox messages, signaling completion).
...
- **If and Else**:
  - Translated into a block with an `ArbitraryJumpIf` instruction.
    - **Structure example**:
...
- **Branch table (`br_table`)**:
  - Translated to a series of conditional checks for each possible branch, with a default branch when no conditional matches occur.
- **`Local.tee`**:
...
WAVM includes several unique opcodes that simplify proving. They break down into several categories:
...
Floating-point operations in WAVM get handled differently to ensure determinism and consistency:

- **Implementation using Berkeley SoftFloat**:
  - Instead of implementing IEEE 754-2019 directly in WAVM, floating-point instructions are replaced with calls to the Berkeley SoftFloat-3e library.
  - The soft float library is linked, and floating-point operations get implemented via cross-module calls.
- **Known divergences**:
...
- **Purpose**:
  - Each stack frame retains information about the caller module and its internal offset.
- **Implementation**:
  ...
  - Only libraries can access their callers' memory; restrictions prevent the main module from doing so.
...
<a data-quicklook-from="stylus">Stylus</a> introduces an execution path alongside the EVM, enabling <a data-quicklook-from="smart-contract">
  smart Contract
</a> execution in WebAssembly (WASM). Given that Stylus programs are fraud-proven directly rather than
...
Unlike EVM-based contracts, Stylus programs are dynamically linked into the replay machine when proving fraud. This linking is possible with two key opcodes:
...
These opcodes ensure that Stylus contracts are treated as part of the replay machine, allowing for precise, fraud-proof bisection at the WASM level.

### 10.2 Execution-proving separation

Stylus leverages Arbitrum's separation of execution and proving, where the replay machine runs in WAVM. Disputes over Stylus transactions follow the standard interactive fraud-proof protocol (<a data-quicklook-from="bold">BoLD</a>), which means:
