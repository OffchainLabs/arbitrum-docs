...
Optimistic Rollups work by separating execution from proving to ensure efficient transaction processing without immediate validation. To learn more about the execution process, you can review the [State Transition Function](/how-arbitrum-works/04-state-transition-function/01-stf-gentle-intro.mdx). This section focuses on validation and proving mechanisms. We will cover how child chain validators confirm the chain's state and how assertions are posted to the parent chain for finalization, with the potential for future challenges if an <a data-quicklook-from="assertion">assertion</a> is determined to be incorrect.
...
This dual compilation approach ensures that Arbitrum nodes execute transactions efficiently while allowing for deterministic, trustless verification on the parent chain.
...
If another validator disputes the assertion, an interactive fraud-proof gets initiated.

#### Confirmed

It becomes final if no one challenges the assertion within the dispute window (6.4 days).

This model ensures that as long as at least one honest validator participates, the correct execution will always get confirmed.
...
On the left, representing an earlier part of the chain's history, we have confirmed assertions. These have been fully accepted and recorded by the parent chain contracts that manage the chain. The newest of the confirmed assertions, assertion 94, is referred to as the "latest confirmed assertion."

On the right, we see a set of newer proposed assertions. The protocol can't yet confirm or reject them because their deadlines haven't yet expired. The oldest assertion whose fate remains to be determined, assertion 95, is referred to as the "first unresolved assertion."
...
Here's another example of what the chain state might look like if several validators are being malicious. It's a contrived example designed to illustrate a variety of cases that can arise in the protocol, all condensed into a single scenario.
...
- Assertion 101 claimed to be a correct successor to assertion 100, but 101 gets rejected.
- Assertion 102 eventually receives <a data-quicklook-from="confirmation">confirmation</a> as the correct successor to 100.
- Assertion 103 gets confirmed and is now the latest confirmed assertion.
- Assertion 104 gets proposed as a successor to assertion 103, and 105 gets proposed as a successor to 104. 104 rejects as incorrect; consequently, 105 also rejects because its predecessor rejected.
...
- Assertions 107 and 108 claim to chain from 106. They are also unresolved. If 106 rejects, they will also get rejected.
- Assertion 109 disagrees with assertion 106 because they both claim the same predecessor. One of them will eventually get rejected, but the protocol hasn't yet resolved it.
- Assertion 110 claims to follow 109. It is unresolved. If 109 gets rejected, 110 will also get rejected.
- Assertion 111 claims to follow 104. 111 will inevitably get rejected because its predecessor was rejected. However, it remains active because the protocol resolves assertions in assertion number order; therefore, it must resolve assertions 106 through 110 before it can resolve assertion 111. After 110 resolves, the protocol will immediately reject 111.

Again, this is very unlikely in practice. In this diagram, at least four different bonds are incorrect assertions, and when the dust settles, at least four bonds will be forfeited. The protocol handles these cases correctly, but they're rare corner cases. This diagram illustrates the possible situations and how the protocol would address them.

## Delays

Even if the Assertion Tree contains multiple conflicting assertions and multiple disputes are in progress, validators can continue to make new assertions. Honest validators will build on one valid assertion (intuitively, an assertion is also an implicit claim of the validity of all of its parent assertions). Likewise, users can continue to transact on the child chain, as transactions will continue to get posted in the chain's inbox.

The only delay users experience during a dispute is their [Child-to-parent chain messages](/how-arbitrum-works/11-l2-to-l1-messaging.mdx) (i.e., withdrawals). A key property of BoLD is that we can guarantee that, in the common case, their withdrawals or messages will only get delayed for one challenge period. In the case of an actual dispute, the withdrawals/messages will be delayed by no more than two challenge periods, regardless of the adversaries' behavior during the challenge.

## Who will be the validators?
...
- Parties with significant assets in a bond on a chain, such as <a data-quicklook-from="dapp">dApp</a> developers, exchanges, power users, and liquidity providers, may choose to validate to protect their investment.
...
- **Child-to-parent chain withdrawals** may experience delays. Typically, withdrawals experience a single challenge period (6.4 days) delay. - A key property of BoLD is that we can guarantee that, in the common case, withdrawals/messages will only experience a delay of one challenge period. In the case of an actual dispute, the withdrawals/messages will be delayed by no more than two challenge periods, regardless of the adversaries' behavior during the challenge.

Despite these delays, Arbitrum guarantees that honest assertions always succeed, maintaining the same level of security as Ethereum.
