...
For more details on how the Rollup Chain works under BoLD, the [Gentle introduction](/how-arbitrum-works/gentle-introduction.mdx) provides an overview that touches on the Rollup Chain.
...
Every validator can choose their own approach, but we expect validators to follow three common strategies (For more details on these strategies see [Run a validator](/run-an-arbitrum-node/more-types/run-a-validator.mdx)):
