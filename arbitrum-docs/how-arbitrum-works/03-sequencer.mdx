...
- [Batch-Posting](#batch-posting): How the Sequencer groups transactions into batches, compresses them to reduce data size and sends them to the Sequencer Inbox Contract on the parent chain. This section also delves into the L1 pricing model and how it affects transaction costs.
...
The force inclusion window is actually the lesser of either the `delayBuffer` and `delayBlocks`, where delayBlocks is a constant, currently set to 24 hours, and `delayBuffer` grows and shrinks between 30 minutes <= `delayBuffer` <= 48 hours.
