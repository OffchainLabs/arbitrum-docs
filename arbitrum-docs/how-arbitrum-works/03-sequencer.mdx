...
- **Batch-Posting**: How the Sequencer groups transactions into batches, compresses them to reduce data size and sends them to the Sequencer Inbox Contract on the parent chain. This section also delves into the L1 pricing model and how it affects transaction costs.
    - **Batching**
    - **Compression**
    - **Submitting to Sequencer Inbox Contract**
    - **L1 Pricing Model**
...
- **L1 Pricing Model**: An overview of the pricing challenges associated with posting data to parent chain and the adaptive algorithms used to determine fair costs per transaction.
...
## Submitting transactions to the Sequencer Inbox
...
**Note**: The Sequencer continuously monitors network conditions to choose the most economical method for batch posting, ensuring optimal operation under varying circumstances.
