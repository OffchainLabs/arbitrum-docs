...
Below is ApplyTransaction's callgraph, with additional info on where the various Arbitrum-specific hooks are. Click on any to go to their section. By default, these hooks do nothing to leave Geth's default behavior unchanged, but for chains configured with [`EnableArbOS`](https://docs.arbitrum.io/how-arbitrum-works/state-transition-function/modified-geth-on-arbitrum#enablearbos) set to true, [`ReadyEVMForL2`](https://docs.arbitrum.io/how-arbitrum-works/state-transition-function/modified-geth-on-arbitrum#readyevmforl2) installs the alternative child chain hooks.

- `core.ApplyTransaction` -> `core.applyTransaction` -> `core.ApplyMessage`
  - `core.NewStateTransition`
    - `ReadyEVMForL2`
  - `core.TransitionDb`
    - [`StartTxHook`](https://docs.arbitrum.io/how-arbitrum-works/state-transition-function/modified-geth-on-arbitrum#starttxhook)
    - `core.transitionDbImpl`
      - if `IsArbitrum()` remove tip
      - [`GasChargingHook`](https://docs.arbitrum.io/how-arbitrum-works/state-transition-function/modified-geth-on-arbitrum#gascharginghook)
      - `evm.Call`
        - `core.vm.EVMInterpreter.Run`
          - [`PushCaller`](https://docs.arbitrum.io/how-arbitrum-works/state-transition-function/modified-geth-on-arbitrum#pushcaller)
          - `PopCaller`
        - `core.StateTransition.refundGas`
          - [`ForceRefundGas`](https://docs.arbitrum.io/how-arbitrum-works/state-transition-function/modified-geth-on-arbitrum#forcerefundgas)
          - [`NonrefundableGas`](https://docs.arbitrum.io/how-arbitrum-works/state-transition-function/modified-geth-on-arbitrum#nonrefundablegas)
        - [EndTxHook](https://docs.arbitrum.io/how-arbitrum-works/state-transition-function/modified-geth-on-arbitrum#endtxhook)
