...
During execution, the EVM uses transient memory (a word-addressed byte array) that only persists for the duration of a transaction. In contrast, each contract maintains a persistent Merkle Patricia _storage_ trie–a word-addressable word array–that forms part of the global state.
...
engine. It processes transactions by executing the smart contract's bytecode and updating the global state, ensuring that every state change is deterministic and secure.
...
Gas prices and base fees on Arbitrum change differently from those on Ethereum. Arbitrum maintains two base fees: one to track the cost of execution on the child chain and another to estimate the cost of posting batches to the parent chain. The mechanism that changes the child chain's base fee differs from what is currently on Ethereum.
...
Specific precompiled contracts on the Arbitrum chain will process features specific to Arbitrum. Moreover, individual Arbitrum chains may implement their custom precompiles.
...
### Custom hooks and interface adaptations

Nitro integrates a series of custom hooks within Geth's transaction processing flow. These hooks enable Nitro to implement dual gas accounting, manage parent chain calldata fees, and handle custom transaction types — such as retryable transactions — without deviating from Ethereum's core logic.
...
Detailed technical explanations of these modifications are in the following sections, where we delve deeper into how Nitro's custom hooks, interface implementations, and Geth adaptations work together to support an advanced, high-performance STF.
...
It facilitates communication between parent and child chains, supporting functionalities such as `ETH` and token deposits and withdrawals.
...
By offloading tasks that would incur high costs on the parent chain, ArbOS enables these operations to be performed quickly and cost-effectively on the child chain. This design reduces computational and storage costs, offering significant flexibility that allows the child chain code to evolve or be customized more easily than a parent chain-enforced architecture.

While Ethereum's STF forms the basis for secure and deterministic state updates, Arbitrum's Nitro stack builds on this foundation with key modifications, including dual gas accounting and cross-chain messaging, to optimize performance and flexibility. These innovations are realized through minimal yet strategic modifications to Geth, integrated seamlessly with ArbOS, forming the "Geth sandwich."
