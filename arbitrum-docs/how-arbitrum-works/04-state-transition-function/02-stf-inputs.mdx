...
<a data-quicklook-from="arbitrum">Arbitrum</a> nodes receive <a data-quicklook-from="transaction">
  transaction</a> inputs that get processed through a dual-channel approach that enhances reliability and consistency.
...
- `L2MessageType_unsignedEOATx` (0): Child chain messages submitted by externally owned accounts (EOAs) to the parent chain that do not include a signature.

- `L2MessageType_unsignedContractTx` (1): Child chain messages submitted on the parent chain that do not include a signature.
