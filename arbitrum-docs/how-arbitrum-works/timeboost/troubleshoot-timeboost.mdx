...
Note that if you get any of the errors below or errors related to invalidity of the transaction (eg nonce too low, malformed transaction), then the sequence number used in your express lane transaction was _not_ consumed.
| Error | Description |
| ------------------------------ | --------------------------------------------------------------------- |
| `MALFORMED_DATA` | wrong input data, failed to deserialize, missing certain fields, etc. |
| `WRONG_CHAIN_ID` | wrong chain id for the target chain |
