...
Nitro makes minimal modifications to Geth in hopes of not violating its assumptions. This section will explore the relationship between Geth and ArbOS, which consists of a series of hooks, interface implmentations, and strategic re-appropriations of Geth's basic types.

We store ArbOS's state at an address inside a Geth `statedb`. In doing so, ArbOS inherits the `statedb`'s statefulness and lifetime properties. For example, a transaction's direct state changes to ArbOS would get discarded upon a revert.
...
Below is `ApplyTransaction`'s callgraph, with additional info on where the various Arbitrum-specific hooks are. Click on any to go to their section. By default, these hooks do nothing to leave Geth's default behavior unchanged, but for chains configured with [`EnableArbOS`](/how-arbitrum-works/04-geth-at-the-core.mdx#enablearbos) set to true, [`ReadyEVMForL2`](/how-arbitrum-works/04-geth-at-the-core.mdx#readyevmforl2) installs the alternative child chain hooks.

- `core.ApplyTransaction` -> `core.applyTransaction` -> `core.ApplyMessage`
   - `core.NewStateTransition`
      - `ReadyEVMForL2`
   - `core.TransitionDb`
      - [`StartTxHook`](/how-arbitrum-works/04-geth-at-the-core.mdx#starttxhook)
      - `core.transitionDbImpl`
         - if `IsArbitrum()` remove tip
         - [`GasChargingHook`](/how-arbitrum-works/04-geth-at-the-core.mdx#gascharginghook)
         - `evm.Call`
             - `core.vm.EVMInterpreter.Run`
               - [`PushCaller`](/how-arbitrum-works/04-geth-at-the-core.mdx#pushcaller)
               - `PopCaller`
         - `core.StateTransition.refundGas`
             - [`ForceRefundGas`](/how-arbitrum-works/04-geth-at-the-core.mdx#forcerefundgas)
             - [`NonrefundableGas`](/how-arbitrum-works/04-geth-at-the-core.mdx#nonrefundablegas)
        - [`EndTxHook`](/how-arbitrum-works/04-geth-at-the-core.mdx#endtxhook)
...
`APIBackend` implements the `ethapi.Backend` interface, which allows simple integration of the Arbitrum chain to the existing Geth API. The `Backend` member answers most calls.
...
| ------| ------| ------| ------|
| [`ArbitrumUnsignedTx`](/how-arbitrum-works/04-geth-at-the-core.mdx#arbitrumunsignedtx) | A parent chain to child chain message | [`EndTxHook`](/how-arbitrum-works/04-geth-at-the-core.mdx#endtxhook) | Bridge |
| [`ArbitrumContractTx`](/how-arbitrum-works/04-geth-at-the-core.mdx#arbitrumcontracttx) | A nonce-less parent chain to child chain message | [`EndTxHook`](/how-arbitrum-works/04-geth-at-the-core.mdx#endtxhook) | Bridge|
| [`ArbitrumDepositTx`](/how-arbitrum-works/04-geth-at-the-core.mdx#arbitrumdeposittx) | A user deposit | [`StartTxHook`](/how-arbitrum-works/04-geth-at-the-core.mdx#starttxhook) | Bridge|
| [`ArbitrumSubmitRetryableTx`](/how-arbitrum-works/04-geth-at-the-core.mdx#arbitrumsubmitretryabletx) | Creating a retryable | [`StartTxHook`](/how-arbitrum-works/04-geth-at-the-core.mdx#starttxhook) | Bridge|
| [`ArbitrumRetryTx`](/how-arbitrum-works/04-geth-at-the-core.mdx#arbitrumretrytx) | A retryable redeem attempt | [`EndTxHook`](/how-arbitrum-works/04-geth-at-the-core.mdx#endtxhook) | Child chain|
| [`ArbitrumInternalTx`](/how-arbitrum-works/04-geth-at-the-core.mdx#arbitruminternaltx) | ArbOS state update | [`StartTxHook`](/how-arbitrum-works/04-geth-at-the-core.mdx#starttxhook) | ArbOS|
...
It represents a retryable submission and may schedule an [`ArbitrumRetryTx`](/how-arbitrum-works/04-geth-at-the-core.mdx#arbitrumretrytx) if enough gas is available. Fore more info, please see the [retryables documentation](/how-arbitrum-works/10-l1-to-l2-messaging.mdx#retryable-tickets).

### [`ArbitrumRetryTx`]((/how-arbitrum-works/04-geth-at-the-core.mdx#arbitrumretrytx))
...
### `EnableArbos`
Introduces [ArbOS](/how-arbitrum-works/04-geth-at-the-core.mdx#arbos), converting what would otherwise be a vanilla parent chain into a child chain Arbitrum rollup.
