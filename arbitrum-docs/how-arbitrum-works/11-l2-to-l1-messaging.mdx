...
Arbitrum's outbox system allows for arbitrary child chain to parent chain contract calls, i.e., messages initiated from the child chain, which eventually resolve in execution on the parent chain. Child-to-parent chain messages (aka "outgoing" messages) bear some things in common with Arbitrum's [parent chain to child chain messages](/how-arbitrum-works/11-l1-to-l2-messaging.mdx), which are "in reverse" though with differences, which we'll explore in this section.
