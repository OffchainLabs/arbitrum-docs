...
The child chain component consists of the traditional fees <a data-quicklook-from="geth">Geth</a> would pay to stakers in a vanilla parent chain, such as the computation and storage charges applying the <a data-quicklook-from="state-transition-function">State Transition Function</a> entails. <a data-quicklook-from="arbos">ArbOS</a> charges additional fees for executing its child chain-specific [precompiles](/build-decentralized-apps/precompiles/01-overview.mdx), whose fees are dynamically priced according to the specific resources used while executing the call.
