...
On calls that return custom Arbitrum type transactions, such as [`eth_getTransactionByHash`](https://ethereum.org/en/developers/docs/apis/json-rpc/#eth_gettransactionbyhash), the `type` field will reflect the custom codes where applicable. 
...
- `104` - `ArbitrumRetryTxType`: Used to [redeem a retryable ticket](arbos/l1-to-l2-messaging#redeeming-a-retryable) on L2, which finalizes a L1 to L2 message that failed to redeem automatically (usually due to low gas)
...
- `106` - `ArbitrumInternalTxType`: Internal transactions created by the ArbOS itself for certain internal state updates, like L1 base fee and block number. 
...
- `gasUsedForL1`: Amount of gas spent on l1 computation in units of l2 gas.
