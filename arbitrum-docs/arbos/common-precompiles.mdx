...
| [![](../assets/e.png)][ss1] [`arbBlockHash`][s1]`()`                         | Gets the L2 block hash, if the block is sufficiently recent |
| [![](../assets/e.png)][ss5] [`isTopLevelCall`][s5]`()`                       | Checks if the call is top-level                             |
| [![](../assets/e.png)][ss9] [`sendTxToL1`][s9]`(destination, calldataForL1)` | Sends a transaction to L1, adding it to the outbox          |
| [![](../assets/e.png)][ss11] [`withdrawEth`][s11]`(destination)`             | Send paid eth to the destination on L1                      |
