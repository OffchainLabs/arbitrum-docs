...
The L1 gas basefee can be queried via [`ArbGasInfo.getL1BaseFeeEstimate`](precompiles.mdx). To estimate the L1 fee a transaction will use, the [`NodeInterface.gasEstimateComponents`](gas.mdx) or [`NodeInterface.gasEstimateL1Component`](gas.mdx) method can be used.
