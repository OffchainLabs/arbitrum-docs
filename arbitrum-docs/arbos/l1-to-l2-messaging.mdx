...
3. **Elaborating on our conceptual primitives** by exploring how each of the conceptual primitives relate to one another in the context of cross-layer messaging.
4. **Learning with an example smart contract** that exposes two functions. We'll compare a number of properties across deployment scenarios to test our understanding of the conceptual primitives.
...
This document assumes that you're familiar with the following terms:
...
The rest of this document focuses on "constructing an understanding" of the following **conceptual primitives**:
 ...
 3. **Outbox contracts**: Smart contracts that are deployed to L1 chains and are responsible for transmitting retryable tickets to L2's inbox contracts. The Arbitrum DAO owns these.
 4. **Inbox contracts**: Smart contracts that are deployed to L2 chains and are responsible for receiving retryable tickets from L1's outbox contracts. The Arbitrum DAO owns these.
 5. **Gateway contracts**: Smart contracts that are deployed to both L1 and L2 and are responsible for managing the flow of information between the two layers. The Arbitrum DAO owns these, but you can create your own.


:::caution Editor note

 - Low-confidence about the claims in 3/4/5
...
## 3: Elaborating on our conceptual primitives
...
Let's compare a number of properties across **two deployment scenarios** in order to 
