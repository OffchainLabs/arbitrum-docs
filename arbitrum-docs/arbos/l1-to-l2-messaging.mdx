...
<MermaidWithHtml>
  <Nodes title="Ticket Submission">
    ...
    <Node id="3">Check user's deposit</Node>
  ...
  <NodeDescriptions>
    ...
    <NodeDescription for='3'><code>Check user's deposit</code> Logic that checks if the user have enough funds to create a ticket. This is done by checking if the `msg.value` provided by the user is greater than or equal to `maxSubmissionCost + l2CallValue + gasLimit * maxFeePerGas`</NodeDescription>
