...
import RetryablesLifeCycleDiagram from './diagrams/\_retryables_lifecycle';
...
- If the ticket is sucessfully auto-redeemed, it will execute with the sender, destination, callvalue, and calldata of the original submission. The submission fee is refunded to the user at L2 (`excessFeeRefundAddress`). Note that to ensure successful auto-redeem of the ticket, you could use the Arbitrum SDK which provides a [function][https://github.com/OffchainLabs/arbitrum-sdk/blob/4cedb1fcf1c7302a4c3d0f8e75fb33d82bc8338d/src/lib/message/L1ToL2MessageGasEstimator.ts#L215] that returns the desired gas parameters when sending L2-L1 messages.
...
- One can redeem live tickets using the [Arbitrum Retryables Transaction Panel][https://retryable-tx-panel-nitro.arbitrum.io]
