...
Let's start with some definitions - we'll connect the dots as we go:
 ...
 8. **Outbox contracts**: Smart contracts that are deployed to L1 and are responsible for transmitting retryable tickets to L2's inbox contracts. The Arbitrum DAO owns these.
 9.  **Inbox contracts**: Smart contracts that are deployed to L2 and are responsible for receiving retryable tickets from L1's outbox contracts. The Arbitrum DAO owns these.
