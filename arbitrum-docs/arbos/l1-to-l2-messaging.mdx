...
Cross-layer messaging is a complex topic, so this is a lengthy concept doc that progressively discloses technical complexity. Here's what we'll cover:
  ...
  1. **[Why this information is important](#1-why-this-information-is-important)**.
  2. **[The conceptual primitives](#2-the-conceptual-primitives)** that work together to enable L1-to-L2 messaging on Arbitrum.
  3. **[Elaborating on the conceptual primitives](#3-elaborating-on-the-conceptual-primitives)** by exploring how each of the conceptual primitives relate to one another in the context of cross-layer messaging.
  4. **[Learning by example](#4-learning-by-example)** with a minimal smart contract that exposes two functions. We'll compare the contract's behavior across two across deployment scenarios to test our understanding of the conceptual primitives.


This document doesn't assume any prior knowledge beyond familiarity with the following terms:
 ...
 2. **Nodes**: Computers running peer-to-peer client software that implements a particular chain's protocol. The protocol tells the nodes how to execute transactions, manage state, and exchange information. 
...
Cross-layer communication is a core feature of Arbitrum's L2 chains. From the perspective of decentralized application (dApp) development, it enables two key capabilities: **L2-to-L1 messaging**, and **L1-to-L2 messaging**:

<MermaidWithHtml>
  <Nodes>
    <Node id="1">Decentralized apps (dApps)</Node>
    <Node id="2">Layer 2 (L2 - Arbitrum)</Node>
    <Node id="3">Layer 1 (L1 - Ethereum)</Node>
    <Connection from="1" to="2" arrows="none"/>
    <Connection from="2" to="3" label="L2-to-L1 messaging"/>
    <Connection from="3" to="2" label="L1-to-L2 messaging"/>
  </Nodes>
  <NodeDescriptions>
    <NodeDescription for='1'>Decentralized apps usually need to: <br/> - exchange information with both both layers<br/> - trust that this information exchange is trustworthy<br/> - be able to verify that their L2 transactions are settling to Ethereum L1<br/> - expose functionality to users on both layers<br/> - enjoy guarantees of bidirectional state synchronization between layers</NodeDescription>
    <NodeDescription for='2'>L2 contracts can send messages to L1 contracts, which can trigger the execution of L1-hosted smart contracts.</NodeDescription>
    <NodeDescription for='3'>L1 contracts can send messages to L2 contracts, which can trigger the execution of L2-hosted smart contracts.</NodeDescription>
  </NodeDescriptions>
</MermaidWithHtml>

Without bidirectional cross-layer messaging, it would be impossible to build dApps that interact with both layers, which would severely limit the value and reach of your dApps.

:::caution Editor note

 - Low-confidence intro - this could be improved through more concrete / compelling examples. A "For example..." could help identify relatable situations in which these capabilities become critical. This could help make the information "sticky" for readers who are trying to solve specific problems in addition to learning how the stuff works.
...
The rest of this concept document focuses on constructing an understanding of L1-to-L2 messaging through the following **conceptual primitives**:

 1. **Messages**: Pieces of information that are sent from one contract to another, and/or one layer to another.
 2. **Retryable tickets**: A type of message sent from L1 chains to L2 chains in order to trigger the execution of an L2-hosted smart contract. Think of these as special-purpose DTOs (data transfer objects) that encapsulate the information needed to execute a smart contract: the contract's address, the function to call, the function's arguments, etc.
 3. **Receipts**: todo
 4. **ETH deposits**: todo
 5. **Delayed Inbox**: todo


## 3: Elaborating on the conceptual primitives

Let's explore how each of these primitives relate to one another in the context of cross-layer communication.
...
### 3.2: Retryable tickets
 ...
 - This atomicity is important because it means that the L1 transaction that requests the **submission** of a retryable ticket can be submitted for a fixed cost (which is a function of its calldata size). The cost of **executing** the retryable's transaction on L2 is... (todo, need to better my understanding of this from vCurrent, and communicate the value of atomicity as clearly as possible)
...
#### 3.2.1: Retryable ticket lifecycle: Submission
...
:::caution Editor note: Editing paused here
 ...
 - Once aligned on the direction of this structural edit, editing will resume here.

:::


#### 3.2.2: Retryable ticket lifecycle: Automatic redemption
...
#### 3.2.3: Retryable ticket lifecycle: Manual redemption
...
#### 3.2.4: Alternative "unsafe" retryable ticket creation
...
### 3.5: Transacting via the Delayed Inbox
...
### 4.2: Comparing behavior across deployment scenarios

Let's compare **the way that our cupcake contract behaves** across **two deployment scenarios**:

1. The contract is deployed straight to **Ethereum L1**
2. The contract is deployed straight to **Arbitrum L2**
...
:::caution Editor note
 ...
 - If we feel like this table is a value-add, we can add more technical precision, partialize it, and embed it elsewhere
...
|                                                                            | Cupcake contract deployed straight to L1                                                                                                            | Cupcake contract deployed straight to L2                                                                                                                      |
| -------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Where is the contract deployed?**                                        | Ethereum L1                                                                                                                                         | Arbitrum L2 (One or Nova)                                                                                                                                     |
...
| **Where is gas paid?**                                                     | • On Ethereum L1<br />• Paid to Ethereum validators                                                                                                 | • On Arbitrum L2<br />• Paid to Arbitrum validators                                                                                                           |
