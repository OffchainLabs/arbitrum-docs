...
<MermaidWithHtml>
  <Nodes>
    <Node id="1">🧍</Node>
    <Node id="2">Initiate an L1-L2 message</Node>
    <Node id="3">Does the user have enough funds to create a ticket?</Node>
    <Node id="4">if no- Ticket creation fails</Node>
    <Node id="5">if yes- Ticket is created</Node>
  ...
  <NodeDescriptions>
    <NodeDescription for='1'>The user who initiates an L1-L2 message</NodeDescription>
    <NodeDescription for='2'>A call to `inbox.createRetryable` function that puts the message in the L2 inbox that can be re-executed for some fixed amount of time if it reverts</NodeDescription>
    <NodeDescription for='3'>Logic that checks if the `msg.value` provided by the user is greater than or equal to `maxSubmissionCost + l2CallValue + gasLimit * maxFeePerGas`
                          This determines if the user's deposit alone will make the submission succeed</NodeDescription>
    <NodeDescription for='4'>Ticket creation fails and no funds are deducted from the user</NodeDescription>
    <NodeDescription for='5'>A ticket is created and added to the retryable buffer on L2
                          Funds (`l2CallValue + submissionCost`) are deducted to cover the callvalue from the user and 
                          placed into escrow (on L2) for later use in redeeming the ticket</NodeDescription>
  </NodeDescriptions>
</MermaidWithHtml>

Once the ticket is successfully cretaed, the two following conditions are checked: 1- If the user's L2 balance is greater than (or equal to) `l2Basefee * gasLimit` **and** 2- if the `maxFeePerGas` (provided by the user in the ticket submission process)  is greater than (or equal to) the `l2Basefee`. If these conditions are both passed successfully, the ticket will be scheduled for the auto-redeem (see Automatic Remeption of the Ticket). Otherwise it'll follow th steps in Manual Redemption of the Ticket.

Title: Automatic Remeption of the Ticket:

<MermaidWithHtml>
  <Nodes>
    <Node id="1">Does the auto-redeem succeed?</Node>
    <Node id="2">If yes- Ticket is executed</Node>
    <Node id="3">Ticket is deleted</Node>
    <Node id="4">if no- excessFeeRefundAddress gets refunded</Node>
    <Connection from="1" to="2"/>
    <Connection from="2" to="3"/>
    <Connection from="1" to="4"/>
  </Nodes>
  <NodeDescriptions>
    <NodeDescription for='1'>Check if the user's L2 Balance is greater than (or equal to) l2Basefee * gasLimit **and** maxFeePerGas is greater than (or equal to) the l2Basefee</NodeDescription>
    <NodeDescription for='2'>Ticket is executed, the actual submissionFee is refunded to the excessFeeRefundAddress since the ticket was not kept in the buffer on L2</NodeDescription>
    <NodeDescription for='3'>The ticket gets deleted from the L2 retryable buffer</NodeDescription>
    <NodeDescription for='4'>excessFeeRefundAddress gets refunded with (maxGas - gasUsed) * gasPrice. Note that this amount is capped by gasUsed * baseFee in the auto-reddem</NodeDescription>
  </NodeDescriptions>
</MermaidWithHtml>


Title: Manual Remeption of the Ticket:

<MermaidWithHtml>
  <Nodes>
    <Node id="1">Is the ticket manually cancelled or not redeemed within 7 days?</Node>
    <Node id="2">If yes- excessFeeRefundAddress gets refunded</Node>
    <Node id="3">Ticket is deleted</Node>
    <Node id="4">if no- Is the ticket manually redeemed?</Node>
    <Connection from="1" to="2"/>
    <Connection from="2" to="3"/>
    <Connection from="1" to="4"/>
  ...
  <NodeDescriptions>
    <NodeDescription for='1'>Logic that determines if the ticket is manually cancelledor  not redeemed within 7 days (i.e., is expired)</NodeDescription>
    <NodeDescription for='2'>excessFeeRefundAddress is refunded with the l2CallValue</NodeDescription>
    <NodeDescription for='3'>The ticket gets deleted from the L2 retryable buffer</NodeDescription>
    <NodeDescription for='4'>Logic that determines if the ticket is manually manually redeemed</NodeDescription>
  </NodeDescriptions>
</MermaidWithHtml>
<!-- 
