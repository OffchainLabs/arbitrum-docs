...
See [here](../assets/retryable_flowchart.png) for a flowchart of the lifecycle of a retryable ticket. -->


Here we walk through the different stages of the lifecycle of a retryable ticket.

1. Creating a retryable ticket is initiated with a transaction to the `createRetryableTicket` functiuon of the inbox contract. This function has a set of parameters which lead to a succesfull creation (and redemption) of the ticket if set carefully:

    - `address to`: The destination L2 contract address
    - `uint256 l2CallValue`: The call value for retryable L2 message
    ...
    - `address excessFeeRefundAddress`: The L2 address to which the excess fee is creadited (gasLimit x maxFeePerGas - ticket execution cost)
...
2. As shown in th chart, the sender's deposit must be enough to make the submission succeed, cover the L1 submission and L2 execution of the ticket.
...
5. It is very important to note that the submission of a tuicket on L1 is separable / asynchronous with its execution on L2, i.e., a successful L1 ticket creation does not guarantee a successful redemption.
Assuming the sender has provided enough gas that can cover the L2 execution/redemption of the ticket on step 1, 
ticket's submission is followed by an attempt to execute it on L2 (i.e., an **auto-redeem** using the supplied gas, as if the [`redeem`](./precompiles#ArbRetryableTx) method of the ArbRetryableTx precompile had been called).
...
- If the ticket is sucessfully redeemed, it will execute with the sender, destination, callvalue, and calldata of the original submission. The submission fee is refunded to the user at L2 (`excessFeeRefundAddress`).
Note that to ensure successful auto-redeem of the ticket, you could use Arbitrum SDK which provides a [function][https://github.com/OffchainLabs/arbitrum-sdk/blob/4cedb1fcf1c7302a4c3d0f8e75fb33d82bc8338d/src/lib/message/L1ToL2MessageGasEstimator.ts#L215] that returns the desired gas parameters when sending L2-L1 messages.

- If a redeem is not done at submission or the submission's initial redeem fails (for example, because the L2 gas price has increased unexpectedly), the submission fee is collected at L2 to cover the resources required to temporarily keep the ticket in memory for a fixed period (currently one week) and 
_anyone_ can attempt to redeem it again by calling `ArbRetryableTx`'s redeem precompile method, which donates the call's gas to the next attempt. 
...
If a ticket that carries a value expires after 7 days without being redeemed or re-scheduled to a future date, those values get lost without possibility of recovering them.
...
- One can redeem live tickets using the [Arbitrum Retryables tx Panel][https://retryable-tx-panel-nitro.arbitrum.io]
- The calldata of a ticket is saved on L2 until it is redeemed or expired 
- Redeeming cost of a ticket will not increase over time, it only depends on the size of the calldata being kept
...
- if the submit transaction contains gas, schedule a redeem of the new Retryable, using the supplied gas, as if the [`redeem`](./precompiles#ArbRetryableTx) method of the [`ArbRetryableTx`](./precompiles#ArbRetryableTx) precompile had been called.
...
A Retryable's beneficiary has the unique power to [`cancel`](./precompiles#ArbRetryableTx) the Retryable. This has the same effect as the Retryable timing out.
...
In addition to L2 gas costs, the creator of a retryable ticket is required to pay a retryable “submission fee.” This submission fee is directly proportional to the size of the retryable’s data. If the retryable’s auto-redeem fails, the submission fee is collected at L2 to cover the resources required to temporarily keep the retryable ticket in memory; if the auto-redeem succeeds, the submission fee is refunded to the user at L2.
