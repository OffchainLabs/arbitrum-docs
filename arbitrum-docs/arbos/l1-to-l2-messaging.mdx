...
- if the submit transaction contains gas, schedule a redeem of the new Retryable, using the supplied gas, as if the [`redeem`](./precompiles.mdx#)ArbRetryableTx) method of the [`ArbRetryableTx`](./precompiles.mdx#)ArbRetryableTx) precompile had been called.
...
A Retryable's beneficiary has the unique power to [`cancel`](./precompiles.mdx#)ArbRetryableTx) the Retryable. This has the same effect as the Retryable timing out.
