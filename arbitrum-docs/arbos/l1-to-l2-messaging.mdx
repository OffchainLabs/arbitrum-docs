...
<MermaidWithHtml>
  <Nodes>
    ...
    <Node id="3">msg.value >= maxSubmissionCost + l2CallValue + gasLimit * maxFeePerGas</Node>
    <Node id="4">Fail</Node>
  ...
  <NodeDescriptions>
    ...
    <NodeDescription for='2'>A function that puts the message in the L2 inbox that can be re-executed for some fixed amount of time if it reverts</NodeDescription>
    <NodeDescription for='3'>Logic within that determines the user's deposit alone will make the submission succeed</NodeDescription>
    <NodeDescription for='3'>If success; a ticket is created and added to the retryable buffer
                            Funds (l2CallValue + submissionCost) are deducted to cover the callvalue from the user and 
                            placed into escrow for later use in redeeming the ticket</NodeDescription>
    <NodeDescription for='3'>If fail; Ticket creation fails and no funds are deducted</NodeDescription>
  </NodeDescriptions>
</MermaidWithHtml>
