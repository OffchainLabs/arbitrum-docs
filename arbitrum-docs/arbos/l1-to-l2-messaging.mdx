...
### EDITOR NOTES
 ...
 1. Introduced frontmatter (see frontmatter guidance in our patterns guide).
 2. Sentence-cased section titles (see style guidance in our patterns guide).
 3. "L1-to-L2" to maintain consistency with L2-to-L1 doc.
 4. Update the introduction to set reader expectations, and to connect explicitly to their need ("build dApps on Arbitrum")
 5. Use our cupcake vending machine smart contract. Reasons:
    - "construct understanding" using familiar concepts and experiences
    - promote awareness of our investments elsewhere
    - IP reuse to save energy
    - using our cupcake example as a canonical education primitive
 6. Use a table to compare properties across deployment scenarios
 7. Identified areas where content can move elsewhere 

TODOS:

  1. Fact check
  2. Ensure that it's ok to refer to creation<>submission interchangeably, in terms of developer expectations
  3. If (2) is a yes, refer to it as submission throughout
  4. If (2) is a no, elaborate on the difference between creation and submission
  5. Vibe-check the numbered sections
  6. Vibe-check the table
  7. Vibe-check the flow
  8. Finish editing
...
This is a complex topic, so this is a lengthy concept doc that progressively discloses technical complexity. Here's what we'll cover:
...
5. **Tying it all together** through an elaboration upon each of the conceptual primitives, when you should care about them, and how they relate to one another.
...
Without cross-layer messaging, it would be impossible to build dApps that interact with both layers. The ability to deploy smart contracts that can be executed and invoked by nodes on both layers is a key enabler of Arbitrum's L2 chains' scalability and interoperability with Ethereum's L1 chain.
...
Let's start with some definitions - we'll connect the dots as we go:
 ...
 4. **Smart contracts**: Programs that can be deployed to a layer and whose code is hosted and executed by nodes on that layer.
 ...
 7. **Retryable tickets**: Special messages sent from L1 nodes to L2 nodes in order to trigger the execution of an L2-hosted smart contract.
 8. **Outbox contracts**: Smart contracts that are deployed to L1 and are responsible for transmitting retryable tickets to L2's inbox contracts.
 9.  **Inbox contracts**: Smart contracts that are deployed to L2 and are responsible for receiving retryable tickets from L1's outbox contracts.
 10. **Gateway contracts**: Smart contracts that are deployed to both L1 and L2 and are responsible for managing the flow of information between the two layers.
...
|                                                                            | Straight to L1 | Stright to L2 |
| -------------------------------------------------------------------------- | -------------- | ------------- |
| **Where is the contract deployed?**                                        | Ethereum L1    | Arbitrum L2   |
| **Where is the executable code hosted?**                                   |                |               |
| **Can the contract code be executed by L1 nodes?**                         |                |               |
| **Can the contract code be executed by L2 nodes?**                         |                |               |
| **Can the contract state be read from L1 nodes?**                          |                |               |
| **Can the contract state be read from L2 nodes?**                          |                |               |
| **To what degree is state mirrored between L1 and L2 nodes?**              |                |               |
| **Where is the contract's state stored?**                                  |                |               |
| **How does the contract deployment impact L1's physical state?**           |                |               |
| **How does the contract deployment impact L2's physical state?**           |                |               |
| **When is gas paid?**                                                      |                |               |
| **Where is gas paid?**                                                     |                |               |
| **Who pays gas?**                                                          |                |               |
| **Can another L1 contract call the deployed contract's methods directly?** |                |               |
| **Can another L2 contract call the deployed contract's methods directly?** |                |               |
| **When is information sent from L1 to L2?**                                |                |               |
| **When is information sent from L2 to L1?**                                |                |               |
| **What primitives are involved with L1-L2 messages?**                      |                |               |
| **What primitives are involved with L2-L1 messages?**                      |                |               |
| **When does L1's state change?**                                           |                |               |
| **When does L2's state change?**                                           |                |               |
| **Can I query L1 state via L2?**                                           |                |               |
| **Can I query L2 state via L1?**                                           |                |               |
| **When are retryable tickets involved?**                                   |                |               |
| **How are retryable tickets involved?**                                    |                |               |
| **When are outbox contracts involved?**                                    |                |               |
| **How are outbox contracts involved?**                                     |                |               |
| **When are inbox contracts involved?**                                     |                |               |
| **How are inbox contracts involved?**                                      |                |               |
| **When are bridge contracts involved?**                                    |                |               |
| **How are bridge contracts involved?**                                     |                |               |
| **How are gateway contracts involved?**                                    |                |               |


<br />

## 5: Breaking down the details
...
### 5.1: Messages
 ...
 - Here's how **messages** relate to some of the other primitives we started with:
   - **Retryable tickets**: Retryable tickets are a type of message. They're used to send information from L1 to L2.
   - todo
...
<!-- todo: move this to procedural guidance (how-to):
...
:::caution Editing paused here

See inline HTML comments below the frontmatter of this document for **edit notes**.
...
<MermaidWithHtml>
  <Nodes>
    <Node id="1">Arbitrum One (L2)</Node>
    <Node id="2">Arbitrum Nova (L2)</Node>
    <Node id="3">Ethereum (L1)</Node>
    <Connection from="1" to="3" arrows="none"/>
    <Connection from="2" to="3" arrows="none"/>
  </Nodes>
  <NodeDescriptions>
    <NodeDescription for='1'><code>🧍</code> The user who initiates an L1-L2 message</NodeDescription>
  </NodeDescriptions>
