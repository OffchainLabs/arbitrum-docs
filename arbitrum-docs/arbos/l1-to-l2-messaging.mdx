...
See [here](../assets/retryable_flowchart.png) for a flowchart of the lifecyclew of a retryable ticket.
...
In addition to costs costs, the creator of a retryable ticket is required to pay a retryable “submission fee.” This submission fee is directly proportional to the size of the retryable’s data. If the retryable’s auto-redeem fails, the submission fee is collected at L2 to cover the resources required to temporarily keep the retryable ticket in memory; if the auto-redeem succeeds, the submission fee is refunded to the user at L2.
