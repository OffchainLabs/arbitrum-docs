...
<p>
If anywhere between 2 and 18 of the committee members are well behaved, the AnyTrust chain operates in "Rollup mode"; i.e., data gets posted on the parent chain.
</p>

<p>
In what should be the common and happy case, however, in which at least 19 of the 20 committee members are well behaved, the system operates without posting the child chain's data on the parent chain, and thus, users pay significantly lower fees. This is the core upside of AnyTrust chains over rollups.
</p>
...
<p>
You'll need the transaction hash of the "initiating transaction":  the parent chain transaction hash for a Parent-to-child chain message (e.g., a deposit), or the child chain transaction hash for a Child-to-parent chain message (e.g., a withdrawal).
</p>
...
### If there is a dispute, can my child chain transaction get reorged / thrown out / "yeeted"?
<p>
Nope; once an Arbitrum transaction is included on the parent chain, there is no way it can be reorged (unless the parent chain itself reorgs, of course). A "dispute" involves Validators disagreeing over execution, i.e., the outputted state of a chain. The inputs, however, can't be disputed; they are determined by the Inbox on the parent chain. (See <a href="https://developer.arbitrum.io/tx-lifecycle">Transaction Lifecycle</a>)
</p>


### ...okay but if there's a dispute, will my transaction get delayed?
<p>
The only thing that a dispute can add delay to is the confirmation of Child-to-parent chain messages. All other transactions continue to be processed, even while a dispute is still undergoing. (Additionally: in practice, most child-to-parent chain messages represent withdrawals of fungible assets; these can be trustlessly completed <em>even during a dispute</em> via trustless fast "liquidity exit" applications. See <a href="https://developer.arbitrum.io/arbos/l2-to-l1-messaging">Child-to-parent chain messages</a>).
</p>
...
<p>
The <em>Validators</em> are the ones responsible for the safety of the chain; i.e., making bonded claims about the chain state, disputing each other, etc.
</p>
...
<p>
A week is expected to be more than enough time for validators to carry out an interactive dispute, assuming they don't encounter difficulty in getting their transactions included on the parent chain. One week was chosen following the general consensus among the Ethereum research community — as well as other layer 2 projects — to provide enough time for the community to socially coordinate in the case of a coordinated Ethereum-bonded censorship attack.
</p>
