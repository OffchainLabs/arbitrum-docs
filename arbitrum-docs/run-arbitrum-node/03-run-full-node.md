...
- Database snapshot (required for Arbitrum One, optional for other chains)
  ...
  - When running more than one node, it's easier to to manually download the different parts of the snapshot, join them into a single archive and host it locally for your nodes. You can then use `--init.url="file:///path/to/snapshot/in/container/snapshot-file.tar"` to use it. (For how to manually donwload the snapshot parts, please see [Downloading the snapshot manually](/run-arbitrum-node/nitro/03-nitro-database-snapshots.md#downloading-the-snapshot-manually))
