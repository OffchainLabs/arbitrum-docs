...
<VanillaAdmonition type="note">
  This documentation is intended for production sequencer deployments. If you want to set up a
  sequencer for testing or on a testnet, please refer to [How to run a testnet sequencer
  node](./04-run-sequencer-node.mdx).
</VanillaAdmonition>
