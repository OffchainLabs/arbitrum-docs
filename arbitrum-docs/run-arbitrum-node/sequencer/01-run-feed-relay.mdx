...
- Here is an example of how to run the feed relay for Arbitrum One:
  ```shell
  docker run --rm -it  -p 0.0.0.0:9642:9642 --entrypoint relay @@latestNitroNodeImage=offchainlabs/nitro-node:v3.6.0-fc07dd2@@ --node.feed.output.addr=0.0.0.0 --node.feed.input.url=wss://arb1.arbitrum.io/feed --chain.id=42161
  ```
