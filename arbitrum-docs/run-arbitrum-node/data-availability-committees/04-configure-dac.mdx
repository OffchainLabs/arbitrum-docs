...
```shell
docker run -v $(pwd):/data/keyset --entrypoint datool @@latestNitroNodeImage=offchainlabs/nitro-node:v3.5.5-90ee45c@@ dumpkeyset --conf.file /data/keyset/keyset-info.json
```
...
The `SequencerInbox` can be configured with the new keyset by invoking the [setValidKeyset](https://github.com/OffchainLabs/nitro-contracts/blob/@@nitroContractsCommit=780366a0c40caf694ed544a6a1d52c0de56573ba@@/src/bridge/SequencerInbox.sol#L751) method. Note that only the chain owner can call this method.
