...
```shell
docker run -v $(pwd):/data/keyset --entrypoint datool @@latestNitroNodeImage=offchainlabs/nitro-node:v3.6.2-5b41a2d@@ dumpkeyset --conf.file /data/keyset/keyset-info.json
```
...
The `SequencerInbox` can be configured with the new keyset by invoking the [setValidKeyset](https://github.com/OffchainLabs/nitro-contracts/blob/@@nitroContractsCommit=601afc77e5b2a2dfd6f7075f2f3a48989c0be740@@/src/bridge/SequencerInbox.sol#L751) method. Note that only the chain owner can call this method.
