...
```shell
docker run -v $(pwd):/data/keyset --entrypoint datool @@latestNitroNodeImage=offchainlabs/nitro-node:v3.6.3-1d5699d@@ dumpkeyset --conf.file /data/keyset/keyset-info.json
```
...
The `SequencerInbox` can be configured with the new keyset by invoking the [setValidKeyset](https://github.com/OffchainLabs/nitro-contracts/blob/@@nitroContractsCommit=0aec58c99fd7af4502d89335cdbe2c29248f44fa@@/src/bridge/SequencerInbox.sol#L751) method. Note that only the chain owner can call this method.
