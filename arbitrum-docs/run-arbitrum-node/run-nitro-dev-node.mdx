...
```bash
docker run -it --rm --name nitro-dev -p 8547:8547 offchainlabs/nitro-node:@latestNitroNodeImage@ --dev --http.addr 0.0.0.0
```

- <code>@latestNitroNodeImage@</code> is the latest version of the Nitro image.
...
These functions allow you to fully configure and optimize the chain for your development needs.
