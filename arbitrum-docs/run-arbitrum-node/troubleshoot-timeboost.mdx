...
In this scenario, an error response is immediately returned after you send an express lane transaction. The transaction's `sequenceNumber` will not be consumed if the error is timeboost related and consumed otherwise, hence you may need to re-submit your transaction after rectifying any errors with the appropriate sequence number. See [How to submit transactions to the express lane](../run-arbitrum-node/how-to-use-timeboost.mdx#how-to-submit-transactions-to-the-express-lane) for a full list of error responses and how to interpret them.
