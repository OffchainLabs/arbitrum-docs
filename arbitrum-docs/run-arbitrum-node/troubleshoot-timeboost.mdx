...
This means the sequence number of the received transaction is ahead of the next expected sequence number by more than the limit. Submit the messages in sequential order.
...
This type of error means that the underlying transaction of the express lane submission has errored and the corresponding `sequenceNumber` has been consumed.
