...
The response for a <a data-quicklook-from="transaction">Transaction</a> submission is returned only once the block containing the transaction is produced. For example, irrespective of the type of transaction (express lane or normal), if a transaction is sent to the <a data-quicklook-from="sequencer">Sequencer</a> at `t=0ms` and it took 50ms to arrive at the sequencer (defined as `time_to_arrive`), then the expected response time is at least t=250ms since blocks are produced every 250ms. As another example, if a transaction is sent at `t=220ms` and the `time_to_arrive` was 50ms, then the response time for this transaction will be at `t=500ms`. The reason for this is because blocks are produced every 250ms and so in this second example, the transation's response is returned when the next block gets produced.
