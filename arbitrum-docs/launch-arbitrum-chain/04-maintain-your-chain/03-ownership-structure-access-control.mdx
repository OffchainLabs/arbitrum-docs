...
A chain owner is simply an address; it is set by the Arbitrum chain's deployer and can represent any sort of governance scheme (i.e., it could be an EOA (as is set via the [Arbitrum chain Quickstart](launch-arbitrum-chain/arbitrum-chain-quickstart.md)), a Multisig, a governance token system, etc.)
