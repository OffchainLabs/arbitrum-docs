...
description: "Launch your own Arbitrum chain with the Arbitrum Nitro codebase's new license. Settle to Arbitrum's L2 chains via bridge contracts on the underlying L2 chain (Sepolia, One or Nova). No need for permission from the Arbitrum DAO or Offchain Labs to create your Arbitrum chain. Modify the Nitro codebase freely for your chain."
sidebar_position: 2
...
This quickstart is for developers who want to launch their own Arbitrum Arbitrum chain using the [Arbitrum chain deployment portal](https://orbit.arbitrum.io/).
...
## Step 1: Acquire Arbitrum Testnet `ETH` (and the native token for Arbitrum chains with custom gas tokens)

You'll need at least 1.2 testnet `ETH` for regular Arbitrum chains or 0.8 `ETH` plus 0.4 of your desired native token for Arbitrum chains with a custom gas token. The funds are needed to cover the cost of deploying your Arbitrum chain's **base contracts** to its **base chain** (Arbitrum Sepolia).
...
## Step 3: Configure your Arbitrum chain's deployment

<!-- https://orbit-deployment-ui.vercel.app/ -->

Visit the [Arbitrum chain deployment portal](https://orbit.arbitrum.io/). You'll be prompted to connect your wallet. You may be prompted to add the **Arbitrum Sepolia** network to your wallet and/or to switch your wallet to this network; approve this.
...
The below table provides a brief description of each of these configuration parameters. We recommend sticking to the defaults; to learn more about customizing your Arbitrum chain's deployment configuration, visit [How (and when) to customize your Arbitrum chain's deployment config](/launch-arbitrum-chain/02-configure-your-chain/common-configurations/02-use-a-custom-gas-token-rollup.mdx):
...
| **Chain name**                | A human-readable way to distinguish your Arbitrum chain from other Arbitrum chains. Users, developers and the wider community will refer to your chain by your `Chain name` and/or your `Chain ID`.                                                                                                                                                                                                                                                                                      |
| **Challenge period (blocks)** | The amount of time that your Arbitrum chain's nodes have to dispute the current state of the chain before it's confirmed (and ultimately finalized) on the underlying L2 chain (e.g., Arbitrum Sepolia). Note that this refers to the number of blocks on the underlying L1 chain (e.g., Ethereum's Sepolia chain).                                                                                                                                                                   |
| **Stake token**               | The token that your chain's validators must bond in order to participate in your chain. This is hardcoded to `ETH` for now, but future versions of Arbitrum chains will let you specify an arbitrary `ERC-20` token contract here.                                                                                                                                                                                                                                                    |
...
| **Gas token**                 | The address of the `ERC-20` token on the parent chain that is intended to be used as the native gas token on the Arbitrum chain. This token must already be deployed natively on the parent chain and is bridged to the Arbitrum chain during chain deployment. This feature is only supported on AnyTrust chains currently, and more information around token restrictions can be found [here](https://docs.arbitrum.io/launch-arbitrum-chain/how-tos/customize-deployment-configuration). |
...
Your chain's validators are responsible for validating the integrity of transactions and posting assertions of the current state of your Arbitrum chain to its base chain. In production scenarios, your Arbitrum chain would likely be hosted by a network of validator nodes working together. For your local Arbitrum chain, you can stick to the auto-generated single validator address.
...
Each of the validator addresses specified in this step will be added to an allow-list in one of your chain's **base contracts**, allowing them each to **bond** and validate transactions submitted to your Arbitrum chain.
...
When we say "base contracts" and "base chain", we're referring to your Arbitrum chain's L2 contracts and the L2 chain that they're deployed to, respectively. We'll use these terms throughout the rest of this guide.
...
- You submitted a deployment transaction to an Arbitrum chain "factory" smart contract on the Arbitrum testnet, the public L2 chain that your local Arbitrum chain will settle transactions to.
- This Arbitrum chain smart contract then initialized your Arbitrum chain's base contracts with the values that you specified in the previous step, and deployed these base contracts to the Arbitrum testnet.

Your Arbitrum chain's base contracts are responsible for facilitating the exchange of information between your chain's node(s) and its base chain's nodes. This includes the batch posting of transactions from your Arbitrum chain to its base chain, the staking of tokens by your Arbitrum chain's validators, the challenge mechanism, bridging mechanisms, and more.
...
The current version of Arbitrum AnyTrust chains uses a single Data Availability Server and assigns a null value to its private key in order to generate an initial keyset. As part of this transaction process, you'll assign this initial keyset to your recently generated `SequencerInbox` contract.
...
3. Deploy your Token Bridge contracts on both L2 and local Arbitrum chains.
...
Your local Arbitrum chain is now running. You'll see an `outputInfo.json` file in the main directory of your script folder––this contains more information about your chain, including the addresses of your chain's base contracts.

### Appendix A: Logging

Run this command in the root directory of your cloned Arbitrum chain setup script repo to view your chain's logs:
...
If you need to deposit more `ETH` (or native tokens) into your Arbitrum chain account, run this command on the base directory of the setup script, replacing `0xYourPrivateKey` with the private key of the originating account, and `<AMOUNT>` with the amount to send:
