...
Arbitrum One's Validator set is currently whitelisted; overtime, we expect [governance](http://docs.arbitrum.foundation/) to expand the whitelist and eventually be removed entirely. For more info see ["State of Progressive Decentralization"](http://docs.arbitrum.foundation/state-of-progressive-decentralization).
