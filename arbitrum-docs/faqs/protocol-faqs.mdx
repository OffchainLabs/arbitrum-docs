...
Arbitrum One's Validator set is currently whitelisted; overtime, we expect [governance](TODO_GOV_ROOT_URL) to expand the whitelist and eventually be removed entirely. For more info see ["State of Progressive Decentralization"](TODO_STATE_OF_DECENTRALIZATION_URL).
