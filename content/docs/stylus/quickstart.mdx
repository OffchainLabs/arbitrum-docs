...
description: 'Leads a developer step by step writing and deploying a smart contract in Rust using Stylus'
...
This guide will get you started with <Term id="stylus">Stylus</Term>' basics. We'll
cover the following steps:

1. [Setting up your development environment](/docs/stylus/quickstart#setting-up-your-development-environment)
2. [Creating a Stylus project with cargo stylus](/docs/stylus/quickstart#creating-a-stylus-project-with-cargo-stylus)
3. [Checking the validity of your contract](/docs/stylus/quickstart#checking-if-your-stylus-project-is-valid)
4. [Deploying your contract](/docs/stylus/quickstart#deploying-your-contract)
5. [Exporting your contract's ABIs](#exporting-the-solidity-abi-interface)
6. [Calling your contract](/docs/stylus/quickstart#calling-your-contract)
7. [Sending a transaction to your contract](/docs/stylus/quickstart#sending-a-transaction-to-your-contract)
...
[cargo stylus](https://github.com/OffchainLabs/stylus-sdk-rs/tree/main/cargo-stylus) is a CLI toolkit built to facilitate the development of Stylus contracts. For a full list of commands and options, see the [`cargo stylus` commands reference](/docs/stylus/cli-tools/commands-reference).
...
At this point, you can move on to the next step of this guide or develop your first Rust smart contract. Feel free to use the [Stylus Rust SDK reference section](/docs/stylus/reference/overview) as a starting point; it offers many examples to help you quickly familiarize yourself with Stylus.
...
The contract can fail the check for various reasons (on compile, deployment, etc...). Reading the [Invalid Stylus WASM Contracts explainer](https://github.com/OffchainLabs/stylus-sdk-rs/blob/main/cargo-stylus/VALID_WASM.md) can help you understand what makes a WASM contract valid or not. For other deployment and activation errors (e.g., `program activation failed`), see [common issues](/docs/stylus/troubleshooting/common-issues).
...
As of ArbOS 61, Stylus contracts can be up to 96 KB. If your compressed WASM exceeds 24 KB, cargo stylus deploy automatically splits the binary into fragments and deploys a collection contract that references them. You don't need to do anything different — the resulting contract address (the collection) is what callers use, and it behaves like any other Stylus contract. Read more in [Deploying contracts larger than 24 KB](/docs/stylus/cli-tools/overview#deploying-contracts-larger-than-24-kb).
...
Ensure you save the console output to a file that you'll be able to use with your <Term id="dapp">decentralized app</Term>.
...
```toml
[dev-dependencies]
stylus-sdk = { version = "@@stylusSdkVersion=0.10.7@@", features = ["stylus-test"] }
```
...
For more advanced testing techniques and best practices, see the [Testing contracts with Stylus guide](/docs/stylus/fundamentals/testing-contracts).

## Conclusion

Congratulations! You've successfully initialized, deployed, and interacted with your first contract using Stylus and Rust.

Feel free to explore the [Stylus Rust SDK reference](/docs/stylus/reference/overview) for more information on using Stylus in your Arbitrum projects.
