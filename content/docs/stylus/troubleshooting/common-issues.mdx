...
stylus-sdk = { version = "<Var name="stylusSdkVersion" />", default-features = false }
```
...
See [Optimizing binaries guide](/docs/stylus/how-tos/optimizing-binaries) for more strategies.
...
3. **Use Stylus-compatible alternatives**: See [recommended libraries](/docs/stylus/advanced/recommended-libraries).
...
See [Activation concepts](/docs/stylus/concepts/activation) for more details.
...
<VanillaAdmonition type="warning" title="Panics produce a dataless revert">

Panicking and other hard errors in Stylus programs cause a **dataless revert** — the transaction reverts but no error data is returned to the caller. If you need rich error messages onchain, return `Result<T, Vec<u8>>` from your function and encode the error explicitly rather than relying on `panic!`.
...
See [Gas optimization guide](/docs/stylus/best-practices/gas-optimization) for more strategies.
...
```toml
[toolchain]
channel = "<Var name="stylusRustToolchainFull" />"
...
1. **Ensure methods use `#[public]` macro**:
...
impl MyContract {
    ...
    // ❌ This won't be exported (no #[public])
...
See [Type conversions guide](/docs/stylus/fundamentals/data-types/conversions-between-types).
...
See [Debugging guide](/docs/stylus/cli-tools/debugging-tx) for detailed walkthrough.
...
- Review [security best practices](/docs/stylus/best-practices/security)
- Study [gas optimization](/docs/stylus/best-practices/gas-optimization)
- Learn [debugging techniques](/docs/stylus/cli-tools/debugging-tx)
