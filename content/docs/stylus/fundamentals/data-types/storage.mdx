...
<ImageZoom src="/img/stylus-storage-types-hierarchy.svg" alt="Stylus storage types" className="img-800px" />

_Storage type hierarchy showing primitives, collections, and custom struct options._
...
#### Available fixed-byte storage types
...
<VanillaAdmonition type="note" title="`.set()` vs `.setter()` — when to use each">

- **Primitives** (`StorageUint`, `StorageAddress`, `StorageBool`, …): call `.set(value)` directly to write.
- **Collections** (`StorageMap`, `StorageVec`, `StorageArray`): call `.setter(key)` to obtain a `StorageGuardMut` handle, then `.set(value)`, `.get()`, or `.erase()` on it. The guard enforces Rust's borrow rules and prevents storage aliasing at compile time.
- If you both read and write the same element, bind the `.setter(...)` result to a `let` so you avoid two slot lookups.
...
Dynamic string and byte storage:
...
<ImageZoom src="/img/stylus-storage-slots-layout.png" alt="Stylus storage slots" className="img-400px" />

_Storage slot layout showing how variables are packed into 32-byte slots and how mapping values are computed using keccak256 hashing._
...
- [Primitives](/docs/stylus/fundamentals/data-types/primitives): Basic types used in storage
- [Compound Types](/docs/stylus/fundamentals/data-types/compound-types): Complex types in storage
- [Type Conversions](/docs/stylus/fundamentals/data-types/conversions-between-types): Converting between types
