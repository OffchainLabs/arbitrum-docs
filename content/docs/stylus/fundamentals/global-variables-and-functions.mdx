...
- For L1-to-L2 retryable ticket transactions, the top-level sender's address will be [aliased](/docs/how-arbitrum-works/deep-dives/l1-to-l2-messaging#address-aliasing)
...
**Important:** Returns the original EOA (Externally Owned Account) that initiated the transaction, even through multiple contract calls. When deploying via a factory pattern, `msg_sender()` returns the factory address — use `tx_origin()` to get the actual deployer. See [Using `msg_sender()` instead of `tx_origin()`](/docs/stylus/how-tos/using-constructors#using-msg_sender-instead-of-tx_origin) for details.
...
**Note:** See [Block Numbers and Time](/docs/arbitrum-essentials/arbitrum-vs-ethereum/block-numbers-and-time) for more information on how this value is determined on Arbitrum.
...
- [Contracts](/docs/stylus/fundamentals/contracts) - Contract structure and methods
