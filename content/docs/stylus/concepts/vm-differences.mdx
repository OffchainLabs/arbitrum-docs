...
For verified per-operation figures, see the [performance comparison](/docs/stylus/best-practices/gas-optimization#performance-comparison) in the gas optimization guide.
...
- **Maximum size**: <Var name="maxCodeSizeBytes" /> bytes (24 KB) of deployed bytecode
- **Limit reason**: Block gas limit and deployment costs
- **Workaround**: Contract splitting, proxies

### Stylus contracts

- **Size limit**: Bounds the **decompressed** WASM, not the EVM 24 KB bytecode limit. It is a chain-configurable ArbOS parameter (`MaxWasmSize`), defaulting to 128 KB and raised to 256 KB at ArbOS61 and later
- **On-chain storage**: WASM is stored compressed, so the on-chain bytecode is smaller than the decompressed ceiling
- **Large programs**: At ArbOS61 and later, programs that exceed a single code account are split into a root plus fragments
...
For verified per-operation gas figures, see the [performance comparison](/docs/stylus/best-practices/gas-optimization#performance-comparison) in the gas optimization guide.
