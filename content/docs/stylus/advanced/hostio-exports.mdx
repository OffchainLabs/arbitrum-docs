...
Hostio (Host I/O) exports are low-level functions that provide direct access to the Stylus VM runtime. These functions are WebAssembly imports that allow Stylus programs to interact with the blockchain environment, similar to how EVM opcodes do in Solidity.
...
During execution, the Stylus VM intercepts calls to these functions and implements the actual functionality using the underlying ArbOS infrastructure.
...
- `address`: Pointer to the 20-byte address
- `dest`: Pointer to write the 32-byte balance value
...
- `key`: Pointer to the 32-byte storage key
- `dest`: Pointer to write the 32-byte value
...
- The SDK manages the cache automatically
...
See [Arbitrum block numbers and time](/docs/arbitrum-essentials/arbitrum-vs-ethereum/block-numbers-and-time) for more information on how block numbers work on Arbitrum chains.
...
For L1-to-L2 retryable ticket transactions, addresses are aliased. See [address aliasing documentation](/docs/how-arbitrum-works/deep-dives/l1-to-l2-messaging#address-aliasing).
...
Gets the price of ink on the EVM gas basis in gas basis points. See [Ink and Gas](https://docs.arbitrum.io/stylus/concepts/gas-metering) for more information.
...
Calls another contract with an optional value and gas limit. Equivalent to EVM's `CALL` opcode.
...
- `return_data_len`: Pointer to store the length of the return data
...
- `contract`: Pointer to write the deployed contract address (20 bytes)
...
- The address is determined by the sender and the nonce
- On failure, the address will be zero
...
Gets the amount of ink remaining—a Stylus-specific metering unit.
...
Efficiently computes the Keccak256 hash. Equivalent to EVM's `SHA3` opcode.
...
- Copies the overlapping portion only
...
- If not called, the return data will be empty
- The program exits naturally when the entrypoint returns
...
Prints UTF-8 text to the console. Only available in debug mode.
...
Prints integers to the console. Only available in debug mode.
...
Prints floating-point numbers to the console. Only available in debug mode with floating point enabled.
...
2. **Lack of bounds checking**: Can cause undefined behavior if pointers are invalid
...
When enabled, hostio functions are stubbed and return `unimplemented!()`—which is used for ABI generation.
...
- [Arbitrum block numbers and time](/docs/arbitrum-essentials/arbitrum-vs-ethereum/block-numbers-and-time)
