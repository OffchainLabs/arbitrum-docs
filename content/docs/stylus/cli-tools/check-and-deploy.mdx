...
2. **[Activation](/docs/stylus/concepts/activation)**: Trigger onchain compilation to native code and cache it for fast execution
...
- **Size limit exceeded**: Decompressed WASM exceeds `MaxWasmSize` (the chain-configurable limit, 128 KB by default and 256 KB at ArbOS61+)
...
Once the estimation looks correct, deploy for real:
...
cargo stylus deploy \
  --private-key-path=./key.txt \
  --cargo-stylus-version=<Var name="cargoStylusVersion" />
```
...
### Check if the contract is activated
...
### 7. Verify the data fee is reasonable
...
1. Use `#[no_std]` to eliminate the standard library:
...
4. Check SDK version compatibility:
   ```toml
   [dependencies]
   stylus-sdk = "<Var name="stylusSdkVersion" />"  # Use latest stable version
   ```
