...
- **Rust** (version <Var name="stylusRustToolchain" /> or higher)
- **cargo-stylus** crate
...
- **[Arbitrum RPC provider](#rpc-endpoint-compatibility)** with tracing endpoints enabled or a [local Stylus dev node](/docs/run-a-node/run-nitro-dev-node)
...
Both tracers are available on local nodes, but `stylusTracer` is more efficient. See the [list of RPC providers](/docs/chain-info#third-party-rpc-providers) for tracing support information.
