...
All Ethereum to Arbitrum token transfers are initiated via the router contract on the parent chain. The L1GatewayRouter forwards the token's deposit call to the appropriate gateway contract on the parent chain. The L1GatewayRouter is responsible for mapping the parent chain token addresses to L1Gateway contracts, thus acting as a parent/child chain address oracle and ensuring each token corresponds to only one gateway. The L1ArbitrumGateway then communicates to its counterpart gateway contract on the child chain via [retryable tickets](/how-arbitrum-works/10-l1-to-l2-messaging).

<ImageZoom src="/img/dapps-bridge_deposits.png" alt="" className="img-600px" />

Similarly, Arbitrum to Ethereum transfers are initiated via the router contract on the child chain. The L2GatewayRouter calls the token's gateway contract on the child chain, which in turn communicates to its corresponding gateway contract on the parent chain via [sending child-to-parent messages to the outbox](/how-arbitrum-works/11-l2-to-l1-messaging).

<ImageZoom src="/img/dapps-bridge_withdrawals.png" alt="" className="img-600px" />

For any given gateway pairing, calls must be initiated through the corresponding router contract on the respective layer.
...
You can find an example of implementation of a custom gateway in the page [How to bridge tokens via a custom gateway](/docs/build-decentralized-apps/token-bridging/bridge-tokens-programmatically/04-how-to-bridge-tokens-custom-gateway).

## Demos

Our [How to bridge tokens](/docs/build-decentralized-apps/token-bridging/bridge-tokens-programmatically/01-get-started) section provides example of interacting with Arbitrum's token bridge via the [Arbitrum SDK](https://github.com/OffchainLabs/arbitrum-sdk).
