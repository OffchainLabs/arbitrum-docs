...
As in Ethereum, Arbitrum clients submit transactions, and the system executes those transactions later. In Arbitrum, clients submit transactions by posting messages to the Ethereum chain, either [through the Sequencer](/how-arbitrum-works/03-sequencer) or via the chain's [Delayed Inbox](/how-arbitrum-works/03-sequencer).
...
Additionally, since the calldata of Arbitrum transactions (or the DAC certificate on <a href="/docs/intro/glossary#arbitrum-anytrust-chain">AnyTrust</a>chains) is posted to Ethereum, the gas paid when executing them includes a parent chain component to cover the costs of the batch poster.
...
The Arbitrum block number can also be retrieved within an Arbitrum contract via [ArbSys](/docs/build-decentralized-apps/precompiles/02-reference#arbsys) precompile:
