...
In addition to the [three transaction types](https://ethereum.org/en/developers/docs/transactions/#types-of-transactions) currently supported on Ethereum, Arbitrum adds additional types listed below and [documented in full detail here](/how-arbitrum-works/04-state-transition-function/04-modified-geth-on-arbitrum#transaction-types).
...
| `104`                 | `ArbitrumRetryTxType`           | Used to [manually redeem a retryable ticket](/how-arbitrum-works/10-l1-to-l2-messaging) on a child chain that failed to execute automatically (usually due to low gas) |
| `105`                 | `ArbitrumSubmitRetryableTxType` | Used to [submit a retryable ticket](/how-arbitrum-works/10-l1-to-l2-messaging#submission) via the Arbitrum bridge on the parent chain                                  |
...
| Field name | Description                                                                                                                                                                               |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `from`     | On parent to child chain transactions, this field will contain the [_aliased_ version](/how-arbitrum-works/10-l1-to-l2-messaging#address-aliasing) of the parent chain's `msg.sender` |
...
| Field name      | Description                                                                                                                                                                                                                       |
| --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `l1BlockNumber` | The block number of the first non-Arbitrum ancestor chain that would be used for `block.number` calls. More information in [Block numbers and time](/docs/build-decentralized-apps/arbitrum-vs-ethereum/02-block-numbers-and-time) |
| `gasUsedForL1`  | Amount of gas spent on parent chain calldata in units of child chain gas. More information in [Gas and fees](/how-arbitrum-works/09-gas-fees)                                                                                 |
...
| Field name      | Description                                                                                                                                                                                                                                  |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `l1BlockNumber` | An approximate block number of the first non-Arbitrum ancestor chain that occurred before this child chain block. More information in [Block numbers and time](/docs/build-decentralized-apps/arbitrum-vs-ethereum/02-block-numbers-and-time) |
...
| `gasLimit`   | Value is fixed at `0x4000000000000`, but it's important to note that Arbitrum One currently has a 32M gas limit per block. See [Chain params](/docs/build-decentralized-apps/reference/03-chain-params) for the gas limit of other chains |
