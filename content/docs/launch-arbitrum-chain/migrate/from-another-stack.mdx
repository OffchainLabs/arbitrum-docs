...
This document is provided for informational purposes only and does not constitute an end-to-end guide for executing the migration. <Term id="offchain-labs">Offchain Labs</Term> doesn’t have special knowledge or a private “fast path.” This document provides the extent of our guidance. Your current <Term id="raas">RaaS</Term> provider owns and executes the migration. We can provide [introductions to RaaSes](/docs/launch-arbitrum-chain/integrations/infrastructure-providers#rollup-as-a-service-raas-providers).
...
- **Lower fees**: <Term id="arbitrum-chain">Arbitrum chain</Term> Rollup or [AnyTrust](/docs/how-arbitrum-works/deep-dives/anytrust-protocol) for low <Term id="transaction">transaction</Term> fees.
- **Various customization options**: Custom gas token, flexible DA, [Timeboost](/docs/how-arbitrum-works/timeboost/gentle-introduction), [BoLD](/docs/how-arbitrum-works/bold/gentle-introduction), etc.
...
- **Ecosystem migration (simpler, faster)**: Launch a fresh Arbitrum chain. Users/app <Term id="bridge">Bridge</Term> funds and redeploy contracts. This approach is quicker and less risky, but it puts a burden on apps and users.
...
Zero knowledge (zk) pre-compiles don’t carry over. Port logic to Solidity or [Stylus](/docs/stylus/gentle-introduction) or remove/replace functionality. Please check with your RaaS regarding this matter.
...
- **<Term id="chain-state">Chain state</Term> migration:** A more complex migration where the chain state gets transplanted into the new Rollup stack/contract system, but the ecosystem and user experience may stay relatively the same.
...
1. Freeze legacy chain writes and make RPC read-only:
   1. Stop the legacy chain’s proposer/[Sequencer](/docs/how-arbitrum-works/deep-dives/sequencer) so no new blocks or transactions get created after the chosen upgrade block.
...
2. Ensure that you can export required state from your EVM-compatible <Term id="client">client</Term> at the upgrade block.
...
- [ ] Testnet deployment: Sequencer, [Batch Poster](/docs/how-arbitrum-works/deep-dives/assertions), <Term id="bonder">Bonder</Term>, DA config.
...
- [ ] Start batch poster/bonder and verify assertions on <Term id="L1">L1</Term>.
