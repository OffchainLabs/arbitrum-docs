...
<Callout type="info" title="Use the Arbitrum Chain SDK">

You can use these contracts to create Arbitrum chains. However, it is strongly recommended to go through the [Chain SDK](/docs/launch-arbitrum-chain/quickstart/sdk-introduction) to interact with them. Doing so prevents misconfiguring parameters and using appropriate defaults for most of them.
...
<Callout type="warning" title="The use of non-canonical factory contracts is unsupported">
...
- <Var name="maxDataSizeL2" /> is the parent chain is Ethereum
- <Var name="maxDataSizeL3" /> if the parent chain is a Layer 2 chain
