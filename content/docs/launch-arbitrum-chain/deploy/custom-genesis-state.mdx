...
- **Enable advanced features**: chain operators can use this feature to launch a chain with advanced customizations, such as [minting/burning gas tokens via third-party bridges](/docs/launch-arbitrum-chain/chain-config/costs/configure-native-mint-burn) and compliance-focused transaction filtering.

<Callout type="error" title="Preallocated balances are not backed by the parent chain">
...
| `enableNativeTokenSupply`    | `boolean` | No       | `false` | Flag to launch your chain with native interop tokens as [minting/burning gas tokens via third-party bridges](/docs/launch-arbitrum-chain/chain-config/costs/configure-native-mint-burn). |
...
<Callout type="warning" title="Generate the genesis before deploying the chain">
...
| `ENABLE_NATIVE_TOKEN_SUPPLY`   | (optional) Set to `true` if you want to launch your chain with native interop tokens as [minting/burning gas tokens via third-party bridges](/docs/launch-arbitrum-chain/chain-config/costs/configure-native-mint-burn). | `false`               |
...
Continue with [`createRollup` in the chain deployment guide](/docs/launch-arbitrum-chain/deploy/deploy-chain#2-deploy-the-arbitrum-chain), passing `createRollupConfig` as `params.config`. That guide documents these inputs, along with the batch poster and validator addresses.

<Callout type="warning" title="Preserve the generated chain configuration and base fee">
