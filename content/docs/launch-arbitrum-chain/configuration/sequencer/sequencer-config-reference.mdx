...
This page explains what the operational <a data-quicklook-from="sequencer">sequencer</a> flags do and what changes when you set them. For the complete list of every Nitro flag, see the [CLI flags reference](/docs/run-a-node/nitro/cli-flags-reference). For failure diagnosis, see [Sequencer troubleshooting](/docs/launch-arbitrum-chain/operate/sequencer-troubleshooting).
...
- **`my-url` must match the priority list exactly.** The coordinator compares strings. A trailing slash or a different hostname form means the sequencer never matches and never becomes chosen. See [Redis priority registration](/docs/run-a-node/high-availability-sequencer-docs#redis-priority-registration).
