...
The length of the <Term id="challenge-period">Challenge Period</Term> is measured based on the parent chain's notion of time, typically reflected in `block.number`. For L3s settling to <Term id="arbitrum-chain">Arbitrum chains</Term>, this period is determined by <Term id="layer-1-l1">L1</Term> block progression rather than Arbitrum's (<Term id="layer-2-l2">L2</Term>) blocks.
