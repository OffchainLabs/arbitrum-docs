...
While any Arbitrum chain can adopt Fast Withdrawals, we only _recommend_ that fast withdrawals be adopted by [AnyTrust](/docs/how-arbitrum-works/deep-dives/anytrust-protocol) chains with a minimum <a data-quicklook-from="validator">validator</a> and DAC member requirement. We explain both these recommendations below:
...
1. Soft finality from the [Sequencer](/docs/how-arbitrum-works/deep-dives/sequencer)'s confirmation of <a data-quicklook-from="transaction">transaction</a> inclusion (~@@l2BlockTimeMs=250@@ms)
2. Safe finality from <a data-quicklook-from="batch">batch</a> inclusion after Arbitrum One's [assertion](/docs/how-arbitrum-works/deep-dives/assertions) is included in an Ethereum block.
