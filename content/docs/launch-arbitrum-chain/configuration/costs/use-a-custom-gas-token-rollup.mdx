...
When deploying an <a data-quicklook-from="arbitrum-chain">Arbitrum chain</a> in [Rollup mode](/docs/run-a-node/data-availability#rollup-mode), you can use a custom **ERC-20** token as the native gas token. This token is usable for <a data-quicklook-from="transaction">Transaction</a> fees on that specific Arbitrum chain and reimbursing the [Batch Poster](/docs/how-arbitrum-works/deep-dives/assertions) for data posted to Ethereum. An example would be an <a data-quicklook-from="L2">L2</a> Rollup that uses **USDC** as its custom gas token but pays **ETH** to post data to <a data-quicklook-from="L1">L1</a> Ethereum.
