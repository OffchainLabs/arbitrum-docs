...
<VanillaAdmonition type="warning" title="Activation requires a separate Constitutional vote">

On Arbitrum One, enabling this feature requires a **Constitutional DAO vote**—it isn't activated as part of the ArbOS61 upgrade itself. The upgrade only makes tip collection _possible_ for chains that opt in. Priority fee collection isn't available on Arbitrum Nova.

</VanillaAdmonition>

## How to enable tip collection

The chain owner calls the [`ArbOwner` precompile](/docs/arbitrum-essentials/precompiles/reference#arbowner) at `0x0000000000000000000000000000000000000070`:
...
Without Step 2, the sequencer continues to use its existing ordering rules—[Timeboost](/docs/how-arbitrum-works/timeboost/gentle-introduction), if enabled, or otherwise first-come, first-served—regardless of the transaction's priority fee.

## References

- [Constitutional AIP: ArbOS61 Elara — Priority fees](https://forum.arbitrum.foundation/t/constitutional-aip-arbos-60-elara/30601)
- [`ArbOwner` precompile reference](/docs/arbitrum-essentials/precompiles/reference#arbowner)
