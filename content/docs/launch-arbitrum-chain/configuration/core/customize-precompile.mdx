...
To support these additional use cases, follow the instructions described in [How to customize your Arbitrum chain's behavior](/docs/launch-arbitrum-chain/configuration/core/customize-stf).
...
```shell
git clone --branch <Var name="nitroVersionTag" /> <https://github.com/OffchainLabs/nitro.git>
cd nitro
...
Next, follow the steps in [How to customize your Arbitrum chain's behavior](/docs/launch-arbitrum-chain/configuration/core/customize-stf#step-3-run-the-node) to build a modified Arbitrum Nitro node Docker image and run it.
...
<VanillaAdmonition type="important">
...
```shell
docker run --rm -it  -v /some/local/dir/arbitrum:/home/user/.arbitrum -p 0.0.0.0:8547:8547 -p 0.0.0.0:8548:8548 <Var name="latestNitroNodeImage" /> --parent-chain.connection.url=<YourParentChainUrl> --chain.id=<YourOrbitChainId> --http.api=net,web3,eth,debug --http.corsdomain=* --http.addr=0.0.0.0 --http.vhosts=*
```
...
If you do not customize the precompile before launching your Arbitrum chain network, please continue to follow the instructions on [Customize ArbOS](/docs/launch-arbitrum-chain/configuration/core/customize-arbos) to perform an ArbOS version control to avoid blockchain reorg.
