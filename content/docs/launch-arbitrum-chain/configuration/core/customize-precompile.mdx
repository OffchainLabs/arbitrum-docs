...
1.  Add new methods to an existing [precompile](https://github.com/OffchainLabs/{var:nitroRepositorySlug}/blob/{var:nitroVersionTag}/{var:nitroPathToPrecompiles}/) (local: `/precompiles`)
...
Using your favorite code editor, open an existing precompile from the [precompiles implementation](https://github.com/OffchainLabs/{var:nitroRepositorySlug}/blob/{var:nitroVersionTag}/{var:nitroPathToPrecompiles}/) directory (local `/precompiles`). We'll use `ArbSys.go` as an example. Open the corresponding Go implementation file (`ArbSys.go`) and add a simple `SayHi` method:
...
Next, follow the steps in [How to customize your Arbitrum chain's behavior](./customize-stf#step-3-run-a-local-node) to build a modified Arbitrum Nitro node Docker image and run it.
...
Next, navigate to [arbitrum_signer.go](https://github.com/OffchainLabs/go-ethereum/blob/{var:goEthereumCommit}/core/types/arbitrum_signer.go) and add the new precompile address.
...
Then, update [precompile.go](https://github.com/OffchainLabs/{var:nitroRepositorySlug}/blob/{var:nitroVersionTag}/{var:nitroPathToPrecompiles}/precompile.go) to register the new precompile under the `Precompiles()` method:
...
First, open the [arbosstate.go](https://github.com/OffchainLabs/{var:nitroRepositorySlug}/blob/{var:nitroVersionTag}/{var:nitroPathToArbosState}/arbosstate.go) file and locate the [ArbosState](https://github.com/OffchainLabs/nitro/blob/v2.2.0/arbos/arbosState/arbosstate.go#L38) structure. This is where ArbOS state is defined.

Define a state key called `myNumber` of type `storage.StorageBackedUint64`. You can find more types in [storage.go](https://github.com/OffchainLabs/{var:nitroRepositorySlug}/blob/{var:nitroVersionTag}/{var:nitroPathToStorage}/storage.go):
...
Next, head back to the [precompiles directory](https://github.com/OffchainLabs/{var:nitroRepositorySlug}/blob/{var:nitroVersionTag}/{var:nitroPathToPrecompiles}/) and create a new `ArbHi.go` (introduced in Option 2).
