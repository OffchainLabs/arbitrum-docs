...
- The `machine.v2.wavm.br` binary is a compressed binary containing the Go replay binary and all its dependencies, compiled to WASM, then translated to the Arbitrum fraud proving variant <a data-quicklook-from="wavm">WAVM</a>.
  - It's used by Arbitrator when actually entering a <a data-quicklook-from="challenge">challenge</a> and performing the fraud proofs,
    and has identical behavior to `replay.wasm`.
- The <a data-quicklook-from="wasm-module-root">WASM module root</a> (stored in `module-root.txt`) is a 32 byte hash usually expressed in hexadecimal which is a merkelization of `machine.v2.wavm.br`.
