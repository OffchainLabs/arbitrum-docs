...
- The STF must not carry state between blocks outside of the "global state". In practice this means persistent state must be stored within block headers or the Ethereum state trie. For instance, [ArbOS](/docs/how-arbitrum-works/deep-dives/arbos) stores all retryables in contract storage under a special ArbOS address.
...
Next, make your changes to the State Transition Function. For example, you could [add a custom precompile](/docs/launch-arbitrum-chain/configuration/core/customize-precompile).

After this step, you should visit [Customize ArbOS version](/docs/launch-arbitrum-chain/configuration/core/customize-arbos) to see if your changes need to upgrade the ArbOS version. If so, please continue to
...
- The `machine.v2.wavm.br` binary is a compressed binary containing the Go replay binary and all its dependencies, compiled to WASM, then translated to the Arbitrum fraud proving variant <Term id="wavm">WAVM</Term>.
  - It's used by Arbitrator when actually entering a <Term id="challenge">challenge</Term> and performing the fraud proofs,
