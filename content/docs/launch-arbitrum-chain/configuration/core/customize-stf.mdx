...
After this step, visit [Customize ArbOS version](./customize-arbos) to see if your changes need to upgrade the ArbOS version. If so, follow that guide to add ArbOS upgrade logic to your code.
...
For local testing before the matching WASM module root is set, run a node with staking disabled (`--node.staker.enable=false`). Do not use `--node.staker.dangerous.without-block-validator` to operate a validator without block validation.

</VanillaAdmonition>

You have two ways of running your local node.
...
The instructions provided in [How to run a full node](/docs/run-a-node/run-full-node) **will not** work with your Arbitrum chain node. See [Optional parameters (Arbitrum chain)](/docs/run-a-node/run-full-node#optional-parameters) for Arbitrum chain-specific CLI flags.
...
- The `machine.v2.wavm.br` binary is a compressed binary containing the Go replay binary and all its dependencies, compiled to WASM, then translated to the Arbitrum fraud proving variant WAVM.
...
- The <Term id="wasm-module-root">WASM module root</Term> (stored in `module-root.txt`) is a 32 byte hash usually expressed in hexadecimal which is a merkelization of `machine.v2.wavm.br`.
...
Once you have set the new WASM module root onchain, the validator will recognize blocks produced by your customized STF. You can now run your node with block validation enabled.
