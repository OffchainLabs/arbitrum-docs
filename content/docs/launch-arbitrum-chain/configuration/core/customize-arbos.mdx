...
Customizing the [ArbOS](/docs/how-arbitrum-works/deep-dives/arbos) version involves modifying the Nitro codebase to introduce a new, version-controlled iteration of ArbOS (Arbitrum's operating system-like layer). This lets you activate custom behaviors, features, or state changes while maintaining backward compatibility and determinism for fraud proofs.
...
After you add a new precompile named `ArbHi` according to the guide in [customize precompile Option 2](./customize-precompile#option-2-create-a-new-precompile) and make changes to [`precompile.go`](https://github.com/OffchainLabs/@@nitroRepositorySlug@@/blob/@@nitroVersionTag@@/@@nitroPathToPrecompiles@@/precompile.go),
...
After you add a new state `myNumber` according to the guide in [customize precompile Option 5](./customize-precompile#option-5-call-and-modify-state), you also need to rewrite [`UpgradeArbosVersion`](https://github.com/OffchainLabs/@@nitroRepositorySlug@@/blob/v3.1.0/arbos/@@nitroPathToArbosState@@/arbosstate.go#L253) in [`arbosstate.go`](https://github.com/OffchainLabs/@@nitroRepositorySlug@@/blob/v3.1.0/@@nitroPathToArbosState@@/arbosstate.go):
...
It should be noted that when you initialize the state (initial code is in [customize precompile Option 5](./customize-precompile#option-5-call-and-modify-state)), you need to initialize it to `0` or `null` value first to avoid a potential re-org on your chain.
...
Please continue reading [Customize STF](./customize-stf#step-3-run-the-node) for follow-up operations.
