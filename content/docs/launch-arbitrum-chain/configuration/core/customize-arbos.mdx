...
Customizing the [ArbOS](/docs/how-arbitrum-works/deep-dives/arbos) version involves modifying the Nitro codebase to introduce a new, version-controlled iteration of ArbOS (<a data-quicklook-from="arbitrum">Arbitrum</a>'s operating system-like layer). This lets you activate custom behaviors, features, or state changes while maintaining backward compatibility and determinism for fraud proofs.
...
1. When you want to make changes to your Nitro code that affect the State Transition Function, or STF (you may refer to the [Customize STF docs](/docs/launch-arbitrum-chain/configuration/core/customize-stf#introduction)), and
...
After you add `sayHi()` to `ArbSys.go` according to the guide in [customize precompile Option 1](/docs/launch-arbitrum-chain/configuration/core/customize-precompile#option-1-add-new-methods-to-an-existing-precompile),
you need to continue to modify [precompile.go](https://github.com/OffchainLabs/@@nitroRepositorySlug=nitro@@/blob/@@nitroVersionTag=v3.11.3@@/@@nitroPathToPrecompiles=precompiles@@/precompile.go).
...
After you add a new precompile named `ArbHi` according to the guide in [customize precompile Option 2](/docs/launch-arbitrum-chain/configuration/core/customize-precompile#option-2-create-a-new-precompile) and make changes to [`precompile.go`](https://github.com/OffchainLabs/@@nitroRepositorySlug=nitro@@/blob/@@nitroVersionTag=v3.11.3@@/@@nitroPathToPrecompiles=precompiles@@/precompile.go),
...
It should be noted that when you initialize the state (initial code is in [customize precompile Option 5](/docs/launch-arbitrum-chain/configuration/core/customize-precompile#option-5-call-and-modify-state)), you need to initialize it to `0` or `null` value first to avoid a potential re-org on your chain.
...
To prevent this, if you are using an external call to the precompile contract to change the value, you can refer to [point 1](/docs/launch-arbitrum-chain/configuration/core/customize-arbos#1-add-a-new-method-to-existing-precompile-on-a-specific-arbos-version) or
[point 2](/docs/launch-arbitrum-chain/configuration/core/customize-arbos#2-create-a-new-precompile-contract-on-a-specific-arbos-version) to set the activation time of the precompile contract method.
If your nitro code needs to call this method to change the state, you can continue reading [Step 4](/docs/launch-arbitrum-chain/configuration/core/customize-arbos#4-any-changes-in-the-stf-logic-that-will-affect-the-final-execution-result-arbos-version-control).
...
In the above code, we use precompiles as an example. Some logic might also affect the STF, such as the methods in [`block_process.go`](https://github.com/OffchainLabs/@@nitroRepositorySlug=nitro@@/blob/@@nitroVersionTag=v3.11.3@@/@@nitroPathToArbos=arbos@@/block_processor.go), [`internal_tx.go`](https://github.com/OffchainLabs/@@nitroRepositorySlug=nitro@@/blob/@@nitroVersionTag=v3.11.3@@/@@nitroPathToArbos=arbos@@/internal_tx.go), [`tx_processor.go`](https://github.com/OffchainLabs/@@nitroRepositorySlug=nitro@@/blob/@@nitroVersionTag=v3.11.3@@/@@nitroPathToArbos=arbos@@/tx_processor.go) and so on.
...
Please continue reading [Customize STF](/docs/launch-arbitrum-chain/configuration/core/customize-stf#step-3-run-the-node) for follow-up operations.
