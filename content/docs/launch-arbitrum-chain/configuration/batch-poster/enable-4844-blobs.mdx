...
Your [<Term id="arbos">ArbOS</Term>](/docs/how-arbitrum-works/deep-dives/arbos) version must be **20 or higher**. To check your current version:

#### Method 1: Smart contract call

Call the `arbOSVersion()` function on the ArbSys [<Term id="precompile">precompile</Term>](/docs/arbitrum-essentials/precompiles/reference) contract:
...
To enable blob <Term id="transaction">transaction</Term> posting, add the following configuration to your [<Term id="batch-poster">Batch Poster</Term>](/docs/how-arbitrum-works/deep-dives/batchposter) node:
