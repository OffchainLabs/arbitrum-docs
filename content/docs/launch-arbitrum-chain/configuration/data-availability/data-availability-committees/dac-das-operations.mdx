...
Batch posters are authorized addresses on the `SequencerInbox` that can post <Term id="transaction">transaction</Term> data. They are configured at rollup creation and managed via `setIsBatchPoster`. During normal AnyTrust operation, the batch poster posts compact DACerts. During fallback, it posts the full calldata.
