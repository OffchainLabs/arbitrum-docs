...
For pure Rollup chains, enabling this feature effectively introduces a committee trust layer, making it less "trust-minimized" than standard mode. Once enabled, the chain processes a potential backlog of unconfirmed assertions before operating at full speed. Confirmations may be slightly delayed under high load (e.g., >1 Mgas/s throughput), but security remains intact.

## Recommended configuration

While any Arbitrum chain can adopt Fast Withdrawals, we only _recommend_ that fast withdrawals be adopted by [AnyTrust](/docs/how-arbitrum-works/deep-dives/anytrust-protocol) chains with a minimum <Term id="validator">validator</Term> and DAC member requirement. We explain both these recommendations below:
...
We recommend that any Fast Withdrawals-enabled chain have at least three DAC members and three validators acting in the fast withdrawals committee. Given that fast withdrawals will enable confirmation of new Rollup state much faster than the usual 6.4-day challenge period (roughly 30–40 minutes for BoLD-enabled L2s and L3s, and 15 seconds for pre-BoLD L3s), it becomes even more important to have additional parties involved in validation to further reduce trust assumptions. This requirement can be be met with three total operators, who each run a single DAS node and a single validator.
...
- **BoLD enabled chains**
  - For all L3s utilizing BoLD, the practical lower bound for fast withdrawals is **two rounds of parent chain finality** — roughly **30–40 minutes** for chains settling to Ethereum. BoLD validators by default only act on finalized parent chain state, so a validator waits for parent chain finality twice: once for the assertion it is confirming, and again for its own `approveHash` signature on the fast confirmation multisig. Each round is ~15 minutes for chains settling to Ethereum.
...
For an Ethereum-based Layer-2, we recommend that the fast withdrawal frequency remain above 12.8 minutes, which is the time for Ethereum to achieve finality. For non-Ethereum L1s, we similarly recommend staying above the accepted finality threshold specific to that <Term id="L1">L1</Term>.

For an <Term id="arbitrum-one">Arbitrum One</Term>-based Layer-3, there are three tiers of finality to consider:

1. Soft finality from the [Sequencer](/docs/how-arbitrum-works/deep-dives/sequencer)'s confirmation of <Term id="transaction">transaction</Term> inclusion (~<Var name="l2BlockTimeMs" />ms)
2. Safe finality from <Term id="batch">batch</Term> inclusion after Arbitrum One's [assertion](/docs/how-arbitrum-works/deep-dives/assertions) is included in an Ethereum block.
...
| `MINIMUM_ASSERTION_PERIOD` | Optional parameter that defaults to 75 blocks (~15 minutes). Minimum number of blocks that have to pass in between assertions (measured in block number of the first non-Arbitrum ancestor chain (i.e., Ethereum blocks for <Term id="L2">L2</Term> chains and L3 chains settling to an Arbitrum chain, and parent chain blocks for chains settling to a non-Arbitrum L2 chain)). |
