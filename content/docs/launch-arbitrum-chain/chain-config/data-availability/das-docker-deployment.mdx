...
[How to deploy a DAS](/docs/launch-arbitrum-chain/chain-config/data-availability/deploy-das) covers the same deployment using Kubernetes, and explains in depth how a DAS works and which configuration options are available. This page doesn't repeat that conceptual content; when in doubt about an option, refer to that guide.
...
For production setups that need self-healing, rolling updates, or multiple replicas behind a load balancer, prefer the [Kubernetes-based guide](/docs/launch-arbitrum-chain/chain-config/data-availability/deploy-das) or the [Helm chart](https://artifacthub.io/packages/helm/offchainlabshelm/das).
...
<Callout type="warning" title="Binary names changed in Nitro v3.10.0">
...
- If you wish to configure a REST aggregator for your DAS, the URL where the list of REST endpoints is kept. Refer to [State synchronization](/docs/launch-arbitrum-chain/chain-config/data-availability/deploy-das#state-synchronization) for more information.

The hardware requirements are the same as for a Kubernetes deployment: a single CPU and 1 GiB of RAM handle normal DAS duties comfortably. See [Hardware requirements](/docs/launch-arbitrum-chain/chain-config/data-availability/deploy-das#hardware-requirements) for details, including guidance for mirrors and CDNs.
...
<Callout type="warning" title="Create mounted directories before starting the container">
...
- Your chain's nodes retrieve data from the **REST interface** (port `9877`), which is safe to expose publicly — ideally behind a CDN, or served by a [mirror DAS](/docs/launch-arbitrum-chain/chain-config/data-availability/deploy-mirror-das) instead of your main DAS.
...
<Callout type="info" title="Approximate costs (as of July 2026)">
...
The chain owner then follows [How to configure the DAC in your chain](/docs/launch-arbitrum-chain/chain-config/data-availability/configure-dac) to generate and register the keyset.
