...
In this how-to, you'll learn how to configure the DAC in your chain. Refer to the [Introduction](/docs/launch-arbitrum-chain/chain-config/data-availability/dac-get-started) for the full process of running DA servers and configuring the chain.
...
- [How to deploy a Data Availability Server (DAS)](/docs/launch-arbitrum-chain/chain-config/data-availability/deploy-das). This is needed to understand where the data we'll be handling in this guide comes from.
...
You should also make sure that at least one DAS is running as an [archive DAS](/docs/launch-arbitrum-chain/chain-config/data-availability/deploy-das#archive-da-servers), otherwise the information will not be available after the expiry time.
...
```shell
docker run -v $(pwd):/data/keyset --entrypoint anytrusttool offchainlabs/nitro-node:v3.11.4-7d5ac27 dumpkeyset --conf.file /data/keyset/keyset-info.json
```
