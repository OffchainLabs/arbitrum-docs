...
The `RewardDistributor` can be _permissionlessly_ called to perform a withdrawal, which simultaneously transfers 90% of accrued fees to the chain’s fee collector and <Var name="aepRevenueSharePercent" />% of accrued fees to a target address on the <Term id="parent-chain">parent chain</Term>. From here, the chain owner has complete control over their earned fees, and the routing contracts can direct AEP fees to a collecting address for the Arbitrum DAO.
...
- Deploying a `RewardDistributor` contract configured to forward <Var name="aepRevenueSharePercent" />% of fees to the `ChildToParentRouter` and 90% to the chain owner’s preferred reward-receiving address.
...
<Callout type="info" title="Please reach out if:">
    1. Your chain is on another <Term id="L2">L2</Term> for which we do not have a `ChildToParent` router deployed
    2. Your custom gas token is only available on the L2 and non-EVM <Term id="L1">L1</Term>
...
<Callout type="info" title="Note for L3 chains with custom gas tokens">
