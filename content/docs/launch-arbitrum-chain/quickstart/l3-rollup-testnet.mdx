...
<Callout type="idea" title="Run all steps in the same terminal">
...
| :--------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Deployed chain**     | Complete [Deploy your first rollup](/docs/launch-arbitrum-chain/quickstart/l3-rollup-from-scratch), Steps 1–4 (deploy, generate config, fund batch poster and validator), **do not complete Step 5** |
...
<Callout type="warning" title="Set sequencer priority after deployment">
...
<Callout type="warning" title="Do not add batch poster to sequencer priority">
...
The validator validates blocks and posts [assertions](/docs/how-arbitrum-works/deep-dives/assertions) to the parent chain. It is required for chain security. The validator wallet needs **ETH** on the parent chain (Arbitrum Sepolia) for gas.
...
<Callout type="warning" title="Required for sequencer activation">
...
<Callout type="idea" title="Add all sequencers to priority list">
...
For running production mainnet, consider a RaaS provider. See the [list of RaaSes on the Third-party providers page](/docs/launch-arbitrum-chain/integrations/infrastructure-providers#rollup-as-a-service-raas-providers).
