...
- If you fail to add this key, or do not enter it correctly, it will fail with: `Error: private key must be 32 bytes, hex or bigint, not string`
...
The batch poster and validator need **ETH** on Arbitrum Sepolia to pay for gas when posting [batches and assertions](/docs/how-arbitrum-works/deep-dives/assertions). Send **ETH** to both addresses from your deployer wallet or a [faucet](https://arbitrum.faucet.dev/).
...
<Accordions>
<Accordion title="Optional: redeem-ticket.mjs">
...
eth_blockNumber advances; RPC reachable at http://localhost:8547
```
