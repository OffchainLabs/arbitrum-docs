...
This guide covers the operational sequencing of a <Term id="bold">BoLD</Term> upgrade for chains where the `execute` call is performed by a multisig or a security council rather than a single <Term id="chain-owner">chain owner</Term> key. It continues from the step-by-step upgrade in [BoLD for Arbitrum chains](/docs/launch-arbitrum-chain/chain-config/validation/bold) and assumes you have read it. Concepts, the full parameter table, and the base upgrade steps are not repeated here.

If your chain owner is a single key that can send the upgrade <Term id="transaction">transaction</Term> immediately, you do not need this guide—follow the [base guide](/docs/launch-arbitrum-chain/chain-config/validation/bold) instead.

## The problem this guide solves

The [base guide](/docs/launch-arbitrum-chain/chain-config/validation/bold) sequence contains two steps that appear to conflict when a security council is involved:
...
What `perform` actually requires is narrower than the [base guide](/docs/launch-arbitrum-chain/chain-config/validation/bold) implies. Confirming another assertion after Step 4 is fine—what matters is that the lookup must contain an entry for whichever assertion is latest-confirmed **at the moment of execution**.
...
Follow Steps 0 through 3 of the [base upgrade guide](/docs/launch-arbitrum-chain/chain-config/validation/bold#step-0-pre-requisites) without changes. Validators keep running. This deploys the action contract with your chain's configuration and fixes the address that the `execute` payload will target.
...
Submit the signed multisig transaction. Then continue with Steps 6 and 7 of the [base guide](/docs/launch-arbitrum-chain/chain-config/validation/bold) to update node configuration, restart nodes, and monitor the new Rollup contract.
...
- If you keep the escape hatch enabled, alert on assertion inactivity well before the window elapses. See the validator section of [Monitoring tools and considerations](/docs/launch-arbitrum-chain/operate/monitoring) for the metrics to watch.
...
To weigh permissioned against <Term id="permissionless-validation">permissionless validation</Term>, see [BoLD for Arbitrum chains](/docs/launch-arbitrum-chain/chain-config/validation/bold#caveats-that-come-with-adopting-arbitrum-bold-for-permissionless-validation). It covers bond sizing and resource exhaustion risk.
...
<Callout type="info" title="Why operators think BoLD chains bond in ETH">
...
For guidance on choosing `stakeToken` and `stakeAmt` values, see [Bond and validator configurations](/docs/launch-arbitrum-chain/chain-config/validation/bond-and-validator) and [Economics of disputes](/docs/how-arbitrum-works/bold/bold-economics-of-disputes).
