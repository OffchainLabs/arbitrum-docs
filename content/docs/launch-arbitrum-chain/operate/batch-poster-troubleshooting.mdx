...
The [Monitoring tools and considerations](/docs/launch-arbitrum-chain/operate/monitoring) guide recommends monitoring the batch poster balance. In practice, this means:
...
<Callout type="info" title="Balance threshold guidance">
...
### "Posting this transaction will exceed max mempool size" [#exceeds-max-mempool-size]
...
<Callout type="info" title="Mempool size limits">
...
#### `ErrExceedsMaxMempoolSize` → "error posting batch" [#err-exceeds-max-mempool-size]
...
- [CAUSE]—<Term id="L1">L1</Term> is confirming the poster's transactions slowly, and the in-flight window (default: 18) is filled.
...
#### `lack of L1 balance prevents posting transaction with desired fee cap` [#lack-of-l1-balance]
...
#### `a large batch posting backlog exists` [#large-backlog-exists]
...
#### `error fetching batch poster wallet balance` / `...gas refunder balance` [#error-fetching-wallet-balance]
...
- **Check the sequencer's transaction backlog size**: As noted in the [Monitoring tools and considerations](/docs/launch-arbitrum-chain/operate/monitoring) guide, a large and growing backlog can be a sign of network health issues.
...
<Callout type="info" title="Backlog metrics">
...
2. **Check the parent chain gas prices**: If prices exceed your fee cap, either wait for prices to drop or increase `--node.batch-poster.data-poster.target-price-gwei` (see [Batch poster fee tuning](/docs/launch-arbitrum-chain/chain-config/batch-poster/fee-tuning)).
...
### "Failed to re-send transaction" [#failed-to-resend-transaction]
...
<Callout type="info" title="Error context">
...
#### `Large gap between last seen and current block number, skipping check for reverts` [#large-gap-skipping-revert-check]
...
#### `Transaction from batch poster reverted` [#poster-transaction-reverted]
...
- [ACTION]—Investigate `txErr`; restart clears the halt flag; a count mismatch after <Term id="forceinclusion">force-inclusion</Term> may need `allow-posting-first-batch-when-sequencer-message-count-mismatch`.

#### `Error checking batch reverts` [#error-checking-batch-reverts]
...
#### `failed to update nonce with queue empty; falling back to using a recent block` [#nonce-update-queue-empty]
...
#### `maybeLogError` family—`failed to replace-by-fee transaction` / `failed to re-send transaction` [#maybelogerror-family]
...
#### `can't meet data poster fee cap obligations with current target max cost` / `can't meet current parent chain fees with current target max cost` [#cannot-meet-fee-cap]
...
#### `submitting transaction with GasFeeCap less than latest basefee` / `...BlobGasFeeCap less than latest blobfee` [#gasfeecap-below-basefee]
...
#### `unable to fetch suggestedTipCap from l1 client to update arb/batchposter/suggestedtipcap metric` [#suggestedtipcap-unavailable]
...
#### `Disabling batch posting due to batch being within reorg resistance margin from layer 1 minimum block or timestamp bounds` [#reorg-resistance-margin-block]
...
- [CAUSE]—The margin guard (default 10m) protects against <Term id="reorg">reorgs</Term> near the lower bound.
- [ACTION]—Expected safety behavior; set margin to `0` only if you accept the <Term id="reorg">reorg</Term> risk.

#### `disabling L1 bound as batch posting message is close to the maximum delay` [#l1-bound-disabled-max-delay]
...
#### `not posting more messages because block number or timestamp exceed L1 bounds` [#messages-exceed-l1-bounds]
- [INFO]—Stopped adding messages that fall outside the current L1 bound window.
- [ACTION]—Normal bounding behavior.

#### `error getting max time variation on L1 bound block; falling back on latest block` [#max-time-variation-fallback]
The L1 node couldn't give state at the finalized bound block, so it falls back to the latest block—usually transient.

#### `unknown L1 block bound config value; falling back on using finalized` [#unknown-l1-block-bound]
...
#### `DataPoster is avoiding creating a mempool nonce gap` [#avoiding-nonce-gap]
...
#### `DA writer failed, operator action required` [#da-writer-failed]
...
#### `DA writer explicitly requested fallback` [#da-writer-fallback]
...
<Callout type="info" title="Related info">
...
#### `Error checking if we could acquire redis lock` [#redis-lock-check-failed]
...
#### `Not posting batches right now because another batch poster has the lock or this node is behind` [#another-poster-holds-lock]
- [DEBUG]—Normal on backup posters/when behind.
- [ACTION]—Expected; not an error.

#### `Failed to estimate gas for EIP-7623 check 1/2` [#eip-7623-gas-estimate-failed]
- [WARN]—An EIP-7623 calldata-cost estimation probe failed.
- [ACTION]—Usually transient; relevant only on EIP-7623 parent chains.

#### `error estimating gas for batch` [#error-estimating-batch-gas]
...
#### `max-size is deprecated; use max-calldata-batch-size...` [#max-size-deprecated]
- [ERROR]—Deprecated flag in use.
- [ACTION]—Migrate to `max-calldata-batch-size`.

#### `Disabling data poster storage, as parent chain appears to be an Arbitrum chain without a mempool` [#data-poster-storage-disabled]
- [INFO]—Auto-switched to no-op storage on an Arbitrum parent.
- [ACTION]—Expected for L3s.

#### `messagesPerBatch is somehow zero` [#messages-per-batch-zero]
