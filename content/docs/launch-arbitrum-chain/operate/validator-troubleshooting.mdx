...
| `Defensive`    | Acts only when it finds a bad <Term id="assertion">assertion</Term>: it bonds and posts the correct rival assertion. On a BoLD chain it does not open the challenge itself — that requires `ResolveNodes` or `MakeNodes`. Idle in normal operation, so an absence of onchain activity is not a fault signal.                                                                                |
