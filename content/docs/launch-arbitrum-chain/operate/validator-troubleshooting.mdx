...
| `Defensive`    | Acts only when it finds a bad <Term id="assertion">assertion</Term>: it bonds and posts the correct rival assertion. On a BoLD chain it does not open the <Term id="challenge">challenge</Term> itself — that requires `ResolveNodes` or `MakeNodes`. Idle in normal operation, so an absence of onchain activity is not a fault signal.                                        |
...
<Callout type="info" title="Switching strategy does not re-bond">
...
<Callout type="warning" title="Multiple MakeNodes validators produce expected reverts">
...
For the full flag list, see the [Nitro CLI flags reference](/docs/run-a-node/nitro/cli-flags-reference). For the onchain parameters, see the parameter table in [BoLD for Arbitrum chains](/docs/launch-arbitrum-chain/chain-config/validation/bold#table-of-arbitrum-bold-parameters).
...
<Callout type="warning" title="The staker queues only one transaction by default">
...
Assertion-progress and failure signals, including which metrics apply to the legacy staker versus the BoLD staker and the `validatorAfkBlocks` allowlist caution, are documented in the validator section of [Monitoring tools and considerations](/docs/launch-arbitrum-chain/operate/monitoring).
