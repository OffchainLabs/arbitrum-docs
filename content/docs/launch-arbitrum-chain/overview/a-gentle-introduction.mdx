...
<VanillaAdmonition type="note">
  The 100ms figure is an optional lower bound on block time, not the default. Most chains run at the @@l2BlockTimeMs=250@@ms default; 100ms is available when you opt into it. By default, <Term id="fast-withdrawals">fast withdrawals</Term> is not enabled—the default withdrawal time is @@arbOneDisputeWindowDays=6.4@@ days.
</VanillaAdmonition>
