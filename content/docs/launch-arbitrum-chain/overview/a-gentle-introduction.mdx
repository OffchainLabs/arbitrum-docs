...
<VanillaAdmonition type="note">
  The 100ms figure is an optional lower bound on block time, not the default. Most chains run at the <Var name="l2BlockTimeMs" />
  ms default; 100ms is available when you opt into it. By default, <Term id="fast-withdrawals">fast withdrawals</Term> is not enabled—the default withdrawal time is <Var name="arbOneDisputeWindowDays" /> days.
</VanillaAdmonition>
