...
To support these additional use cases, follow the instructions described in [How to customize your Arbitrum chain's behavior](/docs/launch-arbitrum-chain/extend-the-protocol/stf).

</Callout>

There are five primary ways to customize your chain's precompiles:

1.  Add new methods to an existing [precompile](https://github.com/OffchainLabs/@@nitroRepositorySlug@@/blob/@@nitroVersionTag@@/@@nitroPathToPrecompiles@@/) (local: `/precompiles`)
...
```shell
git clone --branch v3.11.4 <https://github.com/OffchainLabs/nitro.git>
cd nitro
...
Using your favorite code editor, open an existing precompile from the [precompiles implementation](https://github.com/OffchainLabs/@@nitroRepositorySlug@@/blob/@@nitroVersionTag@@/@@nitroPathToPrecompiles@@/) directory (local `/precompiles`). We'll use `ArbSys.go` as an example. Open the corresponding Go implementation file (`ArbSys.go`) and add a simple `SayHi` method:
...
Next, follow the steps in [How to customize your Arbitrum chain's behavior](/docs/launch-arbitrum-chain/extend-the-protocol/stf#step-3-run-the-node) to build a modified Arbitrum Nitro node Docker image and run it.
...
Then, update [precompile.go](https://github.com/OffchainLabs/@@nitroRepositorySlug@@/blob/@@nitroVersionTag@@/@@nitroPathToPrecompiles@@/precompile.go) to register the new precompile under the `Precompiles()` method:
...
```shell
docker run --rm -it  -v /some/local/dir/arbitrum:/home/user/.arbitrum -p 0.0.0.0:8547:8547 -p 0.0.0.0:8548:8548 offchainlabs/nitro-node:v3.11.4-7d5ac27 --parent-chain.connection.url=<YourParentChainUrl> --chain.id=<YourOrbitChainId> --http.api=net,web3,eth,debug --http.corsdomain=* --http.addr=0.0.0.0 --http.vhosts=*
```
...
First, open the [arbosstate.go](https://github.com/OffchainLabs/@@nitroRepositorySlug@@/blob/@@nitroVersionTag@@/@@nitroPathToArbosState@@/arbosstate.go) file and locate the [ArbosState](https://github.com/OffchainLabs/nitro/blob/v2.2.0/arbos/arbosState/arbosstate.go#L38) structure. This is where ArbOS state is defined.

Define a state key called `myNumber` of type `storage.StorageBackedUint64`. You can find more types in [storage.go](https://github.com/OffchainLabs/@@nitroRepositorySlug@@/blob/@@nitroVersionTag@@/@@nitroPathToStorage@@/storage.go):
...
Next, head back to the [precompiles directory](https://github.com/OffchainLabs/@@nitroRepositorySlug@@/blob/@@nitroVersionTag@@/@@nitroPathToPrecompiles@@/) and create a new `ArbHi.go` (introduced in Option 2).
...
If you do not customize the precompile before launching your Arbitrum chain network, please continue to follow the instructions on [Customize ArbOS](/docs/launch-arbitrum-chain/extend-the-protocol/arbos) to perform an ArbOS version control to avoid blockchain reorg.
