...
- **Fraud-proof compatible**: Full support for [BoLD](/docs/how-arbitrum-works/bold/gentle-introduction) <Term id="challenge">challenge</Term> protocol and fraud proofs
...
- **Normal execution**: [Batch Poster](/docs/how-arbitrum-works/deep-dives/assertions) stores data -> generates certificate -> posts to <Term id="L1">L1</Term>
...
**Prerequisites**: Familiarity with JSON-RPC, Solidity, Ethereum L1/<Term id="L2">L2</Term> architecture, and Arbitrum Nitro basics. Experience with any backend programming language.
...
Returns the header byte that identifies your DA provider in [Sequencer](/docs/how-arbitrum-works/deep-dives/sequencer) messages.
...
`--node.da.external-provider.use-data-streaming`
...
- `--node.da.external-provider.data-stream.rpc-methods.start-stream`
- `--node.da.external-provider.data-stream.rpc-methods.stream-chunk`
- `--node.da.external-provider.data-stream.rpc-methods.finalize-stream`

<Callout type="info" title="Universal Integer Encoding Standard">
...
- **Limit Control:** Use the following flag on the Nitro node:
  `--node.da.external-provider.data-stream.max-store-chunk-body-size`
