...
Customizing the [ArbOS](/docs/how-arbitrum-works/deep-dives/arbos) version involves modifying the Nitro codebase to introduce a new, version-controlled iteration of ArbOS (<Term id="arbitrum">Arbitrum</Term>'s operating system-like layer). This lets you activate custom behaviors, features, or state changes while maintaining backward compatibility and determinism for fraud proofs.

This is an advanced customization primarily for live chains, allowing developers to extend or alter the <Term id="state-transition-function">State Transition Function</Term> (STF)—the core logic for block production and state updates—while ensuring safe, non-disruptive upgrades. Unlike standard upgrades (e.g., to canonical versions like ArbOS 20 "Atlas"), customization involves creating intermediate or project-specific versions (e.g., ArbOS 32, a fork of 31). This lets you incorporate bespoke elements such as new precompiles, EVM opcodes, or state variables without conflicting with official releases.

This customization is recommended for teams with expertise or partners (e.g., Rollup-as-a-Service providers), as it requires audits, maintenance, and careful handling to avoid issues like chain re-orgs or failed fraud proofs.

## Cases where you may want to consider customizing your own ArbOS upgrade

1. When you want to make changes to your Nitro code that affect the State Transition Function, or STF (you may refer to the [Customize STF docs](/docs/launch-arbitrum-chain/extend-the-protocol/stf#introduction)), and
...
After you add `sayHi()` to `ArbSys.go` according to the guide in [customize precompile Option 1](/docs/launch-arbitrum-chain/extend-the-protocol/precompiles#option-1-add-new-methods-to-an-existing-precompile),
you need to continue to modify [precompile.go](https://github.com/OffchainLabs/@@nitroRepositorySlug@@/blob/@@nitroVersionTag@@/@@nitroPathToPrecompiles@@/precompile.go).
...
After you add a new precompile named `ArbHi` according to the guide in [customize precompile Option 2](/docs/launch-arbitrum-chain/extend-the-protocol/precompiles#option-2-create-a-new-precompile) and make changes to [`precompile.go`](https://github.com/OffchainLabs/@@nitroRepositorySlug@@/blob/@@nitroVersionTag@@/@@nitroPathToPrecompiles@@/precompile.go),
...
After you add a new state `myNumber` according to the guide in [customize precompile Option 5](/docs/launch-arbitrum-chain/extend-the-protocol/precompiles#option-5-call-and-modify-state), you also need to rewrite [`UpgradeArbosVersion`](https://github.com/OffchainLabs/@@nitroRepositorySlug@@/blob/v3.1.0/arbos/@@nitroPathToArbosState@@/arbosstate.go#L253) in [`arbosstate.go`](https://github.com/OffchainLabs/@@nitroRepositorySlug@@/blob/v3.1.0/@@nitroPathToArbosState@@/arbosstate.go):
...
It should be noted that when you initialize the state (initial code is in [customize precompile Option 5](/docs/launch-arbitrum-chain/extend-the-protocol/precompiles#option-5-call-and-modify-state)), you need to initialize it to `0` or `null` value first to avoid a potential re-org on your chain.

</Callout>

Also, please make sure your program cannot call the `state.SetNewMyNumber` or other functions that may change the value of `myNumber` before ArbOS v21.
To prevent this, if you are using an external call to the precompile contract to change the value, you can refer to [point 1](/docs/launch-arbitrum-chain/extend-the-protocol/arbos#1-add-a-new-method-to-existing-precompile-on-a-specific-arbos-version) or
[point 2](/docs/launch-arbitrum-chain/extend-the-protocol/arbos#2-create-a-new-precompile-contract-on-a-specific-arbos-version) to set the activation time of the precompile contract method.
If your nitro code needs to call this method to change the state, you can continue reading [Step 4](/docs/launch-arbitrum-chain/extend-the-protocol/arbos#4-any-changes-in-the-stf-logic-that-will-affect-the-final-execution-result-arbos-version-control).
...
In the above code, we use precompiles as an example. Some logic might also affect the STF, such as the methods in [`block_process.go`](https://github.com/OffchainLabs/@@nitroRepositorySlug@@/blob/@@nitroVersionTag@@/@@nitroPathToArbos@@/block_processor.go), [`internal_tx.go`](https://github.com/OffchainLabs/@@nitroRepositorySlug@@/blob/@@nitroVersionTag@@/@@nitroPathToArbos@@/internal_tx.go), [`tx_processor.go`](https://github.com/OffchainLabs/@@nitroRepositorySlug@@/blob/@@nitroVersionTag@@/@@nitroPathToArbos@@/tx_processor.go) and so on.
The aforementioned ArbOS control methods will be needed for interacting with different versions of the STF logic.

<Callout type="idea" title="Backward compatibility">
...
Please continue reading [Customize STF](/docs/launch-arbitrum-chain/extend-the-protocol/stf#step-3-run-the-node) for follow-up operations.
