...
Before the Nitro upgrade, <a data-quicklook-from="arbitrum-one">Arbitrum One</a> ran on the <a data-quicklook-from="arbitrum-classic">Classic</a> stack for a year (before block height 22207817). Although the Nitro chain uses the latest snapshot of the Classic chain's state as its genesis state, **the Nitro stack can serve all but six RPC requests for pre-Nitro blocks. Full details are outlined in [Do you need to run a Classic node?](/docs/run-a-node/more-types/run-classic-node#do-you-need-to-run-a-classic-node).**
