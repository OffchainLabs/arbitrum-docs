...
This page covers the standard sequencer feed, which publishes complete blocks and is free to consume. Arbitrum One also offers the <a data-quicklook-from="fast-feed">Fast Feed</a>, a paid stream that publishes individual transactions before their block closes. See the [introduction to the Fast Feed](/how-arbitrum-works/priority-gas-auction/fast-feed) to compare the two.
...
The `messages[0].message.message.header.blockNumber` is `L1 block number` instead of `L2 block number`
