...
The Sequencer Coordinator Manager (SQM) is a command-line tool that allows you to manage the priority list of <Term id="sequencer">sequencers</Term>, update their positions, add new sequencers to the list, and refresh the lists from the Redis server.
...
- Before starting the Sequencer Coordinator Manager, please read and follow the commands in Step 4 - Step 7 of [Build Nitro's binaries natively](/docs/run-a-node/nitro/build-nitro-locally#build-nitros-binaries-natively) to install the necessary dependencies.
...
<Callout type="info">One of the sequencers is marked with a `chosen` indicator. This visual cue helps you identify the current chosen sequencer.</Callout>
