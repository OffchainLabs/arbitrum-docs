...
The Nitro Docker images bundle multiple CLI binaries that serve different roles in the <Term id="arbitrum">Arbitrum</Term> ecosystem. This page documents the image variants, the binaries they contain, and how to override entrypoints to run specific tools.
...
```shell
docker run @@latestNitroNodeImage=offchainlabs/nitro-node:v3.11.3-beb2108@@-validator \
...
The following `docker-compose.yml` runs an <Term id="arbitrum-one">Arbitrum One</Term> full node. Replace the placeholder URLs with your actual <Term id="parent-chain">parent chain</Term> and beacon endpoints.
