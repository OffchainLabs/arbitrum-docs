...
<VanillaAdmonition type="tip">Because environment variables have the highest precedence, they are the safest way to inject secrets or per-environment overrides without modifying config files.</VanillaAdmonition>
## Config file format
...
```shell
docker run --rm offchainlabs/nitro-node:v3.11.4-7d5ac27 \
  --conf.dump \
