...
When running a Nitro node for the first time on a chain that produced [classic blocks](/docs/arbitrum-essentials/public-chains#classic-deprecated) in the past (like <Term id="arbitrum-one">Arbitrum One</Term>), you need to initialize its database to, at least, the state of the chain after executing the last classic block. The common, and recommended, way of doing that is to provide a <Term id="database-snapshot">database snapshot</Term> using the `--init.url` option (as mentioned in [How to run a full node (Nitro)](/docs/run-a-node/run-full-node)). In this how-to we show you an alternative way for doing that, migrating the state and history of the chain from a fully synced classic node.

<VanillaAdmonition type="info" title="Is this How-to for you?">
...
Keep in mind that this process only applies to Arbitrum One. Other <Term id="arbitrum-chains">Arbitrum chains</Term> didn't produce <Term id="arbitrum-classic">classic</Term> blocks in the past, they started as Nitro chains.
...
- A fully synced classic node: you can find instructions on how to run a classic node in [this page](/docs/run-a-node/more-types/run-classic-node).
- A clean, uninitialized Nitro node: you can find instructions on how to set up a Nitro node in [this page](/docs/run-a-node/run-full-node).
...
<VanillaAdmonition type="caution" title="Caution">

Enabling the export options is only recommended for nodes with no public/external interfaces.

</VanillaAdmonition>

<VanillaAdmonition type="info" title="Exported file contents are not deterministic">
...
- Call the RPC method `arb_exportOutbox` with parameter `"0xffffffffffffffff"` to initiate <Term id="outbox">outbox</Term> export. It will return immediately.
...
- [How to run a full node (Nitro)](/docs/run-a-node/run-full-node)
- [How to run a full node (Classic, pre-Nitro)](/docs/run-a-node/more-types/run-classic-node)
