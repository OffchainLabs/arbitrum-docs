...
Your node's `--execution.caching.state-scheme` must match the snapshot's `state_scheme`. See [Choose a state scheme](/docs/run-a-node/run-full-node#choose-a-state-scheme).
