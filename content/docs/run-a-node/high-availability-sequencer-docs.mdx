...
This documentation is for production sequencer deployments. If you want to set up a sequencer for testing or on a testnet, see [How to run a testnet sequencer node](/docs/run-a-node/sequencer/run-sequencer-node).

:::

## Introduction

The [sequencer](/docs/how-arbitrum-works/deep-dives/sequencer) is a critical component of your <a data-quicklook-from="arbitrum-chain">Arbitrum chain</a> and is responsible for queuing transactions submitted to the network. It serves as the transaction ordering engine, accepting transactions forwarded from <a data-quicklook-from="arbitrum-full-node">full nodes</a>, queuing them, and returning <a data-quicklook-from="sequencer-feed">feed</a> messages to those full nodes. It subsequently sends queued transactions to <a data-quicklook-from="batch-poster">batch posters</a> for data posting.
...
For the full set of coordinator flags and their defaults, see [Sequencer configuration reference](/docs/launch-arbitrum-chain/configuration/sequencer/sequencer-config-reference#sequencer-coordinator-flags).
...
Nitro treats this key as **read-only**. No sequencer can write itself into it. You populate it with the [Sequencer Coordination Manager (SQM)](/docs/run-a-node/sequencer/run-sequencer-coordination-manager), which is the only component that writes the key.
...
For a full diagnostic path, see [Sequencer troubleshooting](/docs/launch-arbitrum-chain/operate/sequencer-troubleshooting#1-no-sequencer-is-chosen).
...
- Follow detailed instructions at: [How to run a Sequencer Coordination Manager (SQM)](/docs/run-a-node/sequencer/run-sequencer-coordination-manager)
...
- [How to run a Sequencer Coordination Manager (SQM)](/docs/run-a-node/sequencer/run-sequencer-coordination-manager)
