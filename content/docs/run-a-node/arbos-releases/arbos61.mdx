...
This page is intended for all Arbitrum node operators and Arbitrum chain owners; it summarizes the changes brought by <Term id="arbos">ArbOS</Term> 61 "Elara" and what you should do to ensure a seamless upgrade.
...
- ArbOS 61 Elara should technically work with all previous nitro-contract versions, but you may need specific versions if you want to enable optional features for your chain:
  - To take advantage of the new AltDA API (new chains only) or the new base stake management functions for <Term id="bold">BoLD</Term>, you will need [nitro-contracts v3.2.0](https://github.com/OffchainLabs/nitro-contracts/releases/tag/v3.2.0) or higher.
  - To enable [Native Token Mint/Burn capabilities](/docs/run-a-node/arbos-releases/arbos51#native-token-mint-and-burn) for your chain, you must use [nitro-contracts v3.1.1](https://github.com/OffchainLabs/nitro-contracts/releases/tag/v3.1.1) or higher.
...
- On activation through `ArbWasm`, the <Term id="state-transition-function">State Transition Function</Term> reads all fragments through the root contract and concatenates their contents _before_ decompression.
...
ArbOS 61 grants <Term id="offchain-labs">Offchain Labs</Term>, acting as a service provider to the Arbitrum Foundation, on behalf of and for the benefit of the ArbitrumDAO, the ability to modify the `minimumL2BaseFee` and `L2BaseFee` values on Arbitrum One and Nova to any value between 0.01 gwei and 0.10 gwei, inclusive.
...
Turning the feature on takes two steps: enabling tip collection itself, and updating the <Term id="sequencer">sequencer</Term>'s sorting logic to take the `PriorityFee` field into account.
...
For configuration details, see the Priority fees section (page not yet ported to this site).
...
ArbOS 61 introduces a pluggable interface to the Arbitrum node software, allowing AltDA layers, such as EigenDA or Celestia, to integrate with an Arbitrum chain without forking the node software. Once a <Term id="chain-owner">chain owner</Term> integrates it, the API also reduces the technical overhead of maintaining existing Nitro forks used to run Arbitrum chains on AltDA layers.
...
- Any transaction that originates from, targets, or otherwise involves a restricted address is filtered prior to execution, including those submitted through the <Term id="delayed-inbox">Delayed Inbox</Term>.
...
See also the [upgrade notice for ArbOS 61](/docs/notices/arbos61-upgrade-notice).

### Reference links for this revision

- [\[Constitutional\] AIP: ArbOS 61 Elara](https://forum.arbitrum.foundation/t/constitutional-aip-arbos-61-elara/30601), the canonical AIP thread, including the June 19, 2026 update
...
- [Arbitrum DAO network upgrades](https://docs.arbitrum.foundation/network-upgrades), the canonical table of governance approval status and per-chain activation timestamps
- [How to integrate with the DA API](/docs/launch-arbitrum-chain/integrations/da-api-integration-guide), the integration guide for building a custom DA provider against the new API
- Priority fees, how to configure priority fee collection on an Arbitrum chain (page not yet ported to this site)
- [`cargo stylus` overview](/docs/stylus/cli-tools/overview), the CLI that handles fragmented deployment for Stylus contracts above the 24 KB limit
- [Trail of Bits ArbOS 60/61 code review, July 10, 2026](/audit-reports/2026_07_10_trail_of_bits_arbitrum_arbos_60_61_code_review_final_comprehensive_report.pdf)
...
- [ArbOS 51 Dia release notes](/docs/run-a-node/arbos-releases/arbos51)
