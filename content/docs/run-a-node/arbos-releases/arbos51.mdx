...
- Following the [Guide for how to upgrade ArbOS on your Arbitrum chain](../../launch-arbitrum-chain/operate/arbos-upgrade.mdx).
...
- If the Arbitrum chain posts <a data-quicklook-from="blob">blobs</a> to a Fusaka-enabled <a data-quicklook-from="parent-chain">Parent chain</a>, ensure that the consensus layer client is configured correctly as per the [Fusaka upgrade notice](../../notices/fusaka-upgrade-notice.mdx).
...
To read more about this feature, see the [Native Token Mint/Burn enablement guide](/launch-arbitrum-chain/chain-config/costs/configure-native-mint-burn.mdx).
...
Refer to [gas floor per token guide](/launch-arbitrum-chain/chain-config/costs/gas-optimization#gas-floor-per-token) for detailed instructions on how to configure this value via the `ArbOwner` precompile.

### Reference links for ArbOS 51 Dia

- [Guide for how to upgrade ArbOS on your Arbitrum chain](../../launch-arbitrum-chain/operate/arbos-upgrade.mdx)
