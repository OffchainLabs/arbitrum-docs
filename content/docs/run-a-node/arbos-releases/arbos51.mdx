...
- If the Arbitrum chain posts <a data-quicklook-from="blob">blobs</a> to a Fusaka-enabled <a data-quicklook-from="parent-chain">Parent chain</a>, ensure that the consensus layer client is configured correctly as per the [Fusaka upgrade notice](/docs/notices/fusaka-upgrade-notice).
...
To read more about this feature, see the [Native Token Mint/Burn enablement guide](/docs/launch-arbitrum-chain/configuration/costs/configure-native-mint-burn-gas-token).
...
Refer to [gas floor per token guide](/docs/launch-arbitrum-chain/configuration/costs/gas-optimization-tools#gas-floor-per-token) for detailed instructions on how to configure this value via the `ArbOwner` precompile.
