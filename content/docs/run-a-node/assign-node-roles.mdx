...
| Batch poster                         | `--node.batch-poster.enable`                                                    | Funded parent-chain wallet; no bond                       | One active (Redis lock coordinates a set) | [Run a batch poster](/docs/run-a-node/run-batch-poster)                 |
...
| Validation server (split validation) | `nitro-val` binary + `--node.block-validator.validation-server.url` on the node | No wallet, no bond (all onchain action stays on the node) | Many (one node can use several)           | [Run a split validator node](/docs/run-a-node/run-split-validator-node) |
...
For the full setup, including snapshots, <a data-quicklook-from="state-pruning">pruning</a>, and Docker details, see [How to run a full node](/docs/run-a-node/run-full-node).
...
For chain-owner configuration, blob posting, and troubleshooting, see [Run a batch poster](/docs/run-a-node/run-batch-poster).
...
For Docker images, Helm charts, and the full setup, see [Run a split validator node](/docs/run-a-node/run-split-validator-node) and [Docker images and CLI binaries](/docs/run-a-node/nitro/docker-and-cli-binaries).
