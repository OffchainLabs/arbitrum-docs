...
A Nitro node's _role_ is not a separate piece of software. Every role in this article runs the same `nitro` binary; what makes a node a <Term id="sequencer">sequencer</Term>, a <Term id="batch-poster">batch poster</Term>, a <Term id="validator">validator</Term>, or a plain RPC node is the set of configuration flags you pass at startup. The exceptions are the <Term id="feed-relay">feed relay</Term> — a small standalone `relay` binary shipped inside the same Docker image—and the optional split-validation setup, which moves block validation into a separate `nitro-val` binary; both are built from the same repository.
