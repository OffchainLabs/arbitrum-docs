...
To learn how the <a data-quicklook-from="batch-poster">batch-poster</a> role relates to the other Nitro node roles, including its wallet requirements and the safety rules for running redundant posters, see [How to assign roles to a Nitro node](/docs/run-a-node/assign-node-roles).

The [batch poster](/docs/how-arbitrum-works/deep-dives/assertions) uses the following default configuration flags and values:
