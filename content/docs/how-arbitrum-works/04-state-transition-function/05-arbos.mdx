...
2. **Processing the batch posting report**:
   ...
   - **Update data units**:
     ...
     - Calculate the data units assigned to this update `U_upd`

       ```
       U_upd = U × (T_upd - T_prev) / (T - T_prev)
       ```
     ...
     - `T_upd`: Time when the update occurred

     - `T_prev`: Time of the previous update

     - Subtract `U_upd` from the total `U`
   ...
   - **Compute surplus and derivative**:
     ...
     - Surplus (`S`):
       ...
       ```
       S = Reimbursement Fund Balance - Amount Owed
       ```
     ...
     - Derivative of surplus (`D`):
       ...
       - `S_prev`: Surplus at the previous update
   ...
   - **Adjust price (`ΔP`)**:
     ...
     - Calculate the change in the data unit price:
       ...
       - `ΔP = (D' - D) × U_upd / (α + U_upd)`

       - `α`: Smoothing parameter to prevent abrupt changes
