...
In the past, Arbitrum chains ordered incoming transactions on a <a data-quicklook-from='first-come-first-serve-fcfs'>"First-Come, First-Serve (FCFS)"</a> basis. This ordering policy is simple to understand and implement, enabling fast block times (starting at @@l2BlockTimeMs=250@@ms and down to 100ms if desired), and protecting users from harmful types of MEV, such as front-running & sandwich attacks.
...
Timeboost is a _transaction ordering policy_. It's a set of rules that the [sequencer](/how-arbitrum-works/deep-dives/sequencer.mdx) of an Arbitrum chain is trusted to follow when ordering transactions submitted by users. In the near future, multiple sequencers will be able to enforce those rules with decentralized Timeboost.

For Arbitrum chains, the sequencer’s sole job is to take arriving, valid transactions from users, place them into an order dictated by the transaction ordering policy, and then publish the final sequence to a [real-time feed](/how-arbitrum-works/deep-dives/sequencer.mdx#persistence-and-broadcast) and in compressed batches to the chain’s data availability layer. Before Timeboost, the transaction ordering policy was FCFS, and Timeboost is a modified FCFS ordering policy.
