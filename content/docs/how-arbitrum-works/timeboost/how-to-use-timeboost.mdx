...
This how-to assumes that you're familiar with [How Timeboost works](/docs/how-arbitrum-works/timeboost/gentle-introduction)

<Callout type="info" title="Note about transferring express lane rights">
...
Remember that, by default, each round lasts <Var name="timeboostRoundSeconds" /> seconds, and the auction for a specific round closes <Var name="timeboostAuctionClosingSeconds" /> seconds before the start of the round. These default values can be configured on a chain using the `roundDurationSeconds` and `auctionClosingSeconds` parameters.
...
<Callout type="info" title="Bidding token in Arbitrum chains">
...
Remember that, by default, auctions for a given round open <Var name="timeboostRoundSeconds" /> seconds before that round starts and close <Var name="timeboostAuctionClosingSeconds" /> seconds before the round starts, so there might be no auctions opened at certain times.
...
<Callout type="idea" title="Fetching the minimum bid amount">
...
<Callout type="info" title="Timeboost-ing third party transactions">
...
<Callout type="info" title="Support for `eth_sendRawTransactionConditional`">
...
<Callout type="info" title="What happens if you're not the express lane controller?">
...
<Callout type="info" title="URL updates for historical bid data">
...
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- |
| `roundDurationSeconds`  | Duration of time that the sequencer will honor the express lane privileges for transactions signed by the current round’s express lane controller.                                                                               | <Var name="timeboostRoundSeconds" /> seconds                            |
| `auctionClosingSeconds` | Time before the start of the next round. The autonomous auctioneer will not accept bids during this time interval.                                                                                                               | <Var name="timeboostAuctionClosingSeconds" /> seconds                   |
...
| `nonExpressDelayMsec`   | The artificial delay applied to the arrival timestamp of non-express lane transactions _before_ the non-express lane transactions are sequenced.                                                                                 | 0.2 seconds, or <Var name="timeboostNonExpressDelayMs" /> milliseconds |
