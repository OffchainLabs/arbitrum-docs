...
<Term id="arbitrum-one">Arbitrum One</Term> has used Timeboost since April 2025. Timeboost auctions off a 60-second "<Term id="express-lane">express lane</Term>" in a sealed-bid, second-price auction and falls back to first-come, first-served (FCFS) ordering for everything else. This policy served to reduce latency-race spam and create the first sequencing revenue stream for the
<Term id="arbitrum-dao">Arbitrum DAO</Term>.
