...
`--execution.sequencer.max-block-speed` (default `@@l2BlockTimeMs=250@@ms`) is therefore the **minimum delay between blocks**, which caps block production at four blocks per second by default. It is not a block time:
