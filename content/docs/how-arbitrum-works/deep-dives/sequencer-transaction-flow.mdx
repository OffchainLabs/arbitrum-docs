...
First-come, first-served is the default ordering policy, and the FIFO behavior described on this page assumes it. Chains that enable <a data-quicklook-from="timeboost">Timeboost</a> modify the ordering: transactions from the current <a data-quicklook-from="express-lane-controller">express lane controller</a> are sequenced as soon as they arrive, while every other transaction has its arrival timestamp delayed (by default, @@timeboostNonExpressDelayMs=200@@ milliseconds) before taking its place in the queue. To learn how the <a data-quicklook-from="express-lane">express lane</a> and its auction work, see [Timeboost's gentle introduction](/docs/how-arbitrum-works/timeboost/gentle-introduction).
...
- [The Sequencer and censorship resistance](/docs/how-arbitrum-works/deep-dives/sequencer): feed, batch posting, finality, and the <a data-quicklook-from="delayed-inbox">delayed inbox</a> escape hatch
- [Finality](/docs/how-arbitrum-works/deep-dives/finality): what soft and <a data-quicklook-from="hard-finality">hard finality</a> guarantee, and which one to wait for
