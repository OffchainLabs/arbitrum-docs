...
- **Compression complexity**: The data posted to the parent chain is compressed using a general-purpose compression algorithm ([Brotli](/docs/how-arbitrum-works/deep-dives/batchposter)). The effectiveness of compression depends on shared patterns among transactions in a batch.
...
Batch posting costs reflect the actual cost a [batch poster](/docs/how-arbitrum-works/deep-dives/batchposter) pays to post batch data on the parent chain. Whenever a batch is posted, the parent chain contract that records it will send a special "batch posting report" message to the child chain <Term id="arbos">ArbOS</Term>, reporting who paid for the batch and the parent chain basefee at the time. This message is placed in the chain's [<Term id="delayed-inbox">delayed inbox</Term>](/docs/how-arbitrum-works/deep-dives/l1-to-l2-messaging#transacting-via-the-delayed-inbox) so that it will be delivered to the child chain ArbOS after some delay.
...
For an operational overview of how the Sequencer applies these levels, see [Compression](/docs/how-arbitrum-works/deep-dives/batchposter). Dynamic adjustments of compression levels are based on backlog size ($B$):
...
Whether a tip does anything depends on the chain's <Term id="transaction-ordering-policy">transaction ordering policy</Term> and on whether the chain collects tips at all. Today, the only ordering policy that uses tips to define order is PGA. Two settings control this, and a chain needs both before a tip changes anything:

1. **Tip collection.** ArbOS 61 added `setCollectTips` on the [`ArbOwner` precompile](/docs/arbitrum-essentials/precompiles/reference#arbowner). It ships disabled. Until a chain owner enables it, the chain ignores the tip and the sender pays the basefee alone.
...
| --------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| <Term id="first-come-first-serve-fcfs">FCFS</Term>, tips not collected | Nothing. Arrival time sets the order, and the sender pays the basefee. |
| [PGA](/docs/how-arbitrum-works/priority-gas-auction/pga), tips collected            | The tip sets the ordering position and the chain collects it.          |

Under PGA, the sequencer keys its priority queue on the <Term id="priority-fee">priority fee</Term>, computed per <Term id="eip-1559">EIP-1559</Term> as `min(max_priority_fee_per_gas, max_fee_per_gas - base_fee_per_gas)`. Paying more moves a transaction up the queue; it never lowers the basefee the transaction owes. A transaction that pays no tip still gets included, because the anti-starvation boost raises its position each round.

<Term id="arbitrum-one">Arbitrum One</Term> runs PGA and collects tips, sending 97% of the proceeds to the <Term id="arbitrum-dao">Arbitrum DAO</Term> treasury and 3% to the Arbitrum Developer Guild. To turn either setting on for your own chain, see [Priority fees collection](/docs/launch-arbitrum-chain/configuration/costs/priority-fees) and [PGA for Arbitrum chains](/docs/launch-arbitrum-chain/configuration/sequencer/pga).
