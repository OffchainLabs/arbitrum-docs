...
<Callout type="info" title="Validators and proposers serve different roles">

Validators validate transactions by computing the next <Term id="chain-state">chain state</Term> using the chain's [STF](/docs/how-arbitrum-works/deep-dives/stf), whereas proposers can also assert and challenge the chain state on the parent chain.
