...
3. **Cross-chain messaging**: Enabling cross-chain communication between parent and child chains for deposits, withdrawals, and <Term id="retryable-ticket">retryable tickets</Term>.
...
ArbOS manages child chain gas pricing and fee collection for both child chain execution costs and parent chain data posting costs. The child chain uses a dynamic basefee mechanism with multiple gas targets over different time windows to handle demand spikes while maintaining long-term chain capacity. Transactions pay fees to both the [batch poster](/docs/how-arbitrum-works/deep-dives/batchposter) (for parent chain data costs) and the network (for child chain execution).
