...
2. **<Term id="hard-finality">Hard finality</Term>**: Occurs when transactions are included in <Term id="batch">batches</Term> posted to and finalized on the <Term id="parent-chain">parent chain</Term>, providing strong security assurances. Final settlement on the parent chain happens via <Term id="assertion">assertions</Term> confirmed by the <Term id="arbitrum-rollup-protocol">Rollup protocol</Term>.
...
| Parent-chain data finality (batch posted and finalized on <Term id="L1">L1</Term>) | Transaction data is durably recorded and inherits the parent chain's consensus security | ~2 epochs, roughly 12–15 minutes (the batch poster defaults to referencing the L1 `safe` block)                                  |
| Full settlement (assertion confirmed)                                              | <Term id="L2">L2</Term> state is finalized and L2→L1 withdrawals can be executed        | A bounded <Term id="challenge-period">challenge period</Term> under <Term id="bold">BoLD</Term>, on the order of days on mainnet |
...
- **High latency**: Achieving hard finality takes longer because the parent chain must process and finalize batches. The <Term id="challenge-protocol">challenge protocol</Term> that backs this guarantee is described in BoLD.
...
- [Finality and chain reorganizations](/docs/how-arbitrum-works/reference/finality-and-reorgs) — the `latest`, `safe`, and `finalized` block tags, reorg depth, and which tag an indexer should wait on.
- [The Sequencer and censorship resistance](/docs/how-arbitrum-works/deep-dives/sequencer) — the real-time feed, batch posting, and the delayed inbox escape hatch.
- [Assertions](/docs/how-arbitrum-works/deep-dives/assertions) — how the Rollup protocol confirms child chain state and completes final settlement.
...
- [AnyTrust protocol](/docs/how-arbitrum-works/deep-dives/anytrust-protocol) — how a DAC and data availability certificates change the data availability guarantee.
- [Child to parent chain messaging](/docs/how-arbitrum-works/deep-dives/l2-to-l1-messaging) — the outbox and why withdrawals wait for assertion confirmation.
