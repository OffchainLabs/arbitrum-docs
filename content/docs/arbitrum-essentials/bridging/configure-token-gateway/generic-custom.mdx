...
In this how-to, you'll learn how to bridge your own token between Ethereum (parent chain) and Arbitrum (child chain), using [Arbitrum's generic-custom gateway](/docs/how-arbitrum-works/deep-dives/token-bridging#the-arbitrum-generic-custom-gateway). For alternative ways of bridging tokens, check out the [token bridging overview](/docs/how-arbitrum-works/deep-dives/token-bridging).

Familiarity with [Arbitrum's token bridge system](/docs/how-arbitrum-works/deep-dives/token-bridging), smart contracts, and blockchain development is expected. If you're new to blockchain development, consider reviewing our [Quickstart: Build a dApp with Arbitrum (Solidity, Hardhat)](/docs/build-decentralized-apps/quickstart-solidity-remix) before proceeding. We'll use [Arbitrum's SDK](https://github.com/OffchainLabs/arbitrum-sdk) throughout this how-to, although no prior knowledge is required.
...
As stated in the [token bridge conceptual page](/docs/how-arbitrum-works/deep-dives/token-bridging#the-arbitrum-generic-custom-gateway), there are a few prerequisites to keep in mind while using this method to make a token bridgeable.
...
For an example of bridging a token from the parent to the child chain using Arbitrum's SDK, check out [How to bridge tokens via Arbitrum's standard **ERC-20** gateway](/docs/arbitrum-essentials/bridging/deposit/tokens), specifically Steps 2-5.
