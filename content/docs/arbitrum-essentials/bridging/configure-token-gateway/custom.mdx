...
<VanillaAdmonition type="warning" title="Do you really need a custom gateway?">

Before implementing and deploying a custom gateway, strongly consider the current solutions that Arbitrum's token bridge provides: the [standard gateway](/docs/arbitrum-essentials/bridging/configure-token-gateway/standard) and the [generic-custom gateway](/docs/arbitrum-essentials/bridging/configure-token-gateway/generic-custom). These solutions cover the majority of bridging needs. If you're unsure about your approach, you can always ask for assistance on our [Discord server](https://discord.gg/arbitrum).

</VanillaAdmonition>
In this how-to, you'll learn how to bridge your own token between Ethereum (the parent chain) and Arbitrum (the child chain), using a custom gateway. For alternative ways of bridging tokens, check out the [token bridging overview](/docs/arbitrum-essentials/bridging/overview).

Familiarity with [Arbitrum's token bridge system](/docs/how-arbitrum-works/deep-dives/token-bridging), smart contracts, and decentralized application development is expected. If you're new to developing on Arbitrum, consider reviewing our [Quickstart: Build a dApp with Arbitrum (Solidity, Remix)](/docs/build-decentralized-apps/quickstart-solidity-remix) before proceeding. We'll use [Arbitrum's SDK](https://github.com/OffchainLabs/arbitrum-sdk) throughout this how-to, although no prior knowledge is required.
...
We won't go through deploying custom tokens in this how-to, but you can find a detailed explanation on the [generic-custom gateway setup page](/docs/arbitrum-essentials/bridging/configure-token-gateway/generic-custom).
...
As mentioned in [How to bridge tokens via Arbitrum's generic-custom gateway](/docs/arbitrum-essentials/bridging/configure-token-gateway/generic-custom), this action needs to be completed by the parent chain token, and we've implemented the function `registerTokenOnL2` to do it. Now we only need to call that function.
...
- [Deposit tokens to the child chain](/docs/arbitrum-essentials/bridging/deposit/tokens)
- [Withdraw tokens back to the parent chain](/docs/arbitrum-essentials/bridging/withdraw/tokens)

## Resources

1. [Concept page: Token Bridge](/docs/how-arbitrum-works/deep-dives/token-bridging)
2. [Arbitrum SDK](https://github.com/OffchainLabs/arbitrum-sdk)
3. [Token bridge contract addresses](/docs/arbitrum-essentials/reference/contract-addresses)
