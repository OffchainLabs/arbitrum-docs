...
- Awareness that child-to-parent messages require <Var name="arbOneDisputeWindowDays" /> days to finalize
...
`NodeInterface` is a "virtual" contract accessible at `0x00000000000000000000000000000000000000C8`. It isn't a true <Term id="precompile">precompile</Term> but provides Arbitrum-specific data without requiring a custom RPC.
...
2. **Challenge period**: Wait <Var name="arbOneDisputeWindowDays" /> days for the <Term id="assertion">assertion</Term> to finalize
...
// After <Var name="arbOneDisputeWindowDays" /> days, execute on parent chain using the steps above
```
...
| Waiting for finalization                 | The assertion containing the message is in the challenge period (<Var name="arbOneDisputeWindowDays" /> days) |
