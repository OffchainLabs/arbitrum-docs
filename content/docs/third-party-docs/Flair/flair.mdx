...
For a diagram of how those primitives fit together, see the [Flair documentation](https://docs.flair.dev/).
...
<VanillaAdmonition type="info" title="Boilerplate instructions will create a **new cluster**, generate **an API Key**, and set up a manifest.yml to index your **first contract** with **sample custom processor** scripts.">
...
```yaml
{ 'cluster': 'dev', 'namespace': 'my-awesome-arbitrum-indexing-dev', 'indexers': [{ 'chainId': 42161, 'enabled': true, 'ingestionFilterGroup': 'default', 'processingFilterGroup': 'default', 'sources': [
