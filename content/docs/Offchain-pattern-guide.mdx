...
| Use glossary terms            | Wrap terms defined in `content/glossary/` in `<Term>` | In the past, Arbitrum chains ordered incoming transactions on a `<Term id="first-come-first-serve-fcfs">First-Come, First-Serve (FCFS)</Term>` basis. | In the past, Arbitrum chains ordered incoming transactions on "First-Come, First-Serve (FCFS)" basis. |
...
- Use `<VanillaAdmonition type="…">` instead of `:::info` / `:::note` fences for callouts in MDX. The component is registered globally in `components/mdx.tsx`, so no import is needed.
